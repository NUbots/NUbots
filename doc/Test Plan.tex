...
\section{Modules}
	...
	\subsection{Platform Darwin HardwareIO}
		...
		\begin{itemize}
			...
			\item The system must take reasonable measures to prevent the robot from damaging itself
		...
		\begin{itemize}
			...
			\test{Ensure that the hardware is prevented from causing self harm}
			\begin{itemize}
				\item Generate a command which would cause the robot to damage itself
				\begin{itemize}
					\item This would be a motion which results in a limb doing a full revolution
				\end{itemize}
				\item Send the command to the hard
				\item Ensure that the command is executed such that the limb does not perform a full revolution
			\end{itemize}
		\end{itemize}
