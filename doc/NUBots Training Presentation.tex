...
\begin{frame}[fragile]
	...
	\begin{lstlisting}[language=nuclear]
		...
		void processImage(const Image& image) {
		    ...
		    data->something; // Access like a pointer * or ->.
		    
		    // theif now owns the data pointed at by data.
		    // data is now invalid. unique_ptr can only be
		    // copied using std::move(...).
		    std::unique_ptr<MyData> thief = std::move(data); 
		    thief->something; // Fine
		    data->something; // Error and very bad!
		} // thief releases memory here. 
	\end{lstlisting}
\end{frame}
