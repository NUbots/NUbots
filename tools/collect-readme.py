...
def check_for_readmes(path):
    ...
    for files in glob.glob(path):  # Glob (collect) for all files in the directory
        if "README.md" in files:  # Filter out the files to only include README.md files
            ...
            if (
                not parent_found
            ):  # Checks if a folder on the website already exists for that parent directory and creates it if not
                path_obj = Path(files)
    ...
    for item in glob.glob(parent_path + "*/"):
        ...
        if item.endswith("/"):
            ...
            for item2 in glob.glob(item + "*/"):
                ...
                if item2.endswith(
                    "/"
                ):  # Goes and checks all files for readme in their name, running it through the check_for_readmes function
...
def file_maker():
    global results, new_folder_name, path_obj, parent_folder, item_path, main_path, readmes_path, processed
    for file in results:
        if file == "None" or file in processed:
            ...
            slug = (
                "/readmes/" + folder4 + "/" + list(slug_obj.parts)[5]
            )  # Gets the slug for the MDX file, based on the file path
            if folder4 == "module":
                folder4 = ""
            else:
                folder4 = folder4 + " "
            markdown_content = (
                ...
                + parent_folder.title()
                + md_content_second
                + list(slug_obj.parts)[-3].title()
                + md_content_third
                + list(slug_obj.parts)[-3]
                + " "
                + folder4
                + md_content_fourth
                + slug
            ...
            with open(file, "r") as firstfile, open(
                readmes_path + "/" + list(slug_obj.parts)[4] + "/" + name_no_md + ".mdx", "a+"
            ) as secondfile:
...
readmes_path = input("Please provide the path to the 04-readmes folder. eg: /home/nubots/NUbook/src/book/04-readmes :")
NUbots_path = input("Please provide the path to the NUbots folder. eg: /home/nubots/NUbots/ :")
other_paths = input(
    "Please provide the path to any other folders you would like to search for README.md files in, excluding /modules, /nusight, /nuclear, /nusight2, /roles. eg: /home/nubots/NUbots/modules:"
)
list_of_dirs = [
    NUbots_path + "/module",
    NUbots_path + "/nuclear",
    NUbots_path + "/nusight2",
    NUbots_path + "/tools",
    NUbots_path + "/roles",
    other_paths,
