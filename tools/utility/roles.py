...
def dir_role_names(dir):
    ...
    return [
        os.path.splitext(f.replace(os.path.join(b.project_dir, "roles", ""), "").replace(os.path.sep, "-"))[0]
        for f in fnames
    ]
...
def role_folders():
    ...
    # Strip everything from file paths except role name and subdirectory in roles/
    return [
        os.path.splitext(d.replace(os.path.join(b.project_dir, "roles", ""), "").replace(os.path.sep, ""))[0]
        for d in dnames
    ]
