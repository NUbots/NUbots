...
def register(command):
    ...
    command.add_argument('-u', '--user', metavar='username', default='hive', help='the username to use when installing')
...
def run(ip_addr, hostname, config, scripts, user, toolchain, **kwargs):
    ...
    if toolchain:
        ...
        call(['rsync', '-avzuPlR', '--checksum', '-e ssh'] + libs + [target_dir + 'toolchain'])
