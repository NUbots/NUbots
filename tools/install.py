...
def register(command):
    ...
    command.add_argument(
        "ip_addr", metavar="ip_addr", help="the IP address of the target to install to"
    )

    command.add_argument(
        "hostname", metavar="hostname", help="the hostname of the target to install to"
    )
...
def run(ip_addr, hostname, config, scripts, user, toolchain, **kwargs):
    ...
    if toolchain:
        ...
        call(
            ["rsync", "-avzuPl", "--checksum", "-e ssh"]
            + libs
            + [target_dir + "toolchain"]
        )

        # Set rpath for all libs on the remote machine
        cprint(
            "Setting rpath for all toolchain libs to {0}".format(
                target_dir + "toolchain"
            ),
            "blue",
            attrs=["bold"],
    ...
    config_files = [
        os.path.relpath(c, build_dir)
        for c in b.cmake_cache["NUCLEAR_MODULE_DATA_FILES"]
    ]
