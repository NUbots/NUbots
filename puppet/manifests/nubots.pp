...
node nubotsvmbuild {
  $archs = {
    ...
    'fitpc2i'   => {'flags'       => ['-march=bonnell', '-mtune=bonnell', '-mno-movbe', '-mfxsr', '-mmmx', '-msahf', '-msse', '-msse2', '-msse3', '-mssse3', ],
                    ...
                    'environment' => {'TARGET' => 'YONAH', 'USE_THREAD' => '1', 'BINARY' => '32', 'NUM_THREADS' => '2', 'AUDIO' => 'PORTAUDIO', 'LDFLAGS' => '-m32', 'PKG_CONFIG_PATH' => '/usr/lib/i386-linux-gnu/pkgconfig', 'CCAS' => '/usr/bin/gcc', 'AS' => '/usr/bin/gcc', 'CCASFLAGS' => '-m32', },
                   },
  ...
  $archives = {
    ...
    'util-linux'   => {'url'         => 'https://www.kernel.org/pub/linux/utils/util-linux/v2.31/util-linux-2.31.tar.xz',
                       'args'        => { 'native'    => [ '--disable-all-programs', '--enable-libblkid', '--enable-libmount', '--enable-libuuid', '--without-python', '--with-bashcompletiondir=PREFIX/share/bash-completion/completions' ],
                                          ...
                                          'fitpc2i'   => [ '--disable-all-programs', '--enable-libblkid', '--enable-libmount', '--enable-libuuid', '--without-python', '--with-bashcompletiondir=PREFIX/share/bash-completion/completions' ], },
