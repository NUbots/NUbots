...
#
# find_package(Boost [version] [EXACT]      # Minimum or EXACT version e.g. 1.36.0 [REQUIRED]             # Fail with
# error if Boost is not found [COMPONENTS <libs>...] # Boost libraries by their canonical name )                      #
# e.g. "date_time" for "libboost_date_time"
#
# This module finds headers and requested component libraries OR a CMake package configuration file provided by a "Boost
# CMake" build.  For the latter case skip to the "Boost CMake" section below.  For the former case results are reported
# in variables::
#
# Boost_FOUND            - True if headers and requested libraries were found Boost_INCLUDE_DIRS     - Boost include
# directories Boost_LIBRARY_DIRS     - Link directories for Boost libraries Boost_LIBRARIES        - Boost component
# libraries to be linked Boost_<C>_FOUND        - True if component <C> was found (<C> is upper-case) Boost_<C>_LIBRARY
# - Libraries to link for component <C> (may include target_link_libraries debug/optimized keywords) Boost_VERSION -
# BOOST_VERSION value from boost/version.hpp Boost_LIB_VERSION      - Version string appended to library filenames
# Boost_MAJOR_VERSION    - Boost major version number (X in X.y.z) Boost_MINOR_VERSION    - Boost minor version number
# (Y in x.Y.z) Boost_SUBMINOR_VERSION - Boost subminor version number (Z in x.y.Z) Boost_LIB_DIAGNOSTIC_DEFINITIONS
# (Windows) - Pass to add_definitions() to have diagnostic information about Boost's automatic linking displayed during
# compilation
#
# This module reads hints about search locations from variables::
#
# BOOST_ROOT             - Preferred installation prefix (or BOOSTROOT) BOOST_INCLUDEDIR       - Preferred include
# directory e.g. <prefix>/include BOOST_LIBRARYDIR       - Preferred library directory e.g. <prefix>/lib
# Boost_NO_SYSTEM_PATHS  - Set to ON to disable searching in locations not specified by these hint variables. Default is
# OFF. Boost_ADDITIONAL_VERSIONS - List of Boost versions not known to this module (Boost install locations may contain
# the version)
#
# and saves search results persistently in CMake cache entries::
#
# Boost_INCLUDE_DIR         - Directory containing Boost headers Boost_LIBRARY_DIR_RELEASE - Directory containing
# release Boost libraries Boost_LIBRARY_DIR_DEBUG   - Directory containing debug Boost libraries Boost_<C>_LIBRARY_DEBUG
# - Component <C> library debug variant Boost_<C>_LIBRARY_RELEASE - Component <C> library release variant
#
# The following :prop_tgt:`IMPORTED` targets are also defined::
#
# Boost::boost                  - Target for header-only dependencies (Boost include directory) Boost::<C> - Target for
# specific component dependency (shared or static library); <C> is lower- case Boost::diagnostic_definitions - interface
# target to enable diagnostic information about Boost's automatic linking during compilation (adds BOOST_LIB_DIAGNOSTIC)
# Boost::disable_autolinking    - interface target to disable automatic linking with MSVC (adds BOOST_ALL_NO_LIB)
# Boost::dynamic_linking        - interface target to enable dynamic linking linking with MSVC (adds BOOST_ALL_DYN_LINK)
#
# Implicit dependencies such as Boost::filesystem requiring Boost::system will be automatically detected and satisfied,
# even if system is not specified when using find_package and if Boost::system is not added to target_link_libraries. If
# using Boost::thread, then Threads::Threads will also be added automatically.
#
# It is important to note that the imported targets behave differently than variables created by this module: multiple
# calls to find_package(Boost) in the same directory or sub-directories with different options (e.g. static or shared)
...
# use the above result variables.  Note that some hint names start in upper-case "BOOST".  One may specify these as
# environment variables if they are not specified as CMake variables or cache entries.
#
# This module first searches for the Boost header files using the above hint variables (excluding BOOST_LIBRARYDIR) and
# saves the result in Boost_INCLUDE_DIR.  Then it searches for requested component libraries using the above hints
# (excluding BOOST_INCLUDEDIR and Boost_ADDITIONAL_VERSIONS), "lib" directories near Boost_INCLUDE_DIR, and the library
# name configuration settings below.  It saves the library directories in Boost_LIBRARY_DIR_DEBUG and
# Boost_LIBRARY_DIR_RELEASE and individual library locations in Boost_<C>_LIBRARY_DEBUG and Boost_<C>_LIBRARY_RELEASE.
# When one changes settings used by previous searches in the same build tree (excluding environment variables) this
# module discards previous search results affected by the changes and searches again.
#
# Boost libraries come in many variants encoded in their file name. Users or projects may tell this module which variant
...
# Boost_USE_RELEASE_LIBS   - Set to ON or OFF to specify whether to search and use the release libraries.  Default is
# ON. Boost_USE_MULTITHREADED  - Set to OFF to use the non-multithreaded libraries ('mt' tag).  Default is ON.
# Boost_USE_STATIC_LIBS    - Set to ON to force the use of the static libraries.  Default is OFF.
# Boost_USE_STATIC_RUNTIME - Set to ON or OFF to specify whether to use libraries linked statically to the C++ runtime
# ('s' tag).  Default is platform dependent. Boost_USE_DEBUG_RUNTIME  - Set to ON or OFF to specify whether to use
# libraries linked to the MS debug C++ runtime ('g' tag).  Default is ON. Boost_USE_DEBUG_PYTHON   - Set to ON to use
# libraries compiled with a debug Python build ('y' tag). Default is OFF. Boost_USE_STLPORT        - Set to ON to use
# libraries compiled with STLPort ('p' tag).  Default is OFF. Boost_USE_STLPORT_DEPRECATED_NATIVE_IOSTREAMS - Set to ON
# to use libraries compiled with STLPort deprecated "native iostreams" ('n' tag).  Default is OFF. Boost_COMPILER - Set
# to the compiler-specific library suffix (e.g. "-gcc43").  Default is auto-computed for the C++ compiler in use. A list
# may be used if multiple compatible suffixes should be tested for, in decreasing order of preference. Boost_THREADAPI -
# Suffix for "thread" component library name, such as "pthread" or "win32".  Names with and without this suffix will
# both be tried. Boost_NAMESPACE          - Alternate namespace used to build boost with e.g. if set to "myboost", will
# search for myboost_thread instead of boost_thread.
#
# Other variables one may set to control this module are::
#
# Boost_DEBUG              - Set to ON to enable debug output from FindBoost. Please enable this before filing any bug
# report. Boost_DETAILED_FAILURE_MSG - Set to ON to add detailed information to the failure message even when the
# REQUIRED option is not given to the find_package call. Boost_REALPATH           - Set to ON to resolve symlinks for
# discovered libraries to assist with packaging.  For example, the "system" component library may be resolved to
# "/usr/lib/libboost_system.so.1.42.0" instead of "/usr/lib/libboost_system.so".  This does not affect linking and
# should not be enabled unless the user needs this information. Boost_LIBRARY_DIR        - Default value for
# Boost_LIBRARY_DIR_RELEASE and Boost_LIBRARY_DIR_DEBUG.
#
# On Visual Studio and Borland compilers Boost headers request automatic linking to corresponding libraries.  This
# requires matching libraries to be linked explicitly or available in the link library search path. In this case setting
# Boost_USE_STATIC_LIBS to OFF may not achieve dynamic linking.  Boost automatic linking typically requests static
...
#
# find_package(Boost 1.36.0) if(Boost_FOUND) include_directories(${Boost_INCLUDE_DIRS}) add_executable(foo foo.cc)
# endif()
#
# Example to find Boost libraries and use imported targets::
#
# find_package(Boost 1.56 REQUIRED COMPONENTS date_time filesystem iostreams) add_executable(foo foo.cc)
# target_link_libraries(foo Boost::date_time Boost::filesystem Boost::iostreams)
#
# Example to find Boost headers and some *static* (release only) libraries::
#
# set(Boost_USE_STATIC_LIBS        ON)  # only find static libs set(Boost_USE_DEBUG_LIBS         OFF) # ignore debug
# libs and set(Boost_USE_RELEASE_LIBS       ON)  # only find release libs set(Boost_USE_MULTITHREADED      ON)
# set(Boost_USE_STATIC_RUNTIME    OFF) find_package(Boost 1.36.0 COMPONENTS date_time filesystem system ...)
# if(Boost_FOUND) include_directories(${Boost_INCLUDE_DIRS}) add_executable(foo foo.cc) target_link_libraries(foo
# ${Boost_LIBRARIES}) endif()
#
# Boost CMake
# ^^^^^^^^^^^
#
# If Boost was built using the boost-cmake project it provides a package configuration file for use with find_package's
# Config mode.  This module looks for the package configuration file called BoostConfig.cmake or boost-config.cmake and
# stores the result in cache entry "Boost_DIR".  If found, the package configuration file is loaded and this module
# returns with no further action.  See documentation of the Boost CMake package configuration for details on what it
# provides.
#
...
# -------------------------------------------------------------------------------
# Before we go searching, check whether boost-cmake is available, unless the user specifically asked NOT to search for
# boost-cmake.
#
# If Boost_DIR is set, this behaves as any find_package call would. If not, it looks at BOOST_ROOT and BOOSTROOT to find
# Boost.
#
if(NOT Boost_NO_BOOST_CMAKE)
  # If Boost_DIR is not set, look for BOOSTROOT and BOOST_ROOT as alternatives, since these are more conventional for
  # Boost.
  ...
  # Boost_FIND_xxxxx variables, so there is no need to delegate them to this find_package call.
  find_package(Boost QUIET NO_MODULE)
  mark_as_advanced(Boost_DIR)

  # If we found boost-cmake, then we're done.  Print out what we found. Otherwise let the rest of the module try to find
  # it.
  if(Boost_FOUND)
...
macro(_Boost_ADJUST_LIB_VARS basename)
  if(Boost_INCLUDE_DIR)
    if(Boost_${basename}_LIBRARY_DEBUG AND Boost_${basename}_LIBRARY_RELEASE)
      # if the generator supports configuration types then set optimized and debug libraries, or if the CMAKE_BUILD_TYPE
      # has a value
      if(CMAKE_CONFIGURATION_TYPES OR CMAKE_BUILD_TYPE)
        set(Boost_${basename}_LIBRARY optimized ${Boost_${basename}_LIBRARY_RELEASE} debug
                                      ${Boost_${basename}_LIBRARY_DEBUG})
      else()
        # if there are no configuration types and CMAKE_BUILD_TYPE has no value then just use the release libraries
...
# Detect changes in used variables. Compares the current variable value with the last one. In short form: v != v_LAST ->
# CHANGED = 1 v is defined, v_LAST not         -> CHANGED = 1 v is not defined, but v_LAST is  -> CHANGED = 1 otherwise
# -> CHANGED = 0 CHANGED is returned in variable named ${changed_var}
...
function(_Boost_COMPILER_DUMPVERSION _OUTPUT_VERSION)
  string(REGEX REPLACE "([0-9]+)\\.([0-9]+)(\\.[0-9]+)?" "\\1\\2" _boost_COMPILER_VERSION ${CMAKE_CXX_COMPILER_VERSION})

  set(${_OUTPUT_VERSION} ${_boost_COMPILER_VERSION} PARENT_SCOPE)
endfunction()

#
# Take a list of libraries with "thread" in it and prepend duplicates with "thread_${Boost_THREADAPI}" at the front of
# the list
#
...
# Guesses Boost's compiler prefix used in built library names Returns the guess by setting the variable pointed to by
# _ret
        ...
        if(APPLE)
          if(Boost_MINOR_VERSION)
            if(${Boost_MINOR_VERSION} GREATER 35)
              # In Boost 1.36.0 and newer, the mangled compiler name used on Mac OS X/Darwin is "xgcc".
...
#
# Get component dependencies.  Requires the dependencies to have been defined for the Boost release version.
#
...
function(_Boost_COMPONENT_DEPENDENCIES component _ret)
  ...
  #
  # The output may be added in a new block below.  If it's the same as the previous release, simply update the version
  # range of the block for the previous release.  Also check if any new components have been added, and add any new
  # components to _Boost_COMPONENT_HEADERS.
  #
  # This information was originally generated by running BoostScanDeps.cmake against every boost release to date
  # supported by FindBoost:
  #
  # % for version in /path/to/boost/sources/* do cmake -DBOOST_DIR=$version -P Utilities/Scripts/BoostScanDeps.cmake
  # done
  #
  ...
  # * Indent s;^set(;    set(;;
  # * Add conditionals s;Scanning /path/to/boost/sources/boost_\(.*\)_\(.*\)_\(.*);  elseif(NOT Boost_VERSION
  #   VERSION_LESS \10\20\3 AND Boost_VERSION VERSION_LESS xxxx);
  #
  # This results in the logic seen below, but will require the xxxx replacing with the following Boost release version
  # (or the next minor version to be released, e.g. 1.59 was the latest at the time of writing, making 1.60 the next, so
  # 106000 is the needed version number).  Identical consecutive releases were then merged together by updating the end
  # range of the first block and removing the following redundant blocks.
  #
  # Running the script against all historical releases should be required only if the BoostScanDeps.cmake script logic
  # is changed. The addition of a new release should only require it to be run against the new release.
  ...
  elseif(NOT Boost_VERSION VERSION_LESS 106300 AND Boost_VERSION VERSION_LESS 106500)
    ...
    set(_Boost_COROUTINE_DEPENDENCIES context system)
    set(
      _Boost_COROUTINE2_DEPENDENCIES
      context
      fiber
      thread
      chrono
...
#
# Get component headers.  This is the primary header (or headers) for a given component, and is used to check that the
# headers are present as well as the library itself as an extra sanity check of the build environment.
#
# component - the component to check _hdrs
#
function(_Boost_COMPONENT_HEADERS component _hdrs)
  # Note: new boost components will require adding here.  The header must be present in all versions of Boost providing
  # a library.
...
#
# Sets _Boost_${COMPONENT}_DEPENDENCIES for each required component, plus _Boost_IMPORTED_TARGETS (TRUE if imported
# targets should be defined; FALSE if dependency information is unavailable).
#
# componentvar - the component list variable name extravar - the indirect dependency list variable name
#
function(_Boost_MISSING_DEPENDENCIES componentvar extravar)
  # _boost_unprocessed_components - list of components requiring processing _boost_processed_components - components
  # already processed (or currently being processed) _boost_new_components - new components discovered for future
  # processing
  #
  ...
  while(_boost_unprocessed_components)
    ...
    foreach(component ${_boost_unprocessed_components})
      ...
      foreach(componentdep ${_Boost_${uppercomponent}_DEPENDENCIES})
        if(
          NOT
          ("${componentdep}" IN_LIST _boost_processed_components OR "${componentdep}" IN_LIST _boost_new_components)
        )
...
#
# Some boost libraries may require particular set of compler features. The very first one was `boost::fiber` introduced
# in Boost 1.62. One can check required compiler features of it in `${Boost_ROOT}/libs/fiber/build/Jamfile.v2`.
#
function(_Boost_COMPILER_FEATURES component _ret)
  # Boost >= 1.62 and < 1.65
  if(NOT Boost_VERSION VERSION_LESS 106200 AND Boost_VERSION VERSION_LESS 106500)
    set(
      _Boost_FIBER_COMPILER_FEATURES
      cxx_alias_templates
      cxx_auto_type
      cxx_constexpr
...
#
# Prebuilt windows binaries (https://sourceforge.net/projects/boost/files/boost-binaries/) have library directories
# named using MSVC compiler version and architecture. This function would append corresponding directories if MSVC is a
# current compiler, so having `BOOST_ROOT` would be enough to specify to find everything.
#
...
if(Boost_FIND_VERSION_EXACT)
  # The version may appear in a directory with or without the patch level, even when the patch level is non-zero.
  set(_boost_TEST_VERSIONS "${Boost_FIND_VERSION_MAJOR}.${Boost_FIND_VERSION_MINOR}.${Boost_FIND_VERSION_PATCH}"
                           "${Boost_FIND_VERSION_MAJOR}.${Boost_FIND_VERSION_MINOR}")
else()
  # The user has not requested an exact version.  Among known versions, find those that are acceptable to the user
  # request.
  #
  # Note: When adding a new Boost release, also update the dependency information in _Boost_COMPONENT_DEPENDENCIES and
  # _Boost_COMPONENT_HEADERS.  See the instructions at the top of _Boost_COMPONENT_DEPENDENCIES.
  set(
...
# The reason that we failed to find Boost. This will be set to a user-friendly message when we fail to find some
# necessary piece of Boost.
...
if(Boost_DEBUG)
  ...
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "_boost_TEST_VERSIONS = ${_boost_TEST_VERSIONS}"
  )
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] "
  ...
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "Boost_USE_STATIC_LIBS = ${Boost_USE_STATIC_LIBS}"
  )
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] "
  ...
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "Boost_NO_SYSTEM_PATHS = ${Boost_NO_SYSTEM_PATHS}"
  )
endif()

# Supply Boost_LIB_DIAGNOSTIC_DEFINITIONS as a convenience target. It will only contain any interface definitions on
# WIN32, but is created on all platforms to keep end user code free from platform dependent code.  Also provide
...
if(WIN32)
  # In windows, automatic linking is performed, so you do not have to specify the libraries.  If you are linking to a
  # dynamic runtime, then you can choose to link to either a static or a dynamic Boost library, the default is to do a
  # static link.  You can alter this for a specific library "whatever" by defining BOOST_WHATEVER_DYN_LINK to force
  # Boost library "whatever" to be linked dynamically.  Alternatively you can force all Boost libraries to dynamic link
  # by defining BOOST_ALL_DYN_LINK.

  # This feature can be disabled for Boost library "whatever" by defining BOOST_WHATEVER_NO_LIB, or for all of Boost by
  # defining BOOST_ALL_NO_LIB.

  # If you want to observe which libraries are being linked against then defining BOOST_LIB_DIAGNOSTIC will cause the
  # auto-linking code to emit a #pragma message each time a library is selected for linking.
...
if(Boost_DEBUG)
  message(
    STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "Declared as CMake or Environmental Variables:"
  )
  message(STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "  BOOST_ROOT = ${BOOST_ROOT}")
  message(
    STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "  BOOST_INCLUDEDIR = ${BOOST_INCLUDEDIR}"
  )
  message(
    STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "  BOOST_LIBRARYDIR = ${BOOST_LIBRARYDIR}"
  )
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "_boost_TEST_VERSIONS = ${_boost_TEST_VERSIONS}"
...
if(NOT Boost_INCLUDE_DIR)
  ...
  # Try to find Boost by stepping backwards through the Boost versions we know about. Build a list of path suffixes for
  # each version.
  set(_boost_PATH_SUFFIXES)
  foreach(_boost_VER ${_boost_TEST_VERSIONS})
    # Add in a path suffix, based on the required version, ideally we could read this from version.hpp, but for that to
    # work we'd need to know the include dir already
    ...
    list(
      APPEND
        _boost_PATH_SUFFIXES "boost-${_boost_BOOSTIFIED_VERSION}" "boost_${_boost_BOOSTIFIED_VERSION}"
        "boost/boost-${_boost_BOOSTIFIED_VERSION}" "boost/boost_${_boost_BOOSTIFIED_VERSION}"
    )

  endforeach()

  if(Boost_DEBUG)
    message(STATUS "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "Include debugging info:")
    message(
      STATUS
        "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] "
    ...
    message(
      STATUS
        "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "  _boost_PATH_SUFFIXES = ${_boost_PATH_SUFFIXES}"
    )
  endif()
...
# Set Boost_FOUND based only on header location and version. It will be updated below for component libraries.
if(Boost_INCLUDE_DIR)
  if(Boost_DEBUG)
    message(
      STATUS
  ...
  file(
    STRINGS "${Boost_INCLUDE_DIR}/boost/version.hpp" _boost_VERSION_HPP_CONTENTS
  ...
  string(
    APPEND Boost_ERROR_REASON
    "Boost version: ${Boost_MAJOR_VERSION}.${Boost_MINOR_VERSION}.${Boost_SUBMINOR_VERSION}\nBoost include path: ${Boost_INCLUDE_DIR}"
  )
  if(Boost_DEBUG)
    message(
      STATUS
        "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "version.hpp reveals boost "
  ...
  if(Boost_FIND_VERSION)
    ...
    elseif(Boost_FIND_VERSION_EXACT AND NOT "${_Boost_VERSION}" VERSION_EQUAL "${Boost_FIND_VERSION}")
      set(Boost_FOUND 0)
...
if((GHSMULTI AND Boost_USE_STATIC_LIBS) OR (WIN32 AND Boost_USE_STATIC_LIBS AND NOT CYGWIN))
...
if(Boost_COMPILER)
  ...
  # Attempt to guess the compiler suffix NOTE: this is not perfect yet, if you experience any issues please report them
  # and use the Boost_COMPILER variable to work around the problems.
  _boost_guess_compiler_prefix(_boost_COMPILER)
  if(Boost_DEBUG)
    message(
      STATUS
        "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "guessed _boost_COMPILER = ${_boost_COMPILER}"
    )
  endif()
endif()
...
if(Boost_DEBUG)
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "_boost_MULTITHREADED = ${_boost_MULTITHREADED}"
  )
endif()

# ======================
# Systematically build up the Boost ABI tag http://boost.org/doc/libs/1_41_0/more/getting_started/windows.html#library-
# naming
set(_boost_RELEASE_ABI_TAG "-")
set(_boost_DEBUG_ABI_TAG "-")
# Key       Use this library when: s        linking statically to the C++ standard library and compiler runtime support
# libraries.
...
if(Boost_DEBUG)
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "_boost_RELEASE_ABI_TAG = ${_boost_RELEASE_ABI_TAG}"
  )
  message(
    STATUS
      "[ ${CMAKE_CURRENT_LIST_FILE}:${CMAKE_CURRENT_LIST_LINE} ] " "_boost_DEBUG_ABI_TAG = ${_boost_DEBUG_ABI_TAG}"
  )
endif()
...
foreach(c DEBUG RELEASE)
  ...
  # Clear Boost_LIBRARY_DIR_${c} if it did not change but other input affecting the location did.  We will find a new
  # one based on the new inputs.
  ...
  if(Boost_LIBRARY_DIR_${c})
    ...
    list(
      APPEND
        _boost_LIBRARY_SEARCH_DIRS_${c} ${Boost_INCLUDE_DIR}/lib ${Boost_INCLUDE_DIR}/../lib
...
# The previous behavior of FindBoost when Boost_USE_STATIC_LIBS was enabled on WIN32 was to: 1. Search for static libs
# compiled against a SHARED C++ standard runtime library (use if found) 2. Search for static libs compiled against a
# STATIC C++ standard runtime library (use if found) We maintain this behavior since changing it could break people's
# builds. To disable the ambiguous behavior, the user need only set Boost_USE_STATIC_RUNTIME either ON or OFF.
...
# On versions < 1.35, remove the System library from the considered list since it wasn't added until 1.35.
...
foreach(COMPONENT ${Boost_FIND_COMPONENTS})
  ...
  if(${COMPONENT} STREQUAL "mpi" OR ${COMPONENT} STREQUAL "mpi_python" OR ${COMPONENT} STREQUAL "graph_parallel")
  ...
  # if Boost_LIBRARY_DIR_RELEASE is not defined, but Boost_LIBRARY_DIR_DEBUG is, look there first for RELEASE libs
  ...
  # if Boost_LIBRARY_DIR_DEBUG is not defined, but Boost_LIBRARY_DIR_RELEASE is, look there first for DEBUG libs
...
# The above setting of Boost_FOUND was based only on the header files. Update it for the requested component libraries.
if(Boost_FOUND)
  ...
  if(_Boost_MISSING_COMPONENTS)
    set(Boost_FOUND 0)
    # We were unable to find some libraries, so generate a sensible error message that lists the libraries we were
    # unable to find.
  ...
  if(NOT Boost_LIBRARY_DIRS AND NOT _boost_CHECKED_COMPONENT)
    # Compatibility Code for backwards compatibility with CMake 2.4's FindBoost module.

    # Look for the boost library path. Note that the user may not have installed any libraries so it is quite possible
    # the Boost_LIBRARY_DIRS may not exist.
    ...
    if(_Boost_IMPORTED_TARGETS AND NOT TARGET Boost::${COMPONENT})
      ...
      if(Boost_${UPPERCOMPONENT}_FOUND)
        if(Boost_USE_STATIC_LIBS)
          ...
          # Even if Boost_USE_STATIC_LIBS is OFF, we might have static libraries as a result.
          add_library(Boost::${COMPONENT} UNKNOWN IMPORTED)
        endif()
        if(Boost_INCLUDE_DIRS)
          set_target_properties(Boost::${COMPONENT} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${Boost_INCLUDE_DIRS}")
        endif()
        if(EXISTS "${Boost_${UPPERCOMPONENT}_LIBRARY}")
          set_target_properties(
            Boost::${COMPONENT}
            PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES "CXX" IMPORTED_LOCATION "${Boost_${UPPERCOMPONENT}_LIBRARY}"
          )
        endif()
        if(EXISTS "${Boost_${UPPERCOMPONENT}_LIBRARY_RELEASE}")
          set_property(TARGET Boost::${COMPONENT} APPEND PROPERTY IMPORTED_CONFIGURATIONS RELEASE)
          set_target_properties(
            Boost::${COMPONENT}
            PROPERTIES
              IMPORTED_LINK_INTERFACE_LANGUAGES_RELEASE "CXX" IMPORTED_LOCATION_RELEASE
              "${Boost_${UPPERCOMPONENT}_LIBRARY_RELEASE}"
          )
        endif()
        if(EXISTS "${Boost_${UPPERCOMPONENT}_LIBRARY_DEBUG}")
          set_property(TARGET Boost::${COMPONENT} APPEND PROPERTY IMPORTED_CONFIGURATIONS DEBUG)
          set_target_properties(
            Boost::${COMPONENT}
            PROPERTIES
              IMPORTED_LINK_INTERFACE_LANGUAGES_DEBUG "CXX" IMPORTED_LOCATION_DEBUG
              "${Boost_${UPPERCOMPONENT}_LIBRARY_DEBUG}"
          )
        endif()
        if(_Boost_${UPPERCOMPONENT}_DEPENDENCIES)
          ...
          set_target_properties(
            Boost::${COMPONENT}
            PROPERTIES INTERFACE_LINK_LIBRARIES "${_Boost_${UPPERCOMPONENT}_TARGET_DEPENDENCIES}"
          )
        endif()
        if(_Boost_${UPPERCOMPONENT}_COMPILER_FEATURES)
          set_target_properties(
            Boost::${COMPONENT}
            PROPERTIES INTERFACE_COMPILE_FEATURES "${_Boost_${UPPERCOMPONENT}_COMPILER_FEATURES}"
          )
        endif()
      endif()
    endif()
...
if(Boost_FOUND)
  ...
  if(Boost_FIND_REQUIRED)
    ...
    if(NOT Boost_FIND_QUIETLY)
      # we opt not to automatically output Boost_ERROR_REASON here as it could be quite lengthy and somewhat imposing in
      # its requests Since Boost is not always a required dependency we'll leave this up to the end-user.
