...
sed -i 's/^\/\* #undef ARMA_USE_LAPACK \*\//#define ARMA_USE_LAPACK/' tmp/build/usr/include/armadillo_bits/config.hpp
sed -i 's/^#define ARMA_USE_WRAPPER/\/\/ #define ARMA_USE_WRAPPER/'   tmp/build/usr/include/armadillo_bits/config.hpp
sed -i 's/^\/\/ #define ARMA_USE_CXX11/#define ARMA_USE_CXX11/'       tmp/build/usr/include/armadillo_bits/config.hpp
sed -i 's/^\/\/ #define ARMA_USE_U64S64/#define ARMA_USE_U64S64/'     tmp/build/usr/include/armadillo_bits/config.hpp
...
*/
  $arma_pkg = 'armadillo_4.650.3_i386.deb'

  wget::fetch { "${arma_pkg}":
  	require    => [Package['libblas-dev'], Package['liblapack-dev']],
    destination => "/tmp/${arma_pkg}",
    source      => "https://dl.dropboxusercontent.com/s/73b61hgbcffkogs/${arma_pkg}",
  }
  ~> package { 'armadillo':
    provider => 'dpkg',
    ensure => 'latest',
