...
execute_process(COMMAND "/nubots/toolchain/find_robot_hosts.sh"
                OUTPUT_VARIABLE HOSTS)
...
foreach(host_pair ${KNOWN_HOSTS})
  ...
  foreach(config "" n u o i t)
    ...
    # Make our installer The install script expects an IP address to install to
    # and a hostname to determine config files to install. IP address is
    # represented by the short-form hostname (e.g. d1) Hostname is represented
    # by the long- form hostname (e.g. darwin1)
    if(config STREQUAL "t")
      add_custom_target(
        "${target}"
        USES_TERMINAL
        COMMAND
          ${PYTHON_EXECUTABLE} "${CMAKE_SOURCE_DIR}/nuclear/b.py" "install"
          "${host}" "${alias}" "--user=${user}" "--toolchain"
      ...
      add_custom_target(
        ...
        COMMAND
          ${PYTHON_EXECUTABLE} "${CMAKE_SOURCE_DIR}/nuclear/b.py" "install"
          "${host}" "${alias}" "--config=${config}" "--user=${user}"
        DEPENDS ${NUCLEAR_ROLES} "${CMAKE_SOURCE_DIR}/tools/install.py")
    endif()
