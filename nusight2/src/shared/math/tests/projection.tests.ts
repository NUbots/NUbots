...
describe("Projection", () => {
  describe("should be the same value in round trip for", () => {
    describe("when normalised using:", () => {
      it("RGB10FOV Parameters", () => {
        runRoundTrip(RGB10FOV.lens, RGB10FOV.dimensions);
      });
      it("RGB40FOV Parameters", () => {
        runRoundTrip(RGB40FOV.lens, RGB40FOV.dimensions);
      });
      it("Thermal10FOV Parameters", () => {
        runRoundTrip(Thermal10FOV.lens, Thermal10FOV.dimensions);
      });
      it("Thermal40FOV Parameters", () => {
        runRoundTrip(Thermal40FOV.lens, Thermal40FOV.dimensions);
      });
      it("CentreWide Parameters", () => {
        runRoundTrip(CenterWide.lens, CenterWide.dimensions);
      });
    });
    describe("when unnormalised using:", () => {
      it("RGB10FOV Parameters", () => {
        runRoundTrip(RGB10FOV.lens, RGB10FOV.dimensions, false);
      });
      it("RGB40FOV Parameters", () => {
        runRoundTrip(RGB40FOV.lens, RGB40FOV.dimensions, false);
      });
      it("Thermal10FOV Parameters", () => {
        runRoundTrip(Thermal10FOV.lens, Thermal10FOV.dimensions, false);
      });
      it("Thermal40FOV Parameters", () => {
        runRoundTrip(Thermal40FOV.lens, Thermal40FOV.dimensions, false);
      });
      it("CentreWide Parameters", () => {
        runRoundTrip(CenterWide.lens, CenterWide.dimensions, false);
      });
    });
  });
});
