...
import { VisionCameraViewProps } from "../../view";
import { VisionCameraView } from "../../view";
...
export const WithVisionRenderables: Story = {
  render: ({ objectFit, viewType, allowPanAndZoom }) => {
    ...
    const box = observable<{ model: VisionCameraModel | undefined }>({
      model: undefined,
    });
  ...
  render: (_props) => {
    ...
    const Component = observer(() => {
      ...
      return (
        ...
        >
          {box.model ? <VisionCameraView objectFit="fill" model={box.model} robot={robot} /> : null}
        </div>
      );
    });
