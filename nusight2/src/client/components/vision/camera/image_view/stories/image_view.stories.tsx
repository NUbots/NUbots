...
const createModel = () => observable.box<Image | undefined>(undefined, { deep: false });

export const JPEG: Story = {
  name: "JPEG",
  render: () => {
    const model = createModel();
    loadImageElement(jpegUrl, ImageFormat.JPEG).then(action((image: Image) => model.set(image)));
    return <Observer>{() => <>{model.get() ? <ImageView image={model.get()!} /> : null}</>}</Observer>;
  },
};
...
export const JPRG: Story = {
  ...
  render: () => {
    const model = createModel();
    loadImageElement(jprgUrl, ImageFormat.JPRG).then(action((image: Image) => model.set(image)));
    return <Observer>{() => <>{model.get() ? <ImageView image={model.get()!} /> : null}</>}</Observer>;
  },
};
...
export const RGGB: Story = {
  ...
  render: () => {
    const model = createModel();
    loadImageElement(rggbUrl, ImageFormat.RGGB).then(action((image: Image) => model.set(image)));
    return <Observer>{() => <>{model.get() ? <ImageView image={model.get()!} /> : null}</>}</Observer>;
  },
};
...
export const RGGBBuffer: Story = {
  name: "RGGB Buffer",
  render: () => {
    const model = useLocalStore(createModel);
    ...
    React.useEffect(() => {
      loadImageData(rggbBinUrl, 600, 480, ImageFormat.RGGB).then(action((image: Image) => model.set(image)));
    });
    return <Observer>{() => <>{model.get() ? <ImageView image={model.get()!} /> : null}</>}</Observer>;
  },
};
