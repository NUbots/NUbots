...
const NavigationItemView = ({ url, Icon, children }: NavigationItemViewProps) => (
  <li className="whitespace-nowrap">
    <NavLink
      className={({ isActive }) => {
        return `p-4 flex flex-col items-center text-gray-300 text-xs leading-5 transition-colors ease-out duration-0 hover:bg-gray-700 focus:bg-accent ${isActive ? "!bg-accent" : ""
          }`;
      }}
      to={url}
    >
