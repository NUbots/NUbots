    ...
    <div className="p-4 border border-black dark:border-white rounded-lg w-full">
      ...
      <ul className="space-y-2">
        {Object.entries(robot.motors).map(([jointName, motor]) => {
          const formattedLabel = jointName.replace(/([a-z])([A-Z])/g, "$1 $2")
                                          .replace(/^./, (match) => match.toUpperCase());
          return (
            <li key={jointName} className="flex justify-between items-center p-2 border-black dark:border-white border-b">
