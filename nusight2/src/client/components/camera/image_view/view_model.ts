...
export class ImageViewModel {
  ...
  constructor(source: Image, canvas?: Canvas) {
    this.canvas = canvas;
    this.source = source;
  }

  static of(source: Image, canvas?: Canvas) {
    return new ImageViewModel(source, canvas);
  }

  private readonly geometry = planeGeometry(() => {
    // Aspect ratio of canvas / image
    const canvasRatio = this.canvas && this.canvas.width !== 0 ? this.canvas.width / this.canvas.height : 1;
  ...
  private readonly bayerMaterial = shaderMaterial((firstRed: [number, number], mosaicSize: number) => ({
    ...
    uniforms: {
      ...
      sourceSize: {
        value: [this.source.width, this.source.height, 1 / this.source.width, 1 / this.source.height],
      },
  ...
  private readonly bitmapTexture = imageTexture(({ bitmap, width, height, format }: BitmapImage) => ({
    image: bitmap,
    width,
    height,
    format: this.textureFormat(format),
