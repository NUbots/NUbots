 ...
 */
export function ProviderGroupView({ group }: ProviderGroupViewProps) {
  const providerIds = (group.providers ?? []).map((p) => p.id);
  const subtasks = group.subtasks ?? [];

  return (
    <div className="border rounded p-2 bg-auto-surface-2 space-y-2 min-w-[200px]" data-testid="provider-group">
      <div className="font-semibold text-sm mb-1">{group.type || `Group ${group}`}</div>
