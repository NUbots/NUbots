...
describe("NUsightNetwork", () => {
  ...
  it("emit() creates a packet for the given message and forwards it to NUClearNet", () => {
    ...
    const expectedPayload = Say.encode(data).finish();

    nusightNetwork.emit(new Say(data));
    ...
    nusightNetwork.emit(new Say(data), { reliable: true, target: "nusight" });
    expect(nuclearnetClient.send).toHaveBeenCalledWith({
      type: "message.output.Say",
      payload: expectedPayload as Buffer,
      reliable: true,
  ...
  it("onNUClearMessage() registers a callback for the given message type", () => {
    ...
    expect(nusightNetwork.onNUClearMessage(Say, jest.fn())).toBe(off);
    expect(nuclearnetClient.on).toHaveBeenCalledWith("message.output.Say", expect.any(Function));

    const off2 = jest.fn();
    nuclearnetClient.on.mockReturnValue(off2);

    expect(nusightNetwork.onNUClearMessage({ type: Say, subtype: 1 }, jest.fn())).toBe(off2);
    expect(nuclearnetClient.on).toHaveBeenCalledWith("message.output.Say#1", expect.any(Function));
  });
});
