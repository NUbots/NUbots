...
export class WebSocketProxyNUClearNetServer {
  ...
  static of(server: WebSocketServer, { fakeNetworking, connectionOpts }: Opts): WebSocketProxyNUClearNetServer {
...
class PacketProcessor {
  ...
  static of(socket: WebSocket) {
    return new PacketProcessor(socket, NodeSystemClock, new LruPriorityQueue({ capacityPerKey: 2 }), {
      outgoingLimit: 10,
      timeout: 5,
    });
  }
