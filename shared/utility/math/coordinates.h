...
namespace math {
    ...
    namespace coordinates {
        inline Eigen::Vector3d sphericalToCartesian(const Eigen::Vector3d& sphericalCoordinates) {
            ...
            double cos_theta = std::cos(sphericalCoordinates[1]);
            double sin_theta = std::sin(sphericalCoordinates[1]);
            double cos_phi   = std::cos(sphericalCoordinates[2]);
            double sin_phi   = std::sin(sphericalCoordinates[2]);
        ...
        inline Eigen::Vector3d cartesianToSpherical(const Eigen::Vector3d& cartesianCoordinates) {
            ...
            result[0] = std::sqrt(x * x + y * y + z * z);  // r
            result[1] = std::atan2(y, x);                  // theta
            if (result[0] == 0) {
                result[2] = 0;
            }
            else {
                result[2] = std::asin(z / (result[0]));  // phi
            }
