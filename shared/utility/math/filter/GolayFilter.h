...
#ifndef UTILITY_MATH_FILTER_GOLAYFILTER_H
...
namespace math {
    namespace filter {
        ...
        /**
         * @brief Filter that smooths data and calculates its first order derivative, based on the Savitzky-Golay class
         *of filters
         **/
        ...
        public:
            ...
            //! Shift the current data points (and thereby also the smooth `x`) by a given increment @p shift, while
            //! leaving the velocity untouched
            ...
            //! Shift the current data points (and thereby also the smooth `x`) by an increment calculated such that the
            //! last passed data point is now @p xnew, while leaving the velocity untouched
            void shiftPosTo(double xnew) {
                shiftPosBy(xnew - m_smooth_x.buffer()->back());
            }  // Note: back() is the newest entry in the buffer
...
#endif  // UTILITY_MATH_FILTER_GOLAYFILTER_H
