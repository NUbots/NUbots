...
namespace math {
    namespace filter {
        ...
        public:
            ...
            // Constructor
            explicit WLBFFilter(size_t numPoints = 0)
                : m_A(0.0), m_B(0.0), m_xMean(0.0), m_yMean(0.0), m_changed(true) {
            ...
            // Reset function (clears all points out of the circular buffers but does not change their capacity, use
            // resetAll() to also zero the capacity)
            void reset() {
                // Reset all data members
            ...
            void resize(size_t numPoints) {
                // Update the circular buffer capacities (if increasing the size we add zero data points, if decreasing
                // the size we keep the last added data points)
            ...
            void setXBuf(Iterator first, Iterator last) {
            ...
            void setWBuf(
                Iterator first,
                Iterator last)  // Note: The weights are squared, so a negative weight is equivalent to a positive one!
            {
                // Set the data in the buffer and fill in the rest with zeros
                size_t i = 0;
                for (; first != last; ++first, ++i) {
            ...
            void setZeroX() {
                std::fill(m_xBuf.begin(), m_xBuf.end(), 0.0);
                m_changed = true;
            }
            void setZeroY() {
                std::fill(m_yBuf.begin(), m_yBuf.end(), 0.0);
                m_changed = true;
            }
            void setZeroW() {
                std::fill(m_wBuf.begin(), m_wBuf.end(), 0.0);
                m_changed = true;
            }

            // Set all weights to be equal
            void setEqualW() {
                std::fill(m_wBuf.begin(), m_wBuf.end(), 1.0);
                m_changed = true;
            }

            // Set an initial XY state of the filter (set all internal data points to be this state, leaving weights
            // untouched)
            ...
            // Add a weight (this keeps the buffer of (x,y) data points fixed and only adds the weight w for the
            // existing newest data, every other point thereby receives a new weight, shifted by one place)
            ...
            // Add a data point (this keeps the buffer of weights fixed and only adds a new (x,y) data point, every
            // other point thereby receives a new weight, shifted by one place)
            ...
            void addXYW(
                double x,
                double y,
                double w = 1.0)  // Note: The weight w is squared, so a negative weight is equivalent to a positive one!
            {
            ...
            // Get functions
            bool empty() const {
                return (m_xBuf.capacity() == 0);
            }
            size_t size() const {
                return m_xBuf.capacity();
            }
            double value() {
                if (m_changed) calculateWLBF();
                return (m_A + m_yMean) + m_B * (m_xBuf.front() - m_xMean);
            }
            double deriv() {
                if (m_changed) calculateWLBF();
                return m_B;
            ...
            double getA() const {
                return m_A + m_yMean - m_B * m_xMean;
            }
            double getB() const {
                return m_B;
            }
            ...
            const Buffer& xBuf() const {
                return m_xBuf;
            }
            const Buffer& yBuf() const {
                return m_yBuf;
            }
            const Buffer& wBuf() const {
                return m_wBuf;
            }  // Note: The weights are the square of whatever was fed into the buffer
