...
#ifndef UTILITY_MATH_FILTER_LINSINFILLET_H
#define UTILITY_MATH_FILTER_LINSINFILLET_H
...
namespace math {
    namespace filter {
        ...
        public:
            ...
            LinSinFillet(double A, double B, double T) {
                update(A, B, T);
            }
            LinSinFillet(double A, double B, double T, double maxPre) {
                updateExact(A, B, T, maxPre);
            }

            // Reset function
            void reset() {
                m_A = m_B = m_a = m_b = m_c = m_tPre = m_tPost = 0.0;
            }
            ...
            // Update functions with maximum allowed pre-time maxPre should be non-negative and be the largest
            // acceptable
            // pre-time (time from start of fillet to the mid-fillet lin-sin transition)
            bool update(double A, double B, double T, double maxPre) {
                ...
                return update(
                    A, B, T * (maxPre / -m_tPre));  // Reduce T to (conservatively) abide by the maxPre specification
            }
            ...
            bool hasEffect(double t) const {
                return (t > m_tPre && t < m_tPost);
            }
            ...
            double eval(double t) const {
                ...
                if (t <= 0.0)
                    ...
                    return parabola - m_A * sin(m_B * t);
            }

            // Static evaluation function
            static double eval(double t, double A, double B, double T) {
                LinSinFillet LSF(A, B, T);
                return LSF.eval(t);
            }
            static double eval(double t, double A, double B, double T, double maxPre) {
                LinSinFillet LSF(A, B, T, maxPre);
                return LSF.eval(t);
            }
            ...
            double startTime() const {
                return m_tPre;
            }  // For small fillets this is approx -T (and is always < -T)
            double endTime() const {
                return m_tPost;
            }  // For successful fillets this is always T
...
#endif  // UTILITY_MATH_FILTER_LINSINFILLET_H
