    ...
    namespace math {
        namespace transform {
            ...
            inline Eigen::Transform<Scalar, 2, Eigen::Affine> interpolate(
                ...
                result.linear() = current.rotation()
                                  * Eigen::Rotation2D<Scalar>(t * (angle(current) - angle(target))).toRotationMatrix();
            ...
            template <typename Scalar>
            inline Eigen::Transform<Scalar, 3, Eigen::Affine> rotateX(
                const Scalar& radians,
            ...
            template <typename Scalar>
            inline Eigen::Transform<Scalar, 3, Eigen::Affine> rotateY(
                const Scalar& radians,
                const Eigen::Transform<Scalar, 3, Eigen::Affine>& t) {
                Eigen::Transform<Scalar, 3, Eigen::Affine> rotationMatrix =
                    Eigen::Transform<Scalar, 3, Eigen::Affine>::Identity();
            ...
            template <typename Scalar>
            inline Eigen::Transform<Scalar, 3, Eigen::Affine> rotateZ(
                const Scalar& radians,
                const Eigen::Transform<Scalar, 3, Eigen::Affine>& t) {
                Eigen::Transform<Scalar, 3, Eigen::Affine> rotationMatrix;
            ...
            inline Eigen::Affine2d lookAt(const Eigen::Vector2d& from, const Eigen::Vector2d& to) {
                ...
                result.linear() =
                    Eigen::Rotation2Dd(utility::math::angle::vectorToBearing(to - from)).toRotationMatrix();
                return result;
            }
