    ...
    namespace kalman {
        arma::vec::fixed<AdaptiveIMUModel::size> AdaptiveIMUModel::limitState(const arma::vec::fixed<size>& state) {
            ...
            if (angle < 45 || angle > 135) {
                // NUClear::log<NUClear::WARN>("AdaptiveIMUModel::limitState - New forward and down vectors are close:
                // angle =", angle, "(degrees) ", angle < 45 ? "( < 45 degrees)" : "( > 135 degrees)");
            }
            ...
            if (normForward == 0) {
                // TODO: RESTART FILTER
                // NUClear::log<NUClear::WARN>("AdaptiveIMUModel::limitState - Projected forward vector has zero
                // length!!!");
                return state;
            }
        ...
        arma::vec::fixed<AdaptiveIMUModel::size> AdaptiveIMUModel::timeUpdate(const arma::vec::fixed<size>& state,
                                                                              double deltaT,
                                                                              const arma::vec3& measurement) {
            // new universal rotation code for gyro (SORA)
            // See:
            // http://en.wikipedia.org/wiki/Axis%E2%80%93angle_representation#Simultaneous_orthogonal_rotation_angle
            arma::mat stateMatrix = arma::reshape(state, 3, 2);
            ...
            stateMatrix.col(0)             = stateMatrix.col(0) * cos(phi) + omegaCrossStateDown * sin(phi)
                                 + unitOmega * arma::dot(unitOmega, stateMatrix.col(0)) * (1.0 - cos(phi));

            const auto omegaCrossStateForward = arma::cross(unitOmega, stateMatrix.col(1));
            stateMatrix.col(1)                = stateMatrix.col(1) * cos(phi) + omegaCrossStateForward * sin(phi)
                                 + unitOmega * arma::dot(unitOmega, stateMatrix.col(1)) * (1.0 - cos(phi));
