    ...
    namespace math {
        namespace kalman {
            ...
            public:
                ...
                AdaptiveIMUModelQ() {} // empty constructor

                arma::vec::fixed<size> timeUpdate(const arma::vec::fixed<size>& state, double deltaT, std::nullptr_t);
            ...
            // Accelerometer
            template <>
            arma::vec AdaptiveIMUModelQ::predictedObservation<AdaptiveIMUModelQ::MeasurementType::ACCELEROMETER>(const arma::vec::fixed<size>& state) {

                return { 2 * state[QX], state[QY] + 2 * state[QX] * state[QW]
                       , 1 - 2 * state[QX] * state[QX] - 2 * state[QZ] * state[QZ]
                       , 2 * state[QY] * state[QZ] - 2 * state[QX] * state[QW] };
            }

            // Gyroscope
            template <>
            arma::vec AdaptiveIMUModelQ::predictedObservation<AdaptiveIMUModelQ::MeasurementType::GYROSCOPE>(const arma::vec::fixed<size>& state) {
                return state.rows(0, 3);
            }

            // Forward Vector
            template <>
            arma::vec AdaptiveIMUModelQ::predictedObservation<AdaptiveIMUModelQ::MeasurementType::FORWARD>(const arma::vec::fixed<size>& state) {
