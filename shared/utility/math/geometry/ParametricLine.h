...
namespace math {
    namespace geometry {
        ...
        public:
            Vector direction;
            Vector point;
            Eigen::Vector2d tLimits = {-std::numeric_limits<double>::infinity(),
                                       std::numeric_limits<double>::infinity()};
            ParametricLine() : direction(Vector::Zero()), point(Vector::Zero()) {}
            ParametricLine(const Vector& p1, const Vector& p2, bool segment = false)
                : direction(Vector::Zero()), point(Vector::Zero()) {
            ...
            void setFromDirection(const Vector& direction_,
                                  const Vector& point_,
                                  const Eigen::Vector2d& tLimits_ = {-std::numeric_limits<double>::infinity(),
                                                                     std::numeric_limits<double>::infinity()}) {
                if (direction_.template lpNorm<1>() <= 0) {
            ...
            Vector intersect(const ParametricLine<n>& l) const {
                ...
                if (determinant == 0) {
                    ...
                    Ainverse << -l.direction[1], l.direction[0],  // row 1
                        -direction[1], direction[0];              // row 2
                    Ainverse *= 1 / determinant;
                }

                Eigen::Vector2d tValues = Ainverse * (l.point - point);
