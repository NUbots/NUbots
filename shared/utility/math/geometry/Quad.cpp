...
namespace math {
    namespace geometry {
        ...
        Quad::Quad()
            : bl(arma::zeros<arma::vec>(2))
            , br(arma::zeros<arma::vec>(2))
            , tr(arma::zeros<arma::vec>(2))
            , tl(arma::zeros<arma::vec>(2)) {
            // Empty constructor.
        }
        ...
        double Quad::aspectRatio() const {
            return ((arma::norm(br - tr, 2) + arma::norm(bl - tl, 2) + 2)
                    / (arma::norm(br - bl, 2) + arma::norm(tr - tl, 2) + 2));
        }
