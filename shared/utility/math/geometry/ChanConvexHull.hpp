#ifndef UTILITY_MATH_GEOMETRY_CHANCONVEXHULL_HPP
#define UTILITY_MATH_GEOMETRY_CHANCONVEXHULL_HPP
...
namespace utility::math::geometry {
    ...
    int chan_orientation(const Eigen::Vector2d& p, const Eigen::Vector2d& q, const Eigen::Vector2d& r) {
    ...
    std::vector<int> graham_scan(const std::vector<int>& indices,
        ...
        for (int i = 1; i < indices_copy.size(); i++) {
        ...
        for (int i = 3; i < indices_copy.size(); i++) {
            // Keep removing top while the angle formed by points next-to-top, top, and points[i] makes a clockwise turn
            while (stack.size() > 1
                   && chan_orientation(points.col(stack[stack.size() - 2]),
                                       points.col(stack.back()),
                                       points.col(indices_copy[i]))
                          != 2) {
                stack.pop_back();
            }
    ...
    // Overloaded so we can directly call with our 3d matrix in the green horizon
    inline std::vector<int> chans_convex_hull(const std::vector<int>& indices,
