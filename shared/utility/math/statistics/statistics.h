...
namespace math {
    namespace statistics {
        ...
        template <int n = 2>
        arma::mat::fixed<n, n> calculateCovarianceMatrix(const std::vector<arma::vec::fixed<n>>& points,
                                                         const arma::vec::fixed<n>& mean) {
        ...
        arma::mat::fixed<n, n> calculateCorrelationMatrix(const arma::mat::fixed<n, n>& covariance) {
            ...
            diag.for_each([](arma::mat::elem_type& val) -> void {
                if (val > 0.0) {
