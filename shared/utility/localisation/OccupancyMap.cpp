...
namespace module::localisation {
    ...
    Eigen::MatrixXf OccupancyMap::get_map() {
        return map;
    }
        ...
        for (int i = Y0 - radius; i <= Y0 + radius; i++) {
            if (i >= 0 && i < map.rows()) {
                ...
                if (start_x >= 0 && end_x < map.cols()) {
                    map.block(i, start_x, 1, end_x - start_x + 1) = Eigen::MatrixXf::Ones(1, end_x - start_x + 1);
                }
            }
        }
        ...
        for (int i = X0 - radius; i <= X0 + radius; i++) {
            if (i >= 0 && i < map.cols()) {
                ...
                if (start_y >= 0 && end_y < map.rows()) {
                    map.block(start_y, i, end_y - start_y + 1, 1) = Eigen::MatrixXf::Ones(end_y - start_y + 1, 1);
                }
            }
        }
    }
    ...
    void OccupancyMap::create_distance_map(float grid_size) {
        ...
        Eigen::MatrixXf dist_map =
            Eigen::MatrixXf::Constant(map.rows(), map.cols(), std::numeric_limits<float>::infinity());
