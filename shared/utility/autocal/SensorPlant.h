...
public:
    ...
    std::map<MocapStream::RigidBodyID, float> multiply(std::map<MocapStream::RigidBodyID, float> m1,
                                                       std::map<MocapStream::RigidBodyID, float> m2);

    void setGroundTruthTransform(std::string streamA,
                                 std::string streamB,
                                 utility::math::matrix::Transform3D mapAtoB,
                                 bool useTruth = false);

    void setAnswers(std::string s1, std::string s2, std::map<int, int> answers);

    void setSimParameters(SimulationParameters a1,
