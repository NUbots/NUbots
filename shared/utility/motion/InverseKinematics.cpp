    ...
    namespace kinematics {
        ...
        */
        ...
        bool legPoseValid(const KinematicsModel& model, utility::math::matrix::Transform3D target, LimbID limb) {
            ...
            // Translate up foot
            Transform3D targetLeg = target.translate(Eigen::Vector3d(0, 0, model.leg.FOOT_HEIGHT));
        ...
        std::vector<std::pair<ServoID, float>> calculateLegJoints(const KinematicsModel& model,
            ...
            inputCoordinatesToCalcCoordinates << 0, 1, 0, 0,  // row 1
                1, 0, 0, 0,                                   // row 2
                0, 0, -1, 0,                                  // row 3
                0, 0, 0, 1;                                   // row 4
            ...
            if (limb != LimbID::LEFT_LEG) {
                const Eigen::Matrix3d id = (Eigen::Matrix3d() << -1, 0, 0, 0, 1, 0, 0, 0, 1).finished();
                target.topLeftCorner<3, 3>() = id * target.topLeftCorner<3, 3>();
                target.topLeftCorner<3, 1>() *= -1;
                target(0, 3) *= -1;
            }

            Eigen::Vector3d ankleX   = target.topLeftCorner<3, 1>();
            Eigen::Vector3d ankleY   = target.block<3, 1>(0, 1);
            Eigen::Vector3d anklePos = target.topRightCorner<3, 1>();
            ...
            Eigen::Vector3d legPlaneTangent = ankleY.cross(hipX);  // Will be unit as ankleY and hipX are normal and
                                                                   // unit
            ...
            hipRoll = (hipRollPositive ? 1 : -1) * std::acos(cosHipRoll);
            ...
            hipPitch = (isHipPitchPositive ? 1 : -1) * std::acos(legPlaneGlobalZ.dot(unitUpperLeg));

            Eigen::Vector3d hipXProjected =
            ...
            hipYaw = (isHipYawPositive ? 1 : -1) * std::acos(hipXProjected.dot(globalX));
        ...
        std::vector<std::pair<ServoID, float>> calculateCameraLookJoints(const KinematicsModel& model,
                                                                         Eigen::Vector3d cameraUnitVector) {
            std::vector<std::pair<ServoID, float>> positions;
            positions.push_back(
                std::make_pair(ServoID::HEAD_YAW, std::atan2(cameraUnitVector[1], cameraUnitVector[0])));
            positions.push_back(std::make_pair(ServoID::HEAD_PITCH,
                                               std::atan2(-cameraUnitVector[2],
        ...
        Eigen::Vector2d calculateHeadJointsToLookAt(Eigen::Vector3d groundPoint,
            ...
            Eigen::Vector3d cameraPosition        = camToGround.topRightCorner<3, 1>();
            Eigen::Vector3d groundSpaceLookVector = groundPoint - cameraPosition;
            Eigen::Vector3d lookVector = bodyToGround.topLeftCorner<3, 3>().transpose() * groundSpaceLookVector;
            Eigen::Vector3d lookVectorSpherical = utility::math::coordinates::cartesianToSpherical(lookVector);

            return lookVectorSpherical.head<3>().tail<2>();
        }

        Eigen::Vector2d headAnglesToSeeGroundPoint(const Eigen::Vector2d& gpos,
        ...
        std::vector<std::pair<ServoID, float>> setHeadPoseFromFeet(
            ...
            Eigen::Vector3d gaze = cameraToFeet.rotation().leftCols<1>();
        ...
        std::vector<std::pair<ServoID, float>> setArm(const KinematicsModel& model,
                                                      ...
                                                      const Eigen::Vector3d& angleHint) {
        ...
        std::vector<std::pair<ServoID, float>> setArmApprox(const KinematicsModel& model,
            ...
            Eigen::Vector3d shoulderPos = {model.arm.SHOULDER_X_OFFSET,
