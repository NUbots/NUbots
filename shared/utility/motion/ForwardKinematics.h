    ...
    namespace kinematics {
        ...
        */
        inline Eigen::Vector4d calculateCentreOfMass(const message::motion::KinematicsModel& model,
                                                     const std::map<uint32_t, Eigen::Matrix4d>& jointPositions,
                                                     bool includeTorso) {
            ...
            for (auto& joint : jointPositions) {
                ...
                massScaler.topLeftCorner<3, 3>() *= jointMass;

                totalMassVector +=
                    joint.second * massScaler * massVector;  // = m * local centre of mass in global robot coords
            }
            ...
            if (totalMassVector[3] > 0) {
                normaliser.topLeftCorner<3, 3>() *= 1 / totalMassVector[3];
        ...
        inline utility::math::geometry::Line calculateHorizon(const math::matrix::Rotation3D Rcw,
            ...
            Eigen::Vector2d normal = -Rcw.bottomRightCorner<2, 1>().normalized();
        ...
        inline utility::math::matrix::Transform3D calculateBodyToGround(Eigen::Vector3d groundNormal_body,
                                                                        double bodyHeight) {
            ...
            if (std::fabs(projectXOnNormal) == 1) {
                ...
                groundMatrixX           = (Z - projectZOnNormal * groundNormal_body).normalized();
                groundMatrixY           = groundNormal_body.cross(groundMatrixX);
            }
            else {
                groundMatrixX = (X - projectXOnNormal * groundNormal_body).normalized();
                groundMatrixY = groundNormal_body.cross(groundMatrixX);
            }

            utility::math::matrix::Transform3D groundToBody;
            groundToBody.topLeftCorner<3, 1>() = groundMatrixX;
        ...
        inline Eigen::Vector4d fsrCentreInBodyCoords(const KinematicsModel& model,
            ...
            Eigen::Vector2d position{foot[0] * model.leg.FOOT_LENGTH * 0.5, foot[1] * model.leg.FOOT_WIDTH * 0.5};
            Eigen::Vector4d centerFoot{
                position[0], position[1] + negativeIfRight * model.leg.FOOT_CENTRE_TO_ANKLE_CENTRE, 0, 1};
        ...
        inline Eigen::Vector3d calculateCentreOfPressure(const KinematicsModel& model, const Sensors& sensors) {
            ...
            if (sensors.leftFootDown) {
                CoP += fsrCentreInBodyCoords(model, sensors, sensors.fsr[LimbID::LEFT_LEG].centre, true);
                number_of_feet_down += 1.0f;
            }
            if (sensors.rightFootDown) {
                CoP += fsrCentreInBodyCoords(model, sensors, sensors.fsr[LimbID::RIGHT_LEG].centre, false);
                number_of_feet_down += 1.0f;
            }
        ...
        */
        inline Eigen::Matrix3d calculateArmJacobian(const KinematicsModel& model,
            ...
            const Eigen::Vector3d t1{
                model.arm.SHOULDER_LENGTH, negativeIfRight * model.arm.SHOULDER_WIDTH, -model.arm.SHOULDER_HEIGHT};
            const Eigen::Vector3d t2{model.arm.UPPER_ARM_X_OFFSET,
                                     negativeIfRight * model.arm.UPPER_ARM_Y_OFFSET,
                                     -model.arm.UPPER_ARM_LENGTH};
            const Eigen::Vector3d t3{model.arm.LOWER_ARM_LENGTH,
                                     negativeIfRight * model.arm.LOWER_ARM_Y_OFFSET,
                                     -model.arm.LOWER_ARM_Z_OFFSET};
            ...
            Eigen::Vector3d col1 = jRY1 * RX2 * RY_PI_2 * RY3 * t3 + jRY1 * RX2 * t2 + jRY1 * t1;

            Eigen::Vector3d col2 = RY1 * jRX2 * RY_PI_2 * RY3 * t3 + RY1 * jRX2 * t2;

            Eigen::Vector3d col3 = RY1 * RX2 * RY_PI_2 * jRY3 * t3;

            Eigen::Matrix3d joined;
            joined << col1, col2, col3;
            return joined;
        }
        /*! @return matrix J such that \overdot{X} = J * \overdot{theta}
        */
        inline Eigen::Vector3d calculateArmPosition(const KinematicsModel& model,
                                                    const Eigen::Vector3d& a,
                                                    bool isLeft) {
            int negativeIfRight = isLeft ? 1 : -1;

            const Eigen::Vector3d t0 = {model.arm.SHOULDER_X_OFFSET,
                                        negativeIfRight * model.arm.DISTANCE_BETWEEN_SHOULDERS / 2.0,
                                        model.arm.SHOULDER_Z_OFFSET};
            ...
            Eigen::Vector3d pos = RY1 * RX2 * RY_PI_2 * RY3 * t3 + RY1 * RX2 * t2 + RY1 * t1 + t0;
