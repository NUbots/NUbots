...
namespace messages
{
	...
	NUImage::NUImage(int width, int height, bool useInternalBuffer):
		flipped(false),
		m_image(nullptr),
		m_imageWidth(width),
		m_imageHeight(height),
		m_usingInternalBuffer(useInternalBuffer)
	{
	...
	NUImage::NUImage(const NUImage& source):
		m_image(nullptr),
		m_imageWidth(0),
		m_imageHeight(0),
		m_usingInternalBuffer(false)
	{
		...
		{
			std::memcpy ( &m_image[y][0], &source.m_image[y][0], sizeof(source.m_image[y][0])*sourceWidth);
		}
