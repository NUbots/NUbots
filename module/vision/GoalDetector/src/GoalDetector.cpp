            ...
            for (auto& cluster : clusters) {
                ...
                for (const auto& idx : cluster) {
                    ...
                    posts->rPWw.push_back(rPWw);
                        ...
                        float point_dist = sqrt(rPWw.x() * rPWw.x() + rPWw.y() * rPWw.y())

                        if (point_dist < best) {
                        best     = point_dist;
                        raw_best = rPWw;
                        normal_best = rPWw.normalized();
                        bestPoint = true;
                        }
                }

                posts->raw_best.push_back(raw_best);
                posts->normal_best.push_back(normal_best);
                    ...
                    if (bestPoint) {
                        ...
                        auto calc_goal_pos = [&](const Eigen::Vector3f& rFWw, const int& x_sign, const int& y_sign) {
                            return Eigen::Vector3f(
                                ...
                                rFWw.y() + (field.dimensions.goal_width  * y_sign),
                                rFWw.z());
                        };

                        auto calc_point_dist = [&](const Eigen::Vector3f& postL, const Eigen::Vector3f& postR, Eigen::Vector3f& bestPoint) {
                            float dist = ((postR - postL).cross(postL - bestPoint)).norm() / (postR - postL).norm();
                            return dist;
                        };
                        ...
                        float bestErrorGW = std::min(calc_point_dist(rGWw_own_l, rGWw_own_r, raw_best), calc_point_dist(rGWw_opp_l, rGWw_opp_r, raw_best));
                        float bestErrorGC = std::min(calc_point_dist(rGCc_own_l, rGCc_own_r, raw_best), calc_point_dist(rGCc_opp_l, rGCc_opp_r, raw_best));

                        emit(graph("True best point errorGW", bestErrorGW));
                        emit(graph("True best point errorGC", bestErrorGC));
                        emit(graph("rGWw own l", rGWw_own_l));
                        emit(graph("rGWw own r", rGWw_own_r));
                        emit(graph("rGWw opp l", rGWw_opp_l));
                        emit(graph("rGWw opp r", rGWw_opp_r));
                        emit(graph("rFWw", rFWw));
                }
            }
