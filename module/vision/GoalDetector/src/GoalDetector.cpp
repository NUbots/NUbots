                    ...
                    if (clusters.size() > 0) {
                        ...
                        for (auto& cluster : clusters) {
                            ...
                            if (config.debug) {
                                log<NUClear::DEBUG>(fmt::format(
                                    "Cluster split into {} left points, {} right points, {} top/bottom points",
                            ...
                            if ((std::distance(cluster.begin(), right) != 0) && (std::distance(right, other) != 0)) {
                                ...
                                if (config.use_median) {
                                    ...
                                    left_side = rays.col(
                                        *std::next(cluster.begin(), std::distance(cluster.begin(), right) / 2));
                                ...
                                g.post.bottom =
                                    horizon.Hcw.topLeftCorner<3, 3>().cast<float>() * bottom_point.normalized();
                        ...
                        for (auto it1 = goals->goals.begin(); it1 != goals->goals.end(); it1 = std::next(it1)) {
                            for (auto it2 = std::next(it1); it2 != goals->goals.end(); it2 = std::next(it2)) {
                                ...
                                const float width =
                                    distance_between(rGCc0, it1->post.distance, rGCc1, it2->post.distance);

                                const float disagreement =
                                    std::abs(width - actual_width) / std::max(width, actual_width);
