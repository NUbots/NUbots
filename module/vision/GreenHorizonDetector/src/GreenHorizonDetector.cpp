        ...
        GreenHorizonDetector::GreenHorizonDetector(std::unique_ptr<NUClear::Environment> environment)
            ...
            on<Trigger<VisualMesh>, Buffer<2>>().then("Green Horizon", [this](const VisualMesh& mesh) {
                ...
                if (clusters.size() < 1) {
                    ...
                    if (config.debug) {
                        log<NUClear::DEBUG>(
                            fmt::format("Making a convex hull from {} points", clusters.front().size()));
                    }
                    // Find the convex hull of the cluster
                    auto hull_indices =
                        utility::math::geometry::upper_convex_hull(clusters.front(), rays, world_offset);
                    ...
                    if (config.debug) {
                        log<NUClear::DEBUG>(
                            fmt::format("Calculated convex hull with {} points from cluster with {} points",
                                        hull_indices.size(),
                                        clusters.front().size()));
                    }
