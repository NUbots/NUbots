        ...
        on<Trigger<GreenHorizon>, With<FieldDescription>, Buffer<2>>().then(
            ...
            [this](const GreenHorizon& horizon, const FieldDescription& field) {
                ...
                for (auto& cluster : clusters) {
                    ...
                    const double angular_distance = field.ball_radius / std::sqrt(1 - radius * radius);
                    ...
                    log<NUClear::DEBUG>("**************************************************");
                    bool keep = true;
                    b.colour.fill(1);  // a valid ball has a white colour in NUsight
                    ...
                    if (deviation > cfg.maximum_deviation) {
                        ...
                        b.colour = keep ? Eigen::Vector4d(0, 1, 0, 1) : b.colour;
                        keep     = false;
                    }
                    ...
                    if ((std::abs(projection_distance - angular_distance) / max_distance) > cfg.distance_disagreement) {
                        ...
                        b.colour = keep ? Eigen::Vector4d(0, 0, 1, 1) : b.colour;
                        keep     = false;
                    }
                    ...
                    if (angular_distance < cfg.minimum_ball_distance) {
                        ...
                        b.colour = keep ? Eigen::Vector4d(1, 0, 0, 1) : b.colour;
                        keep     = false;
                    }
                    ...
                    if (angular_distance > field.dimensions.field_length) {
                        ...
                        b.colour = keep ? Eigen::Vector4d(1, 0, 1, 1) : b.colour;
                        keep     = false;
                    }
 ...
 * Cone fitting ... kind of broken
                    ...
                    auto cone_from_points = [&](const int& a, const int& b, const int& c) {
                        ...
                        A.row(1) = uPCw.row(b);
                        A.row(2) = uPCw.row(c);
                        return A.householderQr().solve(Eigen::Vector3d::Ones()).normalized();
                    };
                    ...
                    auto point_in_cone = [&](const int& a, const int& b, const int& c, const int& d) {
                        ...
                        for (int i = 0; i < 4; ++i) {
                            const Eigen::Vector3d& p0 = uPCw.row(perms[i]);
                            const Eigen::Vector3d& p1 = uPCw.row(perms[(i + 1) % 4]);
                            const Eigen::Vector3d& p2 = uPCw.row(perms[(i + 2) % 4]);
                            const Eigen::Vector3d& p3 = uPCw.row(perms[(i + 3) % 4]);
                            Eigen::Vector3d x = cone_from_points(perms[i], perms[(i + 1) % 4], perms[(i + 2) % 4]);
                    ...
                    for (auto& cluster : clusters) {
                        ...
                        Eigen::Vector3d uBCw = cone_from_points(cluster[0], cluster[1], cluster[2]);
