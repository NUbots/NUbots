...
float BallDetector::approximateCircleGreenRatio(const Circle& circle,
                                                const Image& image,
  ...
  int numGreen = 0;
  int actualSamples = 0;
  for (int i = 0; i < green_radial_samples;
       r = (++i) * circle.radius / float(green_radial_samples)) {
    float theta = 0;
    if (r == 0) {
      arma::ivec2 ipos = arma::ivec({int(std::round(circle.centre[0])),
                                     int(std::round(circle.centre[1]))});
      if (ipos[0] >= 0 && ipos[0] < int(image.dimensions[0]) && ipos[1] >= 0 &&
          ipos[1] < int(image.dimensions[1])) {
        // debug.push_back(std::make_tuple(ipos, ipos + arma::ivec2{1,1},
        // arma::vec4{1,1,1,1}));
        char c = static_cast<char>(utility::vision::getPixelColour(
            lut,
            getPixel(ipos[0], ipos[1], image.dimensions[0], image.dimensions[1],
                     image.data, static_cast<FOURCC>(image.format))));
        if (c == Colour::GREEN) {
          numGreen++;
        }
        actualSamples++;
    ...
    for (int j = 0; j < green_angular_samples;
         theta = (++j) * 2 * M_PI / float(green_angular_samples)) {
      float x = r * std::cos(theta);
      float y = r * std::sin(theta);
      arma::vec2 pos = circle.centre + arma::vec2({x, y});
      arma::ivec2 ipos =
          arma::ivec2({int(std::round(pos[0])), int(std::round(pos[1]))});
      if (ipos[0] >= 0 && ipos[0] < int(image.dimensions[0]) && ipos[1] >= 0 &&
          ipos[1] < int(image.dimensions[1])) {
        // debug.push_back(std::make_tuple(ipos, ipos + arma::ivec2{1,1},
        // arma::vec4{1,1,1,1}));
...
BallDetector::BallDetector(std::unique_ptr<NUClear::Environment> environment)
  ...
  on<Configuration>("BallDetector.yaml")
      .then([this](const Configuration& config) {

        MINIMUM_POINTS_FOR_CONSENSUS =
            config["ransac"]["minimum_points_for_consensus"].as<uint>();
        CONSENSUS_ERROR_THRESHOLD =
            config["ransac"]["consensus_error_threshold"].as<Expression>();

        MAXIMUM_ITERATIONS_PER_FITTING =
            config["ransac"]["maximum_iterations_per_fitting"].as<uint>();
        MAXIMUM_FITTED_MODELS =
            config["ransac"]["maximum_fitted_models"].as<uint>();
        MAXIMUM_DISAGREEMENT_RATIO =
            config["maximum_disagreement_ratio"].as<Expression>();

        maximum_relative_seed_point_distance =
            config["maximum_relative_seed_point_distance"].as<double>();

        measurement_distance_variance_factor =
            config["measurement_distance_variance_factor"].as<Expression>();
        measurement_bearing_variance =
            config["measurement_bearing_variance"].as<Expression>();
        measurement_elevation_variance =
            config["measurement_elevation_variance"].as<Expression>();

        green_ratio_threshold =
            config["green_ratio_threshold"].as<Expression>();
        green_radial_samples = config["green_radial_samples"].as<Expression>();
        green_angular_samples =
            config["green_angular_samples"].as<Expression>();
  ...
  on<Trigger<ClassifiedImage>, With<CameraParameters>, With<FieldDescription>,
     With<LookUpTable>, Single, Priority::LOW>()
      .then("Ball Detector", [this](std::shared_ptr<const ClassifiedImage>
                                        rawImage,
        ...
        Line horizon(convert<double, 2>(image.horizon.normal),
        ...
        for (const auto& point : image.ballPoints) {
          ballPoints.push_back(
              arma::vec2({double(point[0]), double(point[1])}));
        }

        // Use ransac to find the ball
        auto ransacResults = Ransac<RansacCircleModel>::fitModels(
            ballPoints.begin(), ballPoints.end(), MINIMUM_POINTS_FOR_CONSENSUS,
            MAXIMUM_ITERATIONS_PER_FITTING, MAXIMUM_FITTED_MODELS,
            CONSENSUS_ERROR_THRESHOLD);
        ...
        if (print_throwout_logs)
          log("Ransac : ", ransacResults.size(), "results");
        ...
        for (auto& result : ransacResults) {
          ...
          arma::vec2 centre = imageToScreen(result.model.centre,
          ...
          arma::vec3 ballCentreRay = arma::normalise(
              arma::normalise(getCamFromScreen(top, cam.focalLengthPixels)) +
              arma::normalise(getCamFromScreen(base, cam.focalLengthPixels)));

          // Get the centre of our ball in screen space
          arma::vec2 ballCentreScreen =
          ...
          if (horizon.y(result.model.centre[0]) > result.model.centre[1]) {
            if (print_throwout_logs)
              log("Ball discarded: image.horizon.y(result.model.centre[0]) > "
                  "result.model.centre[1]");
            continue;
          }

          // DOES HAVE INTERNAL GREEN
          float greenRatio =
              approximateCircleGreenRatio(result.model, *(image.image), lut);
          ...
          for (uint i = 0; i < 3; ++i) {
            for (auto& s : image.ballSeedPoints[i].points) {
              double dist = std::fabs(
                  result.model.radius -
                  arma::norm(result.model.centre -
                             arma::vec2({double(s[0]), double(s[1])})));
              if (sDist[i] > dist) {
                sDist[i] = dist;
              }
            }
          ...
          if (arma::max(sDist) / result.model.radius >
              maximum_relative_seed_point_distance) {
            if (print_throwout_logs)
          ...
          double widthDistance = widthBasedDistanceToCircle(
              field.ball_radius, top, base, cam.focalLengthPixels);
          ...
          // IF THE DISAGREEMENT BETWEEN THE WIDTH AND PROJECTION BASED
          // DISTANCES ARE TOO LARGE Project this vector to a plane midway
          // through the ball
          ...
          double ballCentreGroundProjDistance =
              arma::norm(ballCentreGroundProj);

          if (std::abs((widthDistance - ballCentreGroundProjDistance) /
                       std::max(ballCentreGroundProjDistance, widthDistance)) >
              MAXIMUM_DISAGREEMENT_RATIO) {
            if (print_throwout_logs)
              log("Ball discarded: Width and proj distance disagree too much: "
                  "width =",
                  widthDistance, "proj =", ballCentreGroundProjDistance);
            continue;
          }
          ...
          const Transform3D& Htc = convert<double, 4, 4>(
              sensors.forwardKinematics.at(ServoID::HEAD_PITCH));
          Transform3D Hcw = Htc.i() * Htw;
          Transform3D Hwc = Hcw.i();

          // Work out how far away the ball must be to be at the distance it is
          // from the camera
          ...
          arma::vec3 width_rBGg =
              Hgc.transformPoint(ballCentreRay * widthDistance);
          ...
          b.visObject.screenAngular = convert<double, 2>(
              arma::atan(convert<double, 2>(cam.pixelsToTanThetaFactor) %
                         ballCentreScreen));
          b.visObject.angularSize
              << getParallaxAngle(left, right, cam.focalLengthPixels),
              getParallaxAngle(top, base, cam.focalLengthPixels);

          // Add our points
          for (auto& point : result) {
            b.edgePoints.push_back(convert<double, 3>(getCamFromScreen(
                imageToScreen(point, convert<uint, 2>(image.dimensions)),
                cam.focalLengthPixels)));
          }
        ...
        for (auto a = balls->begin(); a != balls->end(); ++a) {
          Circle acircle(a->circle.radius,
                         convert<double, 2>(a->circle.centre));

          for (auto b = a + 1; b != balls->end();) {
            // If our balls overlap
            if (acircle.distanceToPoint(convert<double, 2>(b->circle.centre)) <
                b->circle.radius) {
