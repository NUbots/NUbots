...
KickPlanner::KickPlanner(std::unique_ptr<NUClear::Environment> environment)
  ...
  on<Configuration>("KickPlanner.yaml")
      .then([this](const Configuration& config) {
        ...
        cfg.seconds_not_seen_limit =
            config["seconds_not_seen_limit"].as<float>();
        cfg.kick_forward_angle_limit =
            config["kick_forward_angle_limit"].as<float>();
        emit(std::make_unique<KickPlannerConfig>(cfg));
        emit(std::make_unique<WantsToKick>(false));
      });

  on<Trigger<Ball>, With<std::vector<Self>>, With<FieldDescription>,
     With<KickPlan>, With<Sensors>>()
      .then([this](const Ball& ball, const std::vector<Self>& selfs,
                   const FieldDescription& fd, const KickPlan& kickPlan,
        ...
        double secondsSinceLastSeen =
            std::chrono::duration_cast<std::chrono::microseconds>(
                now - ball_last_measurement_time)
                .count() *
            1e-6;
        ...
        arma::vec2 kickTarget =
            WorldToRobotTransform(convert<double, 2>(self.locObject.position),
        ...
        arma::vec3 ballPosition =
            Htw.transformPoint({ball.locObject.position[0],
                                ball.locObject.position[1], fd.ball_radius});
        ...
        // log("KickPlan target
        // global",convert<double,2,1>(kickPlan.target).t());  log("Target of
        // Kick",kickTarget.t());  log("KickAngle",KickAngle);
        ...
        float timeSinceValid = (now - lastTimeValid).count() *
                               (1 / double(NUClear::clock::period::den));
            ...
            case KickType::IK_KICK:
              ...
              if (ballPosition[1] > 0) {
                emit(std::make_unique<KickCommand>(KickCommand(
                    Eigen::Vector3d(0.1, 0.04, 0),
                    Eigen::Vector3d(1.0, 0.0, 0.0), KickCommandType::NORMAL)));
            ...
            case KickType::SCRIPTED:
              ...
              if (ballPosition[1] > 0) {
                emit(std::make_unique<KickScriptCommand>(KickScriptCommand(
                    Eigen::Vector3d(1.0, 0.0, 0.0), LimbID::LEFT_LEG)));
                emit(std::make_unique<WantsToKick>(true));
                ;
              } else {
                emit(std::make_unique<KickScriptCommand>(KickScriptCommand(
                    Eigen::Vector3d(1.0, 0.0, 0.0), LimbID::RIGHT_LEG)));
                emit(std::make_unique<WantsToKick>(true));
                ;
              }
              break;
            default:
              throw new std::runtime_error("KickPlanner: Invalid KickType");
          }
