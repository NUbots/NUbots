...
BezierWalkPathPlanner::BezierWalkPathPlanner(
  ...
  on<Configuration>("BezierWalkPathPlanner.yaml")
      .then([this](const Configuration& file) {
        log(__LINE__);
  ...
  emit<Scope::INITIALIZE>(std::make_unique<RegisterAction>(RegisterAction{
      ...
      "Bezier Walk Path Planner",
      {
          ...
          std::pair<double, std::set<LimbID>>(
              0, {LimbID::LEFT_LEG, LimbID::RIGHT_LEG}),
          std::pair<double, std::set<LimbID>>(
              0, {LimbID::LEFT_ARM, LimbID::RIGHT_ARM}),
      },
      [this](const std::set<LimbID>& givenLimbs) {
        if (givenLimbs.find(LimbID::LEFT_LEG) != givenLimbs.end()) {
          // Enable the walk engine.
          emit<Scope::DIRECT>(std::move(
              std::make_unique<EnableWalkEngineCommand>(subsumptionId)));
        }
      },
      [this](const std::set<LimbID>& takenLimbs) {
        if (takenLimbs.find(LimbID::LEFT_LEG) != takenLimbs.end()) {
          ...
          emit<Scope::DIRECT>(std::move(
              std::make_unique<DisableWalkEngineCommand>(subsumptionId)));
        }
      },
  ...
  on<Trigger<KickFinished>>().then([this](const KickFinished&) {
    ...
    emit(std::make_unique<ActionPriorites>(
        ActionPriorites{subsumptionId, {26, 11}}));
  });

  on<Trigger<MotionCommand>, Sync<BezierWalkPathPlanner>>().then(
      [this](const MotionCommand& cmd) {
        // save the plan
        latestCommand = cmd;
      });

  on<Every<20, Per<std::chrono::seconds>>, With<message::localisation::Ball>,
     With<std::vector<message::localisation::Self>>,
     Optional<With<std::vector<message::vision::Obstacle>>>,
     Sync<BezierWalkPathPlanner>, Single>()
      .then("Updates Bezier Plan", [this](const LocalisationBall& ball,
                                          const std::vector<Self>& selfs,
                                          std::shared_ptr<const std::vector<
                                              VisionObstacle>> /*robots*/) {
        ...
        if (latestCommand.type == MotionCommand::Type::StandStill) {
          ...
          std::unique_ptr<WalkCommand> command = std::make_unique<WalkCommand>(
              subsumptionId, latestCommand.walkCommand);
          emit(std::move(command));
          emit(std::make_unique<ActionPriorites>(
              ActionPriorites{subsumptionId, {26, 11}}));
          ...
          arma::vec2 ball_world_position = RobotToWorldTransform(
              selfs.front().position, selfs.front().heading, ball.position);
          arma::vec2 kick_target =
              latestCommand.kickTarget;  // 2 * ball_world_position -
                                         // selfs.front().position;
          emit(drawSphere("kick_target",
                          arma::vec3({kick_target[0], kick_target[1], 0.0}),
                          0.1, arma::vec3({1, 0, 0}), 0));
          ...
          // From "A Bezier curve based path planning in a multi-agent robot
          // soccer system without violating the acceleration limits" by K.G.
          // Jolly, R. Sreerama Kumar, R. Vijayakumar
          ...
          float theta1 =
              0.5;  // std::atan2(selfs.front().position[1],selfs.front().position[0]);
                    // //angle orientation of robot in space
          float theta2 = std::atan2(
              kick_target[1], kick_target[0]);  // angle wanting to stike ball,
                                                // angle of ball to goal
          // Calculate RP, RS
          float RP = VP * VP / 4;  // Minimum radius of curvature required at
                                   // point P (robot point), 4 = ar is maximum
                                   // radial acceleration
          float RS = VS * VS / 4;  // Mimimum radius of curvature required at
                                   // point S (Ball point)
          ...
          emit(drawSphere("Tangent 1", arma::vec3({B1, A1, 0.0}), 0.1,
                          arma::vec3({0, 1, 0}), 0));
          emit(drawSphere("Tangent 2", arma::vec3({B2, A2, 0.0}), 0.1,
                          arma::vec3({0, 0, 1}), 0));

          // log("A1 = ", A1, "\n B1 = ", B1, "\n A2 = ", A2 , "\n B2 = ", B2);

          // make 2 11 long arrays, to descretize the bezier and bezier
          // derivatives
          // float bezier_X_point;
          // float bezier_Y_point;
          // float bezXdash;
          // float bezYdash;
          double u = 0.1;  // variable determining how long along bezier curve
                           // robot looks, to move to config

          // float bezXdashdash[11];
          // float bezYdashdash[11];

          float bezier_X_point = A0 * (1 - u) * (1 - u) * (1 - u) +
                                 3 * A1 * u * (1 - u) * (1 - u) +
                                 3 * A2 * u * u * (1 - u) + A3 * u * u * u;
          float bezier_Y_point = B0 * (1 - u) * (1 - u) * (1 - u) +
                                 3 * B1 * u * (1 - u) * (1 - u) +
                                 3 * B2 * u * u * (1 - u) + B3 * u * u * u;
          // float bezXdash = 3*(u*u*(-A0+3*A1-3*A2+A3)+
          // 2*u*(A0-2*A1+A2)-A0+A1);  float bezYdash = 3*(u*u*(-B0+3*B1-3*B2+B3)+
          // 2*u*(B0-2*B1+B2)-B0+B1);  bezXdashdash[i] =
          // 6*(A0*(-u)+A0+3*A1*u-2*A1-3*A2 u+A2+A3*u);  bezYdashdash[i] =
          // 6*(B0*(-u)+B0+3*B1*u-2*A1-3*B2 u+B2+B3*u);

          arma::fmat bez_matrix;
          bez_matrix << bezier_X_point << bezier_Y_point << arma::endr << A0
                     << B0;
          ...
          /* More complicated walk path follower, useful with more accurate
          locomotion
          ...
          //Find which points are turning points (using 15), assuming 6 turning
          points const int N = sizeof(radius_of_curvature) / sizeof(int); int
          turning_point[6]; for (int i=0;i<6;i++) { int turning_point[i] =
          distance(radius_of_curvature, min_element(radius_of_curvature,
          radius_of_curvature + N)); radius_of_curvature[turning_point[i]] =
          1000; //hack to make the point high so it is not picked up again
          } */

          // PID controller, which spits out error values compared to use
          // previous curves
