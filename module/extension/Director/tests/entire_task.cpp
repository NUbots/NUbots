    ...
    public:
        explicit TestReactor(std::unique_ptr<NUClear::Environment> environment)
            ...
            on<Provide<ComplexTask>>().then([this](const ComplexTask& t) {
                ...
                emit<Task>(std::make_unique<SimpleTask<0>>(SimpleTask<0>{"from complex " + t.msg}));
                emit<Task>(std::make_unique<SimpleTask<1>>(SimpleTask<1>{"from complex " + t.msg}));
            });

            on<Provide<BlockerTask>>().then([this] {  //
                events.push_back("emitting blocker task");
                emit<Task>(std::make_unique<SimpleTask<0>>(SimpleTask<0>{"from blocker"}));
            });
