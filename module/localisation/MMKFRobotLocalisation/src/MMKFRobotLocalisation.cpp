...
namespace localisation {
    ...
    MMKFRobotLocalisation::MMKFRobotLocalisation(std::unique_ptr<NUClear::Environment> environment)
        ...
        on<Configuration>("MMKFRobotLocalisationEngine.yaml")
            .then("MMKFRobotLocalisationEngineConfig Update",
                  [this](const Configuration& config) { engine_->UpdateRobotLocalisationEngineConfiguration(config); });

        on<Startup, Optional<With<FieldDescription>>>().then(
            "FieldDescription Update", [this](std::shared_ptr<const FieldDescription> desc) {
                if (desc == nullptr) {
                    throw std::runtime_error(
                        "FieldDescription Update: support::configuration::SoccerConfig module might not be installed");
                }
        ...
        on<Trigger<ResetRobotHypotheses>, With<Sensors>, Sync<MMKFRobotLocalisation>, Single>().then(
        ...
        // Emit self
        emit_data_handle =
            on<Every<100, std::chrono::milliseconds>, With<Sensors>, Sync<MMKFRobotLocalisation>, Single>().then(
                "Localisation NUSight Output", [this](const Sensors& sensors) {
                    ...
                    for (auto& model : hypotheses) {
                        ...
                        Self robot_model;
                        robot_model.position =
                            sensors.world.translation().rows(0, 1) - model_state.rows(robot::kX, robot::kY);
        ...
        on<Trigger<Sensors>, Sync<MMKFRobotLocalisation>, Single>().then("MMKFRobotLocalisation Sensors Update",
        ...
        on<Trigger<std::vector<message::vision::Goal>>, With<Sensors>, Sync<MMKFRobotLocalisation>, Single>().then(
            ...
            [this](const std::vector<message::vision::Goal>& goals, const Sensors& /*sensors*/) {
                ...
                for (auto& goal : goals) {
                    ...
                    for (uint i = 0; i < goal.measurements.size(); ++i) {
                        ...
                        msg << ((goal.side == Goal::Side::LEFT) ? "L " : (goal.side == Goal::Side::RIGHT) ? "R" : "U")
                            << ((goal.team == Goal::Team::OWN)
                                    ? "OWN Goal Pos"
                                    : (goal.team == Goal::Team::OPPONENT) ? "OPP Goal Pos" : "UGoal Pos")
                            << " " << i;
                        emit(graph(msg.str(),
                                   goal.measurements[i].position[0],
                                   goal.measurements[i].position[1],
                                   goal.measurements[i].position[2]));
