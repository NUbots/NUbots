...
namespace module::localisation {
    ...
    FieldLocalisation::FieldLocalisation(std::unique_ptr<NUClear::Environment> environment)
        ...
        on<Trigger<FieldLines>>().then("Particle Filter", [this](const FieldLines& field_lines) {
            ...
            field->Hfw         = Hfw.matrix().cast<double>();
            field->covariance  = covariance.cast<double>();
    ...
    float FieldLocalisation::calculate_weight(const Eigen::Vector3f particle,
        ...
        for (auto rORr : observations) {
            ...
            if (occupancy_value != -1 && rORr.norm() < cfg.max_range) {
                weight += std::exp(-0.5f * std::pow(occupancy_value / cfg.measurement_noise, 2))
    ...
    Eigen::Matrix<float, 3, 3> FieldLocalisation::compute_covariance() {
        ...
        for (const auto& particle : particles) {
            const Eigen::Vector3f deviation = particle.state - state;
