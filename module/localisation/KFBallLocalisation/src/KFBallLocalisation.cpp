...
namespace localisation {
    ...
    KFBallLocalisation::KFBallLocalisation(std::unique_ptr<NUClear::Environment> environment)
        : Reactor(std::move(environment)), engine_(), emit_data_handle(), last_measurement_time() {
            ...
            on<Every<100, std::chrono::milliseconds>,
               ...
               Sync<KFBallLocalisation>>()
                .then("NUbugger Output", [this](const Sensors& sensors, const std::vector<Self>& robots) {
                    ...
                    worldSpaceBallPos.rows(0, 1) = utility::localisation::transform::RobotToWorldTransform(
                        robots[0].position, robots[0].heading, robot_space_ball_pos);
                    arma::vec3 worldSpaceBallVel = arma::zeros(3);
                    worldSpaceBallVel.rows(0, 1) = utility::localisation::transform::RobotToWorldTransform(
                        arma::zeros(2), robots[0].heading, robot_space_ball_vel);
        ...
        on<Trigger<std::vector<message::vision::Ball>>, Sync<KFBallLocalisation>>().then(
            "KFBallLocalisation Step", [this](const std::vector<message::vision::Ball>& balls) {
