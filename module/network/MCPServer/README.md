...
- `get_localisation` - Combines the latest cached `Field::Hfw` (from `localisation::FieldLocalisationNLopt`) with `Sensors::Htw` to get the robot's pose in field space (Hft: field to torso), and returns `x`/`y`/`z`/`yaw`/`cost` as JSON text. The field frame is centred on the pitch with x toward the opponent's goal, so this is comparable against pitch markings seen in `get_image`. No parameters. Returns a text message if no sensor data or no field localisation estimate has been received yet.
