...
namespace mcp {
    ...
    public:
        ...
        [[nodiscard]] std::future<ListToolsResult> list_tools(std::optional<std::string> cursor = std::nullopt);

        /// `tools/call` with the given name and JSON-shaped arguments.
        [[nodiscard]] std::future<CallToolResult> call_tool(std::string name, nlohmann::json arguments = nullptr);
        ...
        /// server's default.
        [[nodiscard]] std::future<CreateTaskResult> call_tool_as_task(
            std::string name,
        ...
        [[nodiscard]] std::future<nlohmann::json> task_result(std::string task_id);

        /// `tasks/list` — paginated enumeration of all tasks the server
        /// currently knows about.
        [[nodiscard]] std::future<ListTasksResult> task_list(std::optional<std::string> cursor = std::nullopt);
        ...
        [[nodiscard]] std::future<ListResourcesResult> list_resources(std::optional<std::string> cursor = std::nullopt);

        /// `resources/templates/list`.
        [[nodiscard]] std::future<ListResourceTemplatesResult> list_resource_templates(
        ...
        [[nodiscard]] std::future<void> subscribe(std::string uri);

        /// `resources/unsubscribe`.
        [[nodiscard]] std::future<void> unsubscribe(std::string uri);

        /// Register a handler for inbound resource-update notifications.
        /// The handler is called whenever the server sends
        /// `notifications/resources/updated`.
        using ResourceUpdatedHandler = std::function<void(const ResourceUpdatedNotificationParams&)>;
        ...
        /// `prompts/list`.
        [[nodiscard]] std::future<ListPromptsResult> list_prompts(std::optional<std::string> cursor = std::nullopt);

        /// `prompts/get` with the given prompt name and (optional) string-keyed
        /// arguments to fill in template parameters.
        [[nodiscard]] std::future<GetPromptResult> get_prompt(
            std::string name,
        ...
        std::error_code cancel_request(RequestId request_id, std::optional<std::string> reason = std::nullopt);
        ...
        [[nodiscard]] std::future<void> set_log_level(LoggingLevel level);

        /// Hook for inbound `notifications/message` (server-emitted log).
        using LogMessageHandler = std::function<void(const LoggingMessageNotificationParams&)>;
        void set_log_message_handler(LogMessageHandler handler);

        /// Hook for inbound `notifications/progress`.
        using ProgressHandler = std::function<void(const ProgressNotificationParams&)>;
        ...
        using ElicitationCompleteHandler = std::function<void(std::string elicitation_id)>;
        ...
        /// suggestions.
        [[nodiscard]] std::future<CompleteResult> complete(
