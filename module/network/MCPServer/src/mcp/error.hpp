...
namespace mcp {
    ...
    public:
        explicit Error(ErrorObject err) : std::runtime_error(err.message), error_(std::move(err)) {}

        Error(int code, std::string msg, nlohmann::json data = {})
            : std::runtime_error(msg), error_{code, std::move(msg), std::move(data)} {}

        [[nodiscard]] const ErrorObject& object() const noexcept {
            return error_;
        }
