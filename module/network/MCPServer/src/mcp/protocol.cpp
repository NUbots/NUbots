...
namespace mcp {
    ...
    namespace {
        ...
        T require(const nlohmann::json& j, const char* key) {
            auto it = j.find(key);
            if (it == j.end()) {
                throw Error(error_code::parse_error, std::string{"missing required field: "} + key);
            }
            return it->get<T>();
        }
        ...
        template <typename T>
        void take_optional(const nlohmann::json& j, const char* key, std::optional<T>& out) {
            auto it = j.find(key);
        ...
        inline void take_optional_json(const nlohmann::json& j, const char* key, std::optional<nlohmann::json>& out) {
            auto it = j.find(key);
            if (it == j.end() || it->is_null())
                return;
            out = *it;
        ...
        template <typename T>
        void put_optional(nlohmann::json& j, const char* key, const std::optional<T>& value) {
            if (value.has_value())
                j[key] = *value;
        }
    ...
    void from_json(const nlohmann::json& j, RequestId& id) {
        if (j.is_string()) {
            ...
            if (static_cast<double>(truncated) != v) {
                throw Error(error_code::parse_error, "JSON-RPC id must be an integer (got fractional number)");
            }
            id = RequestId{truncated};
        }
        else {
            throw Error(error_code::parse_error, "JSON-RPC id must be a string or integer");
        }
    }
    ...
    void to_json(nlohmann::json& j, const JsonRpcResponse& r) {
        ...
        if (r.id.has_value())
            ...
            j["id"] = nullptr;
        if (r.is_success())
            j["result"] = std::get<0>(r.outcome);
        else
            j["error"] = std::get<1>(r.outcome);
    }
    ...
    JsonRpcMessage parse_message(const nlohmann::json& j) {
        if (!j.is_object()) {
            throw Error(error_code::invalid_request, "JSON-RPC frame must be an object");
        }
        ...
        if (has_method && has_id)
            return j.get<JsonRpcRequest>();
        if (has_method)
            return j.get<JsonRpcNotification>();
        if (has_result || has_error)
            return j.get<JsonRpcResponse>();

        throw Error(error_code::invalid_request, "JSON-RPC frame has neither \"method\" nor \"result\"/\"error\"");
    }
    ...
    namespace {
        ...
        Role role_from_string(std::string_view s) {
            ...
            throw Error(error_code::parse_error, std::string{"unknown role: "} + std::string{s});
        }
    ...
    void to_json(nlohmann::json& j, const Role& r) {
        j = std::string{to_string(r)};
    }
    void from_json(const nlohmann::json& j, Role& r) {
        r = role_from_string(j.get<std::string>());
    }
    ...
    void to_json(nlohmann::json& j, const ImageContent& c) {
        j = nlohmann::json{{"type", "image"}, {"data", c.data}, {"mimeType", c.mime_type}};
        put_optional(j, "annotations", c.annotations);
    }
    ...
    void to_json(nlohmann::json& j, const AudioContent& c) {
        j = nlohmann::json{{"type", "audio"}, {"data", c.data}, {"mimeType", c.mime_type}};
        put_optional(j, "annotations", c.annotations);
    }
    ...
    void from_json(const nlohmann::json& j, ResourceContents& c) {
        if (j.contains("text")) {
            c = j.get<TextResourceContents>();
            return;
        }
        if (j.contains("blob")) {
            c = j.get<BlobResourceContents>();
            return;
        }
        throw Error(error_code::parse_error, "ResourceContents must have either \"text\" or \"blob\"");
    }
    ...
    void from_json(const nlohmann::json& j, PlainContentBlock& c) {
        ...
        throw Error(error_code::parse_error, std::string{"plain content block must not be \"" + t + "\""});
    }
    ...
    void from_json(const nlohmann::json& j, ContentBlock& c) {
        ...
        if (t == "tool_use") {
            c = j.get<ToolUseContent>();
            return;
        }
        if (t == "tool_result") {
            c = j.get<ToolResultContent>();
            return;
        }
        throw Error(error_code::parse_error, std::string{"unknown content block type: "} + t);
    }
    ...
    void from_json(const nlohmann::json& j, TaskSupport& t) {
        ...
        if (v == "forbidden")
            t = TaskSupport::forbidden;
        else if (v == "optional")
            t = TaskSupport::optional;
        else if (v == "required")
            t = TaskSupport::required;
        else
            throw Error(error_code::invalid_params, "tool execution: unknown taskSupport \"" + v + "\"");
    }
    ...
    void to_json(nlohmann::json& j, const GetPromptRequestParams& p) {
        ...
        if (p.arguments.has_value()) {
            ...
            for (const auto& [k, v] : *p.arguments)
                args[k] = v;
        ...
        if (auto it = j.find("arguments"); it != j.end() && it->is_object()) {
            ...
            for (auto i = it->begin(); i != it->end(); ++i) {
                if (i.value().is_string())
                    args.emplace(i.key(), i.value().get<std::string>());
            }
    ...
    std::string ProgressToken::canonical() const {
        return is_string() ? "s:" + as_string() : "i:" + std::to_string(as_integer());
    }
    ...
    void from_json(const nlohmann::json& j, ProgressToken& t) {
        if (j.is_string()) {
            t = ProgressToken{j.get<std::string>()};
            return;
        }
        if (j.is_number_integer()) {
            t = ProgressToken{j.get<std::int64_t>()};
            return;
        }
        if (j.is_number_float()) {
            ...
            const auto truncated = static_cast<std::int64_t>(v);
            if (static_cast<double>(truncated) != v) {
                throw Error(error_code::parse_error, "progressToken must be an integer (got fractional number)");
            }
            t = ProgressToken{truncated};
            return;
        }
        throw Error(error_code::parse_error, "progressToken must be a string or integer");
    }
    ...
    namespace {
        LoggingLevel logging_level_from_string(std::string_view s) {
            ...
            throw Error(error_code::parse_error, std::string{"unknown logging level: "} + std::string{s});
        }
    }  // namespace

    void to_json(nlohmann::json& j, const LoggingLevel& l) {
        j = std::string{to_string(l)};
    }
    ...
    void to_json(nlohmann::json& j, const IncludeContext& c) {
        j = std::string{to_string(c)};
    }
    void from_json(const nlohmann::json& j, IncludeContext& c) {
        const auto s = j.get<std::string>();
        if (s == "none")
            c = IncludeContext::none;
        else if (s == "thisServer")
            c = IncludeContext::this_server;
        else if (s == "allServers")
            c = IncludeContext::all_servers;
        else
            throw Error(error_code::parse_error, "unknown includeContext: " + s);
    }
    ...
    void to_json(nlohmann::json& j, const ToolChoiceMode& m) {
        j = std::string{to_string(m)};
    }
    void from_json(const nlohmann::json& j, ToolChoiceMode& m) {
        const auto s = j.get<std::string>();
        if (s == "auto")
            m = ToolChoiceMode::auto_;
        else if (s == "any")
            m = ToolChoiceMode::any;
        else if (s == "none")
            m = ToolChoiceMode::none;
        else if (s == "tool")
            m = ToolChoiceMode::tool;
        else
            throw Error(error_code::parse_error, "unknown toolChoice mode: " + s);
    }
    ...
    void from_json(const nlohmann::json& j, ToolChoice& c) {
        ...
        throw Error(error_code::parse_error, "toolChoice must be a string or an object");
    }
    ...
    void from_json(const nlohmann::json& j, SamplingMessage& m) {
        ...
        if (c.is_array()) {
            ...
            for (const auto& block : c)
                m.content.push_back(block.get<ContentBlock>());
        }
        else {
    ...
    void to_json(nlohmann::json& j, const CreateMessageRequestParams& p) {
        ...
        if (p.stop_sequences.has_value())
            j["stopSequences"] = *p.stop_sequences;
    ...
    void from_json(const nlohmann::json& j, CreateMessageRequestParams& p) {
        ...
        if (!mt.is_number_integer() && !mt.is_number_unsigned()) {
            throw Error(error_code::invalid_params, "maxTokens must be an integer");
        }
        if (mt.is_number_integer() && mt.get<std::int64_t>() < 0) {
            throw Error(error_code::invalid_params, "maxTokens must be non-negative");
        }
        if (mt.is_number_unsigned()
            && mt.get<std::uint64_t>() > static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max())) {
            throw Error(error_code::invalid_params, "maxTokens overflows int64");
        }
    ...
    void from_json(const nlohmann::json& j, CreateMessageResult& r) {
        ...
        if (c.is_array()) {
            ...
            for (const auto& block : c)
                r.content.push_back(block.get<ContentBlock>());
        }
        else {
    ...
    void from_json(const nlohmann::json& j, CompletionReference& r) {
        const auto t = require<std::string>(j, "type");
        if (t == "ref/resource") {
            r = j.get<ResourceTemplateReference>();
            return;
        }
        if (t == "ref/prompt") {
            r = j.get<PromptReference>();
            return;
        }
        throw Error(error_code::parse_error, std::string{"unknown completion reference type: "} + t);
    }
    ...
    void to_json(nlohmann::json& j, const CompleteRequestParams& p) {
        ...
        if (p.context_arguments.has_value()) {
            ...
            for (const auto& [k, v] : *p.context_arguments)
                ctx[k] = v;
        ...
        if (auto it = j.find("context"); it != j.end() && it->is_object()) {
            if (auto a = it->find("arguments"); a != it->end() && a->is_object()) {
                ...
                for (auto i = a->begin(); i != a->end(); ++i) {
                    if (i.value().is_string())
                        ctx.emplace(i.key(), i.value().get<std::string>());
                }
    ...
    void from_json(const nlohmann::json& j, ElicitFormRequestParams& p) {
        ...
        if (!p.requested_schema.is_object()) {
            throw Error(error_code::invalid_params, "elicitation/create: `requestedSchema` must be a JSON object");
        }
    }
    ...
    void from_json(const nlohmann::json& j, ElicitRequestParams& p) {
        ...
        if (auto it = j.find("mode"); it != j.end()) {
            if (!it->is_string()) {
                throw Error(error_code::invalid_params, "elicitation/create: `mode` must be a string");
            }
        ...
        if (mode == "form") {
            ...
            throw Error(error_code::invalid_params, "elicitation/create: unknown mode \"" + mode + "\"");
        }
    }
    ...
    void from_json(const nlohmann::json& j, ElicitAction& a) {
        const auto s = j.get<std::string>();
        if (s == "accept")
            a = ElicitAction::accept;
        else if (s == "decline")
            a = ElicitAction::decline;
        else if (s == "cancel")
            a = ElicitAction::cancel;
        else
            throw Error(error_code::invalid_params, "elicitation result: unknown action \"" + s + "\"");
    }
    ...
    void from_json(const nlohmann::json& j, TaskStatus& s) {
        const auto v = j.get<std::string>();
        if (v == "working")
            s = TaskStatus::working;
        else if (v == "input_required")
            s = TaskStatus::input_required;
        else if (v == "completed")
            ...
            throw Error(error_code::invalid_params, "task: unknown status \"" + v + "\"");
    }
