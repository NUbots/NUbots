...
namespace mcp {
    ...
    public:
        ...
        /// JSON-RPC error.
        using ResourceReadHandler = std::function<ReadResourceResult(const std::string& uri)>;
        ...
        Server& tool(std::string name, nlohmann::json input_schema, ToolHandler handler, ToolMetadata meta);

        /// Register a resource the server exposes. The handler is invoked
        /// when a client calls `resources/read` for this URI.
        Server& resource(Resource descriptor, ResourceReadHandler handler);
        ...
        Server& prompt(Prompt descriptor, PromptGetHandler handler);
        ...
        [[nodiscard]] std::future<CreateMessageResult> sample(CreateMessageRequestParams params);
        ...
        /// the Session worker.
        [[nodiscard]] std::future<ElicitResult> elicit(ElicitRequestParams params);
        ...
        [[nodiscard]] std::future<ElicitResult> elicit(ElicitRequestParams params, std::chrono::milliseconds timeout);
        ...
        using CompletionHandler = std::function<CompletionValues(const CompleteRequestParams&)>;
        ...
        Server& enable_tasks(std::optional<std::int64_t> default_ttl_ms = std::nullopt, std::size_t max_concurrent = 0);
    ...
    private:
        void on_initialize(const InitializeRequestParams& params, JsonRpcResponse& resp);
