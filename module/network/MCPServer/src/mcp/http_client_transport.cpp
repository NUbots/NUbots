...
namespace mcp {
    ...
    struct HttpClientTransport::ClientImpl {
        explicit ClientImpl(const std::string& base) : client_post(base), client_get(base) {}
    ...
    HttpClientTransport::HttpClientTransport(Options opts) : opts_(std::move(opts)) {
        ...
        const auto secs     = std::chrono::duration_cast<std::chrono::seconds>(opts_.connect_timeout).count();
        const auto usecs    = (opts_.connect_timeout.count() % 1000) * 1000;
        const auto rt_secs  = std::chrono::duration_cast<std::chrono::seconds>(opts_.request_timeout).count();
    ...
    void HttpClientTransport::on_message(MessageCallback cb) {
        on_message_ = std::move(cb);
    }
    void HttpClientTransport::on_error(ErrorCallback cb) {
        on_error_ = std::move(cb);
    }
    void HttpClientTransport::on_close(CloseCallback cb) {
    ...
    void HttpClientTransport::close() {
        ...
        if (sid.has_value() && !sid->empty() && impl_) {
            try {
                ...
                for (const auto& [k, v] : opts_.extra_headers)
                    headers.emplace(k, v);
    ...
    void HttpClientTransport::fire_close() noexcept {
        if (close_fired_.exchange(true, std::memory_order_acq_rel))
            return;
        if (!on_close_)
            return;
        try {
    ...
    std::error_code HttpClientTransport::send(std::string_view frame) {
        if (!started_.load(std::memory_order_acquire) || closed_.load(std::memory_order_acquire)) {
    ...
    void HttpClientTransport::worker_loop() {
        ...
        while (true) {
            std::string frame;
            {
                ...
                out_cv_.wait(lk, [this] { return closed_.load(std::memory_order_acquire) || !out_.empty(); });
                if (closed_.load(std::memory_order_acquire) && out_.empty())
                    return;
    ...
    void HttpClientTransport::process_send(std::string frame) {
        ...
        if (auto it = res->headers.find("Mcp-Session-Id"); it != res->headers.end() && !it->second.empty()) {
        ...
        if (res->status == 401 || res->status == 403) {
            ...
            if (auto it = res->headers.find("WWW-Authenticate"); it != res->headers.end()) {
                challenge = it->second;
            }
            bool expected = true;
            if (opts_.on_unauthorized
                && auth_callback_armed_.compare_exchange_strong(expected, false, std::memory_order_acq_rel)) {
                try {
                    opts_.on_unauthorized(res->status, challenge);
                }
                catch (...) {
                }
            }
        ...
        if (ct.find("text/event-stream") != std::string::npos) {
            ...
            drain_sse(buf, [this](SseEvent se) {
                ...
                if (!se.data.empty())
                    deliver_frame(std::move(se.data));
            });
            return;
        }
    ...
    void HttpClientTransport::run_get_stream() noexcept {
        ...
        while (started_.load(std::memory_order_acquire) && !closed_.load(std::memory_order_acquire)) {
            std::optional<std::string> sid;
            {
                std::unique_lock<std::mutex> lk(session_id_mu_);
                session_id_cv_.wait(lk, [this] {
                    return session_id_.has_value() || closed_.load(std::memory_order_acquire);
                });
            ...
            httplib::Result res;
            try {
                res = impl_->client_get.Get(path_.c_str(), headers, [&](const char* data, std::size_t n) {
                    if (closed_.load(std::memory_order_acquire))
                        return false;
                    buf.append(data, n);
                    drain_sse(buf, [this](SseEvent se) {
