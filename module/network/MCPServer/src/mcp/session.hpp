...
namespace mcp {
    ...
    public:
        ...
        [[nodiscard]] std::future<nlohmann::json> send_request(
            std::string method,
        ...
        /// background).
        template <class T>
        [[nodiscard]] std::future<T> send_request_for(std::string method,
            ...
            Pending pending;
            pending.resolve = [pr, convert = std::move(convert)](nlohmann::json j) {
                try {
        ...
        [[nodiscard]] std::future<void> send_request_void(std::string method,
                                                          nlohmann::json params             = nullptr,
                                                          std::chrono::milliseconds timeout = std::chrono::milliseconds{
                                                              0});

        /// Send a JSON-RPC notification (fire-and-forget).
        std::error_code send_notification(std::string method, nlohmann::json params = nullptr);

        [[nodiscard]] bool is_open() const noexcept {
            return started_.load(std::memory_order_acquire) && !closed_.load(std::memory_order_acquire);
        }
