...
namespace mcp {
    ...
    namespace detail {
        ...
        public:
            ...
            std::optional<nlohmann::json> wait_result(const std::string& id, std::chrono::milliseconds timeout);

            /// Paginated listing in creation order.
            [[nodiscard]] ListTasksResult list(std::optional<std::string> cursor, std::size_t page_size) const;
        ...
        private:
            ...
            void touch_locked(Entry& e, TaskStatus new_status, std::optional<std::string> message) {
                e.task.status          = new_status;
                e.task.last_updated_at = iso_utc_now();
                if (message.has_value())
                    e.task.status_message = std::move(*message);
            }
        ...
        std::optional<nlohmann::json> TaskStore::wait_result(const std::string& id, std::chrono::milliseconds timeout) {
            std::unique_lock<std::mutex> lk(mu_);
            if (!cv_.wait_for(lk, timeout, [&] {
                    auto it = entries_.find(id);
                    if (it == entries_.end())
                        return true;  // gone — also "terminal"
                    return is_terminal(it->second->task.status) || shutting_down_.load(std::memory_order_acquire);
                })) {
            ...
            if (it == entries_.end()) {
                ...
                throw Error{error_code::invalid_request, "task not found: " + id};
            }
            auto& e = *it->second;
            switch (e.task.status) {
                case TaskStatus::completed:
                    ...
                    throw Error{error_code::internal_error, "task completed without a result payload"};
                case TaskStatus::failed:
                    throw Error{error_code::internal_error,
                                "task failed: " + e.error.value_or(std::string{"<no message>"})};
                case TaskStatus::cancelled: throw Error{error_code::invalid_request, "task cancelled: " + id};
                default:
                    // Reached on shutdown if not terminal. Treat as failure.
                    throw Error{error_code::internal_error, "server shutting down before task completed"};
            }
        }

        ListTasksResult TaskStore::list(std::optional<std::string> cursor, std::size_t page_size) const {
            std::lock_guard<std::mutex> lk(mu_);
            ...
            if (cursor.has_value() && !cursor->empty()) {
                ...
                for (char c : s) {
                    if (c < '0' || c > '9') {
                        throw Error{error_code::invalid_params, "invalid pagination cursor: " + s};
                    }
                }
                try {
                    offset = static_cast<std::size_t>(std::stoull(s));
                }
                catch (...) {
                    throw Error{error_code::invalid_params, "invalid pagination cursor: " + s};
                }
            ...
            const std::size_t end = (page_size == 0) ? order_.size() : std::min(order_.size(), offset + page_size);
            ...
            if (end < order_.size())
                out.next_cursor = std::to_string(end);
            return out;
        }
        ...
        std::size_t TaskStore::gc_expired() {
            ...
            while (it != entries_.end()) {
                ...
                if (is_terminal(e.task.status) && e.task.ttl.has_value()) {
                    const auto age =
                        std::chrono::duration_cast<std::chrono::milliseconds>(now - e.created_steady).count();
                    if (age >= *e.task.ttl) {
    ...
    Server::Server(Implementation server_info) : server_info_(std::move(server_info)) {}
    ...
    namespace {
        ...
        template <typename T>
        std::pair<std::vector<T>, std::optional<std::string>> paginate(const std::vector<T>& items,
                                                                       std::size_t offset,
                                                                       std::size_t page_size) {
            if (offset >= items.size())
                return {{}, std::nullopt};
            if (page_size == 0 || offset + page_size >= items.size()) {
                return {{items.begin() + static_cast<std::ptrdiff_t>(offset), items.end()}, std::nullopt};
            }
    ...
    Server& Server::tool(std::string name, nlohmann::json input_schema, ToolHandler handler, ToolMetadata meta) {
        Tool t{
            .name          = name,
            .title         = std::nullopt,
    ...
    bool Server::log(LoggingLevel level, nlohmann::json data, std::optional<std::string> logger) {
        if (!logging_enabled_.load(std::memory_order_acquire))
            return false;
        if (static_cast<int>(level) < static_cast<int>(log_level_.load(std::memory_order_acquire)))
            return false;
        ...
        auto ec = session->send_notification(std::string{method_notifications_message}, nlohmann::json(params));
        return !ec;
    }
    ...
    void Server::report_progress(const ProgressToken& token,
        ...
        (void) session->send_notification(std::string{method_notifications_progress}, nlohmann::json(params));
    }

    std::future<CreateMessageResult> Server::sample(CreateMessageRequestParams params) {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "Server::sample: server is not running"};
        }
    ...
    std::future<ListRootsResult> Server::list_roots() {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "Server::list_roots: server is not running"};
        }
        return session->send_request_for<ListRootsResult>(std::string{method_roots_list},
                                                          nullptr,
                                                          [](nlohmann::json j) { return j.get<ListRootsResult>(); });
    }

    std::future<ElicitResult> Server::elicit(ElicitRequestParams params) {
        return elicit(std::move(params), std::chrono::milliseconds{0});
    }

    std::future<ElicitResult> Server::elicit(ElicitRequestParams params, std::chrono::milliseconds timeout) {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "Server::elicit: server is not running"};
        }
    ...
    void Server::set_elicitation_complete_handler(ElicitationCompleteHandler handler) {
        std::lock_guard<std::mutex> lk(completion_mu_);
        elicitation_complete_handler_ = std::move(handler);
    }

    std::error_code Server::notify_elicitation_complete(std::string elicitation_id) {
        auto session = acquire_session();
        if (!session)
            return std::make_error_code(std::errc::not_connected);
        return session->send_notification(std::string{method_notifications_elicitation_complete},
    ...
    Server& Server::enable_tasks(std::optional<std::int64_t> default_ttl_ms, std::size_t max_concurrent) {
        if (!tasks_)
            tasks_ = std::make_unique<detail::TaskStore>();
    ...
    nlohmann::json Server::handle_initialize(const nlohmann::json& params) {
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid initialize params: "} + e.what()};
        }
        ...
        {
            std::lock_guard<std::mutex> lk(resources_mu_);
            if (!resources_.empty() || !resource_templates_.empty() || fallback_resource_handler_) {
    ...
    nlohmann::json Server::handle_list_tools(const nlohmann::json& params) {
        ...
        auto parsed = params.is_null() ? ListToolsRequestParams{} : params.get<ListToolsRequestParams>();
        std::vector<Tool> all;
        {
            std::lock_guard<std::mutex> lk(tools_mu_);
    ...
    nlohmann::json Server::handle_call_tool(const nlohmann::json& params) {
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid tools/call params: "} + e.what()};
        }
        ...
        std::optional<TaskSupport> tool_task_support;
        {
            ...
            if (it == tools_.end()) {
                throw Error{error_code::method_not_found, "tool not found: " + parsed.name};
            }
            h = it->second.handler;
        ...
        if (tool_task_support.has_value()) {
            if (*tool_task_support == TaskSupport::required && !parsed.task.has_value()) {
                throw Error{error_code::invalid_request,
                            "tool \"" + parsed.name + "\" requires task augmentation; pass `task` in params"};
            }
            if (*tool_task_support == TaskSupport::forbidden && parsed.task.has_value()) {
                throw Error{error_code::invalid_request, "tool \"" + parsed.name + "\" forbids task augmentation"};
            }
        }
        ...
        if (parsed.task.has_value()) {
            ...
            if (tasks_max_concurrent_ > 0
                && static_cast<std::size_t>(tasks_->workers_inflight()) >= tasks_max_concurrent_) {
                throw Error{error_code::invalid_request,
                            "task concurrency limit reached (" + std::to_string(tasks_max_concurrent_) + ")"};
            }
            // Resolve effective TTL: per-call override wins, else the
            // server-wide default.
            const auto ttl  = parsed.task->ttl.has_value() ? parsed.task->ttl : tasks_default_ttl_ms_;
            ...
            std::thread([store = tasks_.get(), h = std::move(h), args = std::move(args), id]() mutable {
                try {
    ...
    nlohmann::json Server::handle_list_resources(const nlohmann::json& params) {
        ...
        auto parsed = params.is_null() ? ListResourcesRequestParams{} : params.get<ListResourcesRequestParams>();
    ...
    nlohmann::json Server::handle_list_resource_templates(const nlohmann::json& params) {
        ...
        auto parsed =
            params.is_null() ? ListResourceTemplatesRequestParams{} : params.get<ListResourceTemplatesRequestParams>();
    ...
    nlohmann::json Server::handle_read_resource(const nlohmann::json& params) {
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid resources/read params: "} + e.what()};
        }
        ...
        if (!h) {
            throw Error{error_code::method_not_found, "resource not found: " + parsed.uri};
        }
        return h(parsed.uri);
    }
    ...
    nlohmann::json Server::handle_list_prompts(const nlohmann::json& params) {
        ...
        auto parsed = params.is_null() ? ListPromptsRequestParams{} : params.get<ListPromptsRequestParams>();
    ...
    nlohmann::json Server::handle_get_prompt(const nlohmann::json& params) {
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid prompts/get params: "} + e.what()};
        }
        ...
        if (!h) {
            throw Error{error_code::method_not_found, "prompt not found: " + parsed.name};
        }
        const auto args = parsed.arguments.value_or(std::unordered_map<std::string, std::string>{});
        return h(args);
    }
    ...
    nlohmann::json Server::handle_set_level(const nlohmann::json& params) {
        if (!logging_enabled_.load(std::memory_order_acquire)) {
            throw Error{error_code::method_not_found, "logging capability is not enabled"};
        }
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid logging/setLevel: "} + e.what()};
        }
    ...
    nlohmann::json Server::handle_complete(const nlohmann::json& params) {
        ...
        if (!h) {
            throw Error{error_code::method_not_found, "completion is not enabled on this server"};
        }
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid completion/complete params: "} + e.what()};
        }
    ...
    nlohmann::json Server::handle_get_task(const nlohmann::json& params) {
        ...
        if (!tasks_) {
            throw Error{error_code::method_not_found, "tasks: server has not enabled the tasks capability"};
        }
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid tasks/get params: "} + e.what()};
        }
        auto t = tasks_->get(parsed.task_id);
        if (!t.has_value()) {
            throw Error{error_code::invalid_request, "task not found: " + parsed.task_id};
        }
        return *t;
    }
    ...
    nlohmann::json Server::handle_get_task_result(const nlohmann::json& params) {
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid tasks/result params: "} + e.what()};
        }
        ...
        if (value->is_object()) {
            ...
            meta[std::string{tasks_related_task_meta_key}] = nlohmann::json{{"taskId", parsed.task_id}};
        }
        return *value;
    }
    ...
    nlohmann::json Server::handle_list_tasks(const nlohmann::json& params) {
        ...
        if (!params.is_null()) {
            try {
                ...
                throw Error{error_code::invalid_params, std::string{"invalid tasks/list params: "} + e.what()};
            }
        }
    ...
    nlohmann::json Server::handle_cancel_task(const nlohmann::json& params) {
        ...
        try {
            ...
            throw Error{error_code::invalid_params, std::string{"invalid tasks/cancel params: "} + e.what()};
        }
    ...
    void Server::run(std::unique_ptr<Transport> transport) {
        ...
        if (tasks_) {
            ...
            tasks_->set_status_listener([weak](const Task& t) {
                auto s = weak.lock();
                if (!s)
                    return;
                (void) s->send_notification(std::string{method_notifications_tasks_status},
        ...
        local->set_notification_handler(
            ...
            [this](const nlohmann::json& params) {
                ...
                try {
                    ...
                    MCP_LOG_WARN(std::string{"malformed elicitation/complete: "} + e.what());
                    return;
                }
                catch (...) {
                ...
                try {
                    ...
                    MCP_LOG_ERROR(std::string{"elicitation/complete handler threw: "} + e.what());
                }
                catch (...) {
        ...
        local->set_notification_handler(std::string{method_notifications_initialized}, [](const nlohmann::json&) {
...

        {
            ...
            stop_cv_.wait(lk, [this, &local] {
                return stop_requested_.load(std::memory_order_acquire) || !local->is_open();
            });
        }
        local->close();
        {
