...
namespace mcp {
    ...
    Session::Session(std::unique_ptr<Transport> transport) : Session(std::move(transport), Options{}) {}
    ...
    Session::Session(std::unique_ptr<Transport> transport, Options opts)
        ...
        transport_->on_error([](std::error_code ec) { MCP_LOG_WARN(std::string{"transport error: "} + ec.message()); });
    ...
    void Session::close() {
        ...
        bool expected    = false;
        const bool first = closed_.compare_exchange_strong(expected, true, std::memory_order_acq_rel);
    ...
    std::future<nlohmann::json> Session::send_request(std::string method,
    ...
    std::future<void> Session::send_request_void(std::string method,
                                                 nlohmann::json params,
                                                 std::chrono::milliseconds timeout) {
        ...
        register_pending(std::move(method), std::move(params), std::move(pending), timeout);
        return fut;
    }
    ...
    void Session::register_pending(std::string method,
        ...
        if (state == Parked::kClosed) {
            reject(std::make_exception_ptr(Error{error_code::internal_error, "session closed"}));
            return;
        }
        if (state == Parked::kDuplicate) {
            reject(std::make_exception_ptr(
                Error{error_code::internal_error, "internal: duplicate request id " + id.canonical()}));
            return;
        }
        ...
        try {
            ...
            if (found && parked.reject)
                parked.reject(std::current_exception());
        }
    }

    std::error_code Session::send_notification(std::string method, nlohmann::json params) {
    ...
    void Session::handle_notification(JsonRpcNotification note) {
        NotificationHandler handler;
        {
            ...
            if (it != note_handlers_.end())
                handler = it->second;
            else
                handler = fallback_note_;
        }
        if (!handler)
            return;
        try {
    ...
    void Session::handle_response(JsonRpcResponse resp) {
        ...
        if (resp.is_success()) {
            if (pending.resolve)
                pending.resolve(std::move(resp).result());
        }
        else {
            if (pending.reject)
        ...
        while (!closed_.load(std::memory_order_acquire)) {
            ...
            if (timeout_cv_.wait_until(lk, soonest) == std::cv_status::timeout) {
                ...
                for (auto& reject : expired) {
                    reject(std::make_exception_ptr(Error{error_code::internal_error, "request timed out"}));
                }
                lk.lock();
            }
        }
    ...
    void Session::cancel_all_pending(const ErrorObject& reason) {
        ...
        for (auto& [id, p] : drained) {
            (void) id;
            if (p.reject)
                p.reject(std::make_exception_ptr(Error{reason}));
        }
    }
