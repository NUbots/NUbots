...
namespace mcp {
    ...
    namespace {
        ...
        public:
            ...
            void close() override {
                ...
                for (auto& [id, p] : drained) {
                    try {
                        throw Error{error_code::internal_error, "session closed"};
                    }
                    catch (...) {
            ...
            [[nodiscard]] std::error_code send(std::string_view frame) override {
                ...
                if (has_id && !has_method) {
                    ...
                    std::promise<std::string> awaiter;
                    {
                        ...
                        if (it == pending_.end()) {
                            MCP_LOG_WARN("HttpSessionTransport: no awaiter for response id=" + key);
                            return std::make_error_code(std::errc::invalid_argument);
                        }
            ...
            std::optional<std::string> wait_get_frame(std::chrono::duration<Rep, Period> timeout) {
                std::unique_lock<std::mutex> lk(get_mu_);
                if (!get_cv_.wait_for(lk, timeout, [&] {
                        return !get_queue_.empty() || closed_.load(std::memory_order_acquire);
                    }))
                    return std::nullopt;
                if (get_queue_.empty())
                    return std::nullopt;
            ...
            std::optional<std::future<std::string>> deliver_request_or_notification(std::string raw) {
                nlohmann::json j;
                try {
                ...
                const bool is_request = j.contains("id") && !j["id"].is_null() && j.contains("method");
        ...
        public:
            explicit SharedTransportShim(std::shared_ptr<HttpSessionTransport> inner) : inner_(std::move(inner)) {}

            void on_message(MessageCallback cb) override {
                inner_->on_message(std::move(cb));
            }
            void on_error(ErrorCallback cb) override {
                inner_->on_error(std::move(cb));
            }
            void on_close(CloseCallback cb) override {
                inner_->on_close(std::move(cb));
            }
            void start() override {
                inner_->start();
            }
        ...
        void invoke_session_closed(const HttpServerHost::Options& opts, Server& server) {
            if (!opts.on_session_closed)
                return;
            try {
                opts.on_session_closed(server);
            }
            catch (const std::exception& e) {
                MCP_LOG_ERROR(std::string{"HttpServerHost: on_session_closed threw: "} + e.what());
            }
            catch (...) {
                MCP_LOG_ERROR(
                    "HttpServerHost: on_session_closed threw a non-std "
                    "exception");
            }
        }
    ...
    struct HttpServerHost::Impl {
        ...
        std::unordered_map<std::string, std::shared_ptr<HttpServerHost::SessionContext>> sessions;
    };
    ...
    HttpServerHost::HttpServerHost(Implementation server_info, Options opts, SessionFactory factory)
    ...
    int HttpServerHost::port() const noexcept {
    ...
    namespace {
        ...
        bool origin_allowed(const httplib::Request& req, const std::vector<std::string>& allow) {
            auto it = req.headers.find("Origin");
            if (it == req.headers.end())
                return true;  // no Origin header -> not a browser-initiated XS request
            if (allow.empty())
                return false;
        ...
        inline std::string resolve_cors_origin(const httplib::Request& req, const std::vector<std::string>& allow) {
        ...
        inline bool authorize(const httplib::Request& req,
            ...
            if (count_auth == 0) {
                ...
                res.set_header(
                    "WWW-Authenticate",
                    build_bearer_challenge(opts.auth_realm, opts.resource_metadata_url, "invalid_request", {}));
            ...
            switch (outcome.status) {
                case HttpServerHost::Options::BearerStatus::allow: return true;
                case HttpServerHost::Options::BearerStatus::invalid_token:
                    res.status = 401;
                    res.set_header(
                        "WWW-Authenticate",
                        build_bearer_challenge(opts.auth_realm, opts.resource_metadata_url, "invalid_token", {}));
    ...
    void HttpServerHost::start() {
        ...
        if (opts_.path.empty() || opts_.path.front() != '/') {
            started_.store(false, std::memory_order_release);
            throw std::invalid_argument("HttpServerHost: path must begin with '/'");
        }
        ...
        const bool is_loopback = (opts_.host == "127.0.0.1" || opts_.host == "::1" || opts_.host == "localhost");
        if (opts_.bearer_validator && !is_loopback && !opts_.allow_insecure_http) {
            started_.store(false, std::memory_order_release);
            throw std::invalid_argument(
        ...
        srv.Post(opts_.path, [host](const httplib::Request& req, httplib::Response& res) {
            ...
            if (auto it = req.headers.find("MCP-Protocol-Version");
                it != req.headers.end() && !valid_protocol_version_header(it->second)) {
                res.status = 400;
                res.set_content("unsupported MCP-Protocol-Version: " + it->second, "text/plain");
                return;
            }
            if (!authorize(req, res, host->opts_))
                return;
            ...
            if (!ctx) {
                if (!sid.empty()) {
                    ...
                    res.status = 404;
                    res.set_content("unknown or expired session id", "text/plain");
                }
                else {
                    // No session id and not an initialize — malformed
                    // usage rather than a stale session.
                    res.status = 400;
                    res.set_content("missing session id; send initialize first", "text/plain");
                }
                return;
            }
            ...
            try {
                ...
                res.status = 500;
                res.set_content(std::string{"handler error: "} + e.what(), "text/plain");
            }
        });
        ...
        srv.Get(opts_.path, [host](const httplib::Request& req, httplib::Response& res) {
            if (!origin_allowed(req, host->opts_.allowed_origins)) {
                res.status = 403;
                res.set_content("Origin not allowed", "text/plain");
                return;
            ...
            res.set_chunked_content_provider(
                "text/event-stream",
                [transport](std::size_t /*offset*/, httplib::DataSink& sink) -> bool {
                ...
                [transport](bool /*success*/) { transport->leave_get_stream(); });
        });

        // DELETE: terminate session.
        srv.Delete(opts_.path, [host](const httplib::Request& req, httplib::Response& res) {
            if (!origin_allowed(req, host->opts_.allowed_origins)) {
                res.status = 403;
                res.set_content("Origin not allowed", "text/plain");
                return;
        ...
        if (opts_.enable_cors_preflight) {
            srv.Options(opts_.path, [host](const httplib::Request& req, httplib::Response& res) {
                const auto acao = resolve_cors_origin(req, host->opts_.allowed_origins);
                if (acao.empty()) {
                    res.status = 403;
                    return;
                }
        ...
        if (opts_.resource_metadata.has_value()) {
            ...
            const auto handler     = [body](const httplib::Request&, httplib::Response& res) {
            ...
            if (opts_.path.size() > 1) {
                srv.Get("/.well-known/oauth-protected-resource" + opts_.path, handler);
            }
        }
        ...
        if (opts_.port != 0) {
            if (!impl_->http.bind_to_port(opts_.host, opts_.port)) {
                started_.store(false, std::memory_order_release);
                throw std::runtime_error("HttpServerHost: bind failed for " + opts_.host + ":"
            ...
            if (impl_->port < 0) {
                started_.store(false, std::memory_order_release);
                throw std::runtime_error("HttpServerHost: bind failed for " + opts_.host + " (OS-assigned port)");
            }
        }
