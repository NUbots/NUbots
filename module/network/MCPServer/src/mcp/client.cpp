...
namespace mcp {
    ...
    Client::Client(Implementation client_info) : client_info_(std::move(client_info)) {}
    ...
    std::future<InitializeResult> Client::initialize() {
        ...
        ClientCapabilities caps;
        {
            ...
            if (capabilities_override_.has_value()) {
                ...
                if (o.experimental.has_value())
                    caps.experimental = o.experimental;
                if (o.roots.has_value())
                    caps.roots = o.roots;
                if (o.sampling.has_value())
                    caps.sampling = o.sampling;
                if (o.elicitation.has_value())
                    caps.elicitation = o.elicitation;
                if (o.tasks.has_value())
                    caps.tasks = o.tasks;
            }
        }
        ...
        return std::async(std::launch::async, [this, session, payload = nlohmann::json(params)]() -> InitializeResult {
            auto raw = session->send_request(std::string{method_initialize}, payload).get();
            auto res = raw.get<InitializeResult>();
            {
                std::lock_guard<std::mutex> lk(server_mu_);
                server_ = res;
            }
            const auto note_ec = session->send_notification(std::string{method_notifications_initialized});
            if (note_ec) {
                throw Error{error_code::internal_error,
                            "failed to send notifications/initialized: " + note_ec.message()};
            }
            return res;
        });
    }
    ...
    std::future<ListToolsResult> Client::list_tools(std::optional<std::string> cursor) {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "client not connected"};
        }
        ListToolsRequestParams params{.cursor = std::move(cursor)};
        return session->send_request_for<ListToolsResult>(std::string{method_tools_list},
                                                          nlohmann::json(params),
                                                          [](nlohmann::json j) { return j.get<ListToolsResult>(); });
    }

    std::future<CallToolResult> Client::call_tool(std::string name, nlohmann::json arguments) {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "client not connected"};
        }
        CallToolRequestParams params{
            .name      = std::move(name),
            .arguments = arguments.is_null() ? std::nullopt : std::optional<nlohmann::json>(std::move(arguments)),
            .task      = std::nullopt,
        };
        return session->send_request_for<CallToolResult>(std::string{method_tools_call},
                                                         nlohmann::json(params),
                                                         [](nlohmann::json j) { return j.get<CallToolResult>(); });
    }

    std::future<CreateTaskResult> Client::call_tool_as_task(std::string name,
        ...
        return session->send_request_for<CreateTaskResult>(std::string{method_tools_call},
                                                           nlohmann::json(params),
    ...
    std::future<Task> Client::task_get(std::string task_id) {
        ...
        return session->send_request_for<Task>(std::string{method_tasks_get},
    ...
    std::future<ListTasksResult> Client::task_list(std::optional<std::string> cursor) {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "client not connected"};
        }
        ListTasksRequestParams params{.cursor = std::move(cursor)};
        return session->send_request_for<ListTasksResult>(std::string{method_tasks_list},
                                                          nlohmann::json(params),
    ...
    std::future<Task> Client::task_cancel(std::string task_id) {
        ...
        return session->send_request_for<Task>(std::string{method_tasks_cancel},
    ...
    void Client::set_task_status_handler(TaskStatusHandler handler) {
        ...
        if (handler) {
            session->set_notification_handler(method, [h = std::move(handler)](const nlohmann::json& params) {
                if (params.is_null())
                    return;
                Task projection;
                try {
    ...
    std::future<ListResourcesResult> Client::list_resources(std::optional<std::string> cursor) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
    ...
    std::future<ListResourceTemplatesResult> Client::list_resource_templates(std::optional<std::string> cursor) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
    ...
    std::future<ReadResourceResult> Client::read_resource(std::string uri) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
    ...
    std::future<void> Client::subscribe(std::string uri) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
        SubscribeRequestParams params{.uri = std::move(uri)};
        return session->send_request_void(std::string{method_resources_subscribe}, nlohmann::json(params));
    }

    std::future<void> Client::unsubscribe(std::string uri) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
        UnsubscribeRequestParams params{.uri = std::move(uri)};
        return session->send_request_void(std::string{method_resources_unsubscribe}, nlohmann::json(params));
    }
    ...
    void Client::set_resources_list_changed_handler(ListChangedHandler handler) {
        ...
        if (handler) {
            session->set_notification_handler(method, [h = std::move(handler)](const nlohmann::json&) { h(); });
        }
        else {
            session->clear_notification_handler(method);
        }
    ...
    std::future<ListPromptsResult> Client::list_prompts(std::optional<std::string> cursor) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
        ListPromptsRequestParams params{.cursor = std::move(cursor)};
        return session->send_request_for<ListPromptsResult>(
            std::string{method_prompts_list},
            nlohmann::json(params),
            [](nlohmann::json j) { return j.get<ListPromptsResult>(); });
    }

    std::future<GetPromptResult> Client::get_prompt(
        std::string name,
        ...
        return session->send_request_for<GetPromptResult>(std::string{method_prompts_get},
                                                          nlohmann::json(params),
    ...
    std::error_code Client::cancel_request(RequestId request_id, std::optional<std::string> reason) {
        auto session = acquire_session();
        if (!session) {
            throw Error{error_code::internal_error, "client not connected"};
        }
        ...
        return session->send_notification(std::string{method_notifications_cancelled}, nlohmann::json(params));
    }
    ...
    std::future<void> Client::set_log_level(LoggingLevel level) {
        ...
        return session->send_request_void(std::string{method_logging_set_level}, nlohmann::json(params));
    }
        ...
        if (handler) {
            session->set_request_handler(
                ...
                [h = std::move(handler)](const nlohmann::json& params) -> nlohmann::json {
                    if (params.is_null()) {
                        throw Error{error_code::invalid_params, "sampling/createMessage requires params"};
                    }
                    ...
                    try {
                        ...
                        throw Error{error_code::invalid_params, std::string{"invalid sampling params: "} + e.what()};
                    }
                    return h(parsed);
                });
        }
    ...
    void Client::set_elicitation_handler(ElicitationHandler handler) {
        ...
        if (handler) {
            session->set_request_handler(
                ...
                [h = std::move(handler)](const nlohmann::json& params) -> nlohmann::json {
                    if (params.is_null()) {
                        throw Error{error_code::invalid_params, "elicitation/create requires params"};
                    }
                    ...
                    try {
                        ...
                        throw Error{error_code::invalid_params, std::string{"invalid elicitation params: "} + e.what()};
                    }
                    return h(parsed);
                });
        }
    ...
    void Client::set_elicitation_complete_handler(ElicitationCompleteHandler handler) {
        auto session = acquire_session();
        if (!session)
            throw Error{error_code::internal_error, "client not connected"};
        {
    ...
    void Client::set_roots_list_handler(RootsListHandler handler) {
        ...
        if (handler) {
            session->set_request_handler(method, [h = std::move(handler)](const nlohmann::json&) -> nlohmann::json {
                return h();
            });
        }
        else {
    ...
    std::future<CompleteResult> Client::complete(
        ...
        return session->send_request_for<CompleteResult>(std::string{method_completion_complete},
                                                         nlohmann::json(params),
    ...
    std::error_code Client::notify_roots_list_changed() {
        ...
        return session->send_notification(std::string{method_notifications_roots_list_changed});
    }

    std::error_code Client::notify_elicitation_complete(std::string elicitation_id) {
        auto session = acquire_session();
        if (!session)
            return std::make_error_code(std::errc::not_connected);
