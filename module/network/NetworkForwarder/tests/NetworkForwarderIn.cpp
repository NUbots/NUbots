...
namespace module::network::test::forwarder::in {
    ...
    private:
        ...
        auto configure_no_forwarding() {
            return [this] {
                emit<Scope::INLINE>(std::make_unique<NetworkForwarderConfig>());
    ...
    public:
        TestReactor(std::unique_ptr<NUClear::Environment> environment) : MockNetworkReactor(std::move(environment)) {
            ...
            on<Trigger<Step<3>>, MainThread>().then(configure_no_forwarding());
...
SCENARIO("Messages are forwarded internally as configured", "[module][network][NetworkForwarder][NetworkForwarderIn]") {
    ...
    const auto& test_reactor = plant.install<module::network::test::forwarder::in::TestReactor>();
