...
namespace module::network::test::forwarder::out::disable {
    ...
    public:
        TestReactor(std::unique_ptr<NUClear::Environment> environment) : MockNetworkReactor(std::move(environment)) {
            ...
            // AND WHEN the network forwarder is reconfigured to disable forwarding for the same message types
            on<Trigger<Step<5>>, MainThread>().then(disable_test_forwarding("Target 1"));
...
SCENARIO("Outbound message forwarding is disabled as configured",
         "[module][network][NetworkForwarder][NetworkForwarderOutDisable]") {
    ...
    const auto& test_reactor = plant.install<module::network::test::forwarder::out::disable::TestReactor>();
    ...
    const std::vector<std::string> expected_events = {"Tester<LOCAL> -> Config, Target 1, 'Test' enabled",
                                                      ...
                                                      "Tester<LOCAL> -> NetworkJoin, Target 1",
                                                      "Forwarder<NETWORK> -> Test, Target 1, Test 1",
                                                      "Forwarder<NETWORK> <- MessageRequest, Target 1",
                                                      "Forwarder<NETWORK> -> Test, Target 1, Test 1",
