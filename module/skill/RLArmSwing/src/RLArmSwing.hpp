#ifndef MODULE_SKILL_RLARMSWING_HPP
#define MODULE_SKILL_RLARMSWING_HPP
...
namespace module::skill {
    ...
    static constexpr int JOINT_POS_SIZE    = 4;  // Only using the shoulder and elbow pitch joints
    static constexpr int COMMAND_SIZE      = 4;
    static constexpr int JOINT_ACTION_SIZE = 4;  // Only controlling the shoulder and elbow pitch joints
    static constexpr int TOTAL_OBS_SIZE    = JOINT_POS_SIZE + JOINT_POS_SIZE + JOINT_ACTION_SIZE + COMMAND_SIZE;  // 16
    ...
    using JointVector = Eigen::Matrix<double, 20, 1>;
    /// @brief Fixed-size command vector type
    using CommandVector = Eigen::Matrix<double, COMMAND_SIZE, 1>;

    class RLArmSwing : public ::extension::behaviour::BehaviourReactor {
    private:
    ...
    public:
        ...
        explicit RLArmSwing(std::unique_ptr<NUClear::Environment> environment);
    ...
    private:
        ...
        Eigen::Vector4d run_inference(const ObservationVector& observation);
    };

}  // namespace module::skill

#endif  // MODULE_SKILL_RLARMSWING_HPP
