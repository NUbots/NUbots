...
namespace module::skill {
    ...
    RLArmSwing::RLArmSwing(std::unique_ptr<NUClear::Environment> environment)
        : BehaviourReactor(std::move(environment)) {

        on<Configuration>("RLArmSwing.yaml").then([this](const Configuration& config) {
            // Use configuration here from file RLArmSwing.yaml
            ...
            // Initialize vectors
            last_action      = Eigen::Vector4d::Zero();  // Can't be a JointVector as we only control 4 joints
        ...
        on<Every<1000, Per<std::chrono::milliseconds>>, Single>().then([this] {
            ...
            if (initialise_model_locked()) {
                log<INFO>("RLArmSwing inference model initialised successfully");
            }
           ...
           Priority::HIGH>()
            .then([this](const WalkTask& walk_task,
                ...
                if (stability >= Stability::DYNAMIC) {
                    ...
                    // Extract the relevant joints for the policy (shoulder and elbow pitch joints)
                    Eigen::Vector4d subset_current_joints_rel_mj = current_joints_rel_mj({14, 16, 17, 19});
                    observation.segment<JOINT_POS_SIZE>(idx)     = subset_current_joints_rel_mj;
                    if (log_level <= DEBUG) {
                    ...
                    // Extract the relevant joints for the policy (shoulder and elbow pitch joints)
                    Eigen::Vector4d subset_current_velocities_mj = current_velocities_mj({14, 16, 17, 19});
                    observation.segment<JOINT_POS_SIZE>(idx)     = subset_current_velocities_mj;
                    if (log_level <= DEBUG) {
                    ...
                    // Command (4)
                    Eigen::VectorXd command = Eigen::VectorXd(COMMAND_SIZE);
                    command << 1.0,  // Frequency (Hz)
                        1.0,         // Amplitude (rad)
                        1.0,         // Shoulders? (bool)
                        0.0;         // Elbows? (bool)
                    ...
                    // Last action (4)
                    observation.segment<JOINT_ACTION_SIZE>(idx) = last_action;
                    idx += JOINT_POS_SIZE;

                    // Run inference
                    Eigen::Vector4d inference_output_raw = run_inference(observation);
                    if (log_level <= DEBUG) {
                    ...
                    Eigen::Vector4d filtered_action_raw = inference_output_raw;
                    if (have_last_action) {
                        filtered_action_raw =
                    ...
                    const Eigen::Vector4d filtered_joint_offsets_mj = filtered_action_raw * NUGUS_ACTION_SCALE;

                    // Fill in the rest of the joint offset vector with zeros for the joints we are not controlling
                    // left_shoulder_pitch, left_elbow, right_shoulder_pitch, right_elbow are elements 14, 16, 17, 19 in
                    // Mjlab order
                    Eigen::Matrix<double, 20, 1> full_joint_offsets_mj = Eigen::Matrix<double, 20, 1>::Zero();
                    full_joint_offsets_mj(14) = filtered_joint_offsets_mj(0);  // left_shoulder_pitch
                    full_joint_offsets_mj(16) = filtered_joint_offsets_mj(1);  // left_elbow
                    full_joint_offsets_mj(17) = filtered_joint_offsets_mj(2);  // right_shoulder_pitch
                    full_joint_offsets_mj(19) = filtered_joint_offsets_mj(3);  // right_elbow


                    // Convert into NUbots order
                    JointVector joint_offsets_scaled_nubots = mjlab_to_nubots(full_joint_offsets_mj);
    ...
    void RLArmSwing::invalidate_model_locked() {
        compiled_model = ov::CompiledModel{};
        infer_request  = ov::InferRequest{};
        model_state    = ModelState::FAILED;
    }

    bool RLArmSwing::initialise_model_locked() {
        try {
            ...
            log<INFO>("RLArmSwing init model_path=",
                      abs,
                      " device=",
                      cfg.device,
                      " input_name=",
            ...
            if (in_shape.size() != 2 || in_shape[1] != static_cast<size_t>(TOTAL_OBS_SIZE)) {
                throw std::runtime_error("Unexpected input shape. Expected [1, " + std::to_string(TOTAL_OBS_SIZE)
                                         + "], got [" + std::to_string(in_shape[0]) + ", " + std::to_string(in_shape[1])
                                         + "]");
            }
            if (out_shape.size() != 2 || out_shape[1] != static_cast<size_t>(JOINT_ACTION_SIZE)) {
                throw std::runtime_error("Unexpected output shape. Expected [1, " + std::to_string(JOINT_ACTION_SIZE)
                                         + "], got [" + std::to_string(out_shape[0]) + ", "
            ...
            log<ERROR>("RLArmSwing model init failed: ", e.what());
    ...
    Eigen::Vector4d RLArmSwing::run_inference(const ObservationVector& observation) {
        if (log_level <= DEBUG) {
        ...
        try {
            ...
            Eigen::Vector4d joint_angles_raw;
            for (int i = 0; i < JOINT_ACTION_SIZE; ++i) {
