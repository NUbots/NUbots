...
namespace motion {
    ...
    void KinematicsConfiguration::configureLeg(KinematicsModel& model, const YAML::Node& objLeg) {
        const Eigen::Vector3f leg_hipOffset = objLeg["hip_offset"].as<Expression>();
    ...
    void KinematicsConfiguration::configureHead(KinematicsModel& model, const YAML::Node& objHead) {
        ...
        const Eigen::Vector3f head_neckToCamera = objHead["neck_to_camera"].as<Expression>();
        ...
        const Eigen::Vector3f neck_basePositionFromOrigin = objNeck["base_position_from_origin"].as<Expression>();
        ...
        const Eigen::Vector2f headMovementLimits_yaw   = objHeadMovementLimits["yaw"].as<Expression>();
        const Eigen::Vector2f headMovementLimits_pitch = objHeadMovementLimits["pitch"].as<Expression>();
    ...
    void KinematicsConfiguration::configureArm(KinematicsModel& model, const YAML::Node& objArm) {
        ...
        const Eigen::Vector2f shoulderOffset = objShoulder["offset"].as<Expression>();
        ...
        const Eigen::Vector2f upperArmOffset = objUpperArm["offset"].as<Expression>();
        ...
        const Eigen::Vector2f lowerArmOffset = objLowerArm["offset"].as<Expression>();
