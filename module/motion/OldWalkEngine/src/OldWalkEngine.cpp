...
OldWalkEngine::OldWalkEngine(std::unique_ptr<NUClear::Environment> environment)
  ...
  on<Trigger<EnableWalkEngineCommand>>().then(
      [this](const EnableWalkEngineCommand& command) {
  ...
  updateHandle = on<Every<UPDATE_FREQUENCY, Per<std::chrono::seconds>>,
                    With<Sensors>, Single, Priority::HIGH>()
  ...
  on<Trigger<WalkCommand>>().then([this](const WalkCommand& walkCommand) {
    ...
    velocity.x() *=
        velocity.x() > 0 ? velocityLimits(0, 1) : -velocityLimits(0, 0);
    velocity.y() *=
        velocity.y() > 0 ? velocityLimits(1, 1) : -velocityLimits(1, 0);
    velocity.angle() *=
        velocity.angle() > 0 ? velocityLimits(2, 1) : -velocityLimits(2, 0);
  ...
  on<Configuration>("OldWalkEngine.yaml")
      .then([this](const Configuration& config) { configure(config.config); });
  ...
  on<Trigger<WalkOptimiserCommand>>().then(
      [this](const WalkOptimiserCommand& command) {
...
void OldWalkEngine::configure(const YAML::Node& config) {
  ...
  stanceLimitY2 = kinematicsModel.leg.LENGTH_BETWEEN_LEGS -
                  stance["limit_margin_y"].as<Expression>();
  ...
  step_height_slow_fraction =
      walkCycle["step"]["height_slow_fraction"].as<float>();
  step_height_fast_fraction =
      walkCycle["step"]["height_fast_fraction"].as<float>();
...
void OldWalkEngine::generateAndSaveStandScript(const Sensors& sensors) {
  ...
  Script::Frame frame;
  frame.duration =
      std::chrono::milliseconds(int(round(1000 * STAND_SCRIPT_DURATION)));
  for (auto& waypoint : *waypoints) {
    frame.targets.push_back(Script::Frame::Target(
        {waypoint.id, waypoint.position, std::max(waypoint.gain, 60.0f), 100}));
  }
...
void OldWalkEngine::stanceReset() {
  ...
  if (startFromStep) {
    ...
    // stance resetted
    uLeftFoot = uTorso.localToWorld(
        {footOffset[0], kinematicsModel.leg.HIP_OFFSET_Y - footOffset[1], 0});
    uRightFoot = uTorso.localToWorld(
        {footOffset[0], -kinematicsModel.leg.HIP_OFFSET_Y + footOffset[1], 0});
    initialStep = 2;
  }
...
void OldWalkEngine::updateStep(double phase, const Sensors& sensors) {
  ...
  auto& limit = (velocityCurrent.x() > velocityHigh
                     ? accelerationLimitsHigh
                     : accelerationLimits);  // TODO: use a function instead
  float speed =
      std::min(1.0, std::max(std::abs(velocityCurrent.x() / (limit[0] * 10)),
                             std::abs(velocityCurrent.y() / (limit[1] * 10))));
  float scale =
      (step_height_fast_fraction - step_height_slow_fraction) * speed +
      step_height_slow_fraction;
  foot[2] *= scale;
  ...
  uTorso = zmpCom(phase, zmpCoefficients, zmpParams, stepTime, zmpTime,
                  phase1Single, phase2Single, uSupport, uLeftFootDestination,
                  uLeftFootSource, uRightFootDestination, uRightFootSource);
  ...
  Transform2D uTorsoActual =
      uTorso.localToWorld({-kinematicsModel.leg.HIP_OFFSET_X, 0, 0});
  Transform3D torso =
      arma::vec6({uTorsoActual.x(), uTorsoActual.y(), bodyHeight, 0, bodyTilt,
                  uTorsoActual.angle()});
  ...
  if (swingLeg == LimbID::LEFT_LEG) {
    rightFootTorso = rightFootTorso.rotateZLocal(
        ...
        convert<double, 4, 4>(
            sensors.forwardKinematics.find(ServoID::R_HIP_ROLL)->second));
  } else {
    leftFootTorso = leftFootTorso.rotateZLocal(
        hipRollCompensation * phaseComp,
        convert<double, 4, 4>(
            sensors.forwardKinematics.find(ServoID::L_HIP_ROLL)->second));
  }
  ...
  auto joints = calculateLegJointsTeamDarwin(kinematicsModel, leftFootTorso,
                                             rightFootTorso);
...
std::unique_ptr<std::vector<ServoCommand>> OldWalkEngine::updateStillWayPoints(
  ...
  if (balanceEnabled) {
    ...
    balancer.balance(kinematicsModel, rightFootTorso, LimbID::RIGHT_LEG,
                     sensors);
  }
...
std::unique_ptr<std::vector<ServoCommand>> OldWalkEngine::motionLegs(
  ...
  NUClear::clock::time_point time =
      NUClear::clock::now() +
      std::chrono::nanoseconds(std::nano::den / UPDATE_FREQUENCY);

  for (auto& joint : joints) {
    waypoints->push_back({subsumptionId, time, joint.first, joint.second,
...
std::unique_ptr<std::vector<ServoCommand>> OldWalkEngine::motionArms(
    double phase) {
  ...
  double leftMinValue =
      5 * M_PI / 180 + std::max(0.0, rotLeftA) / 2 +
      std::max(0.0, leftLegTorso.y() - 0.04) / 0.02 * (6 * M_PI / 180);
  double rightMinValue =
      -5 * M_PI / 180 - std::max(0.0, rotRightA) / 2 -
  ...
  NUClear::clock::time_point time =
      NUClear::clock::now() +
      std::chrono::nanoseconds(std::nano::den / UPDATE_FREQUENCY);
  waypoints->push_back({subsumptionId, time, ServoID::R_SHOULDER_PITCH,
                        float(qRArmActual[0]),
                        jointGains[ServoID::R_SHOULDER_PITCH], 100});
  waypoints->push_back({subsumptionId, time, ServoID::R_SHOULDER_ROLL,
                        float(qRArmActual[1]),
                        jointGains[ServoID::R_SHOULDER_ROLL], 100});
  waypoints->push_back({subsumptionId, time, ServoID::R_ELBOW,
                        float(qRArmActual[2]), jointGains[ServoID::R_ELBOW],
                        100});
  waypoints->push_back({subsumptionId, time, ServoID::L_SHOULDER_PITCH,
                        float(qLArmActual[0]),
                        jointGains[ServoID::L_SHOULDER_PITCH], 100});
  waypoints->push_back({subsumptionId, time, ServoID::L_SHOULDER_ROLL,
                        float(qLArmActual[1]),
                        jointGains[ServoID::L_SHOULDER_ROLL], 100});
  waypoints->push_back({subsumptionId, time, ServoID::L_ELBOW,
                        float(qLArmActual[2]), jointGains[ServoID::L_ELBOW],
                        100});

  return std::move(waypoints);
}

Transform2D OldWalkEngine::stepTorso(Transform2D uLeftFoot,
                                     Transform2D uRightFoot,
                                     double shiftFactor) {
  Transform2D uLeftFootSupport =
      uLeftFoot.localToWorld({-footOffset[0], -footOffset[1], 0});
  Transform2D uRightFootSupport =
      uRightFoot.localToWorld({-footOffset[0], footOffset[1], 0});
...
void OldWalkEngine::setVelocity(Transform2D velocity) {
  ...
  velocity.x() = std::min(std::max(velocity.x(), velocityLimits(0, 0)),
                          velocityLimits(0, 1));
  velocity.y() = std::min(std::max(velocity.y(), velocityLimits(1, 0)),
                          velocityLimits(1, 1));
  velocity.angle() = std::min(std::max(velocity.angle(), velocityLimits(2, 0)),
                              velocityLimits(2, 1));
  ...
  double magFactor =
      std::min(velocityLimits(0, 1) * vFactor, stepMag) / (stepMag + 0.000001);
  ...
  velocityCommand.x() =
      std::min(std::max(velocityCommand.x(), velocityLimits(0, 0)),
               velocityLimits(0, 1));
  velocityCommand.y() =
      std::min(std::max(velocityCommand.y(), velocityLimits(1, 0)),
               velocityLimits(1, 1));
  velocityCommand.angle() =
      std::min(std::max(velocityCommand.angle(), velocityLimits(2, 0)),
               velocityLimits(2, 1));
}

Transform2D OldWalkEngine::getVelocity() { return velocityCurrent; }

arma::vec2 OldWalkEngine::zmpSolve(double zs, double z1, double z2, double x1,
                                   double x2, double phase1Single,
                                   double phase2Single, double stepTime,
                                   double zmpTime) {
  /*
...
Transform2D OldWalkEngine::zmpCom(double phase, arma::vec4 zmpCoefficients,
                                  arma::vec4 zmpParams, double stepTime,
                                  double zmpTime, double phase1Single,
                                  double phase2Single, Transform2D uSupport,
  ...
  com.x() =
      uSupport.x() + zmpCoefficients[0] * expT + zmpCoefficients[1] / expT;
  com.y() =
      uSupport.y() + zmpCoefficients[2] * expT + zmpCoefficients[3] / expT;
  if (phase < phase1Single) {
    com.x() += zmpParams[0] * stepTime * (phase - phase1Single) -
               zmpTime * zmpParams[0] *
                   std::sinh(stepTime * (phase - phase1Single) / zmpTime);
    ...
    com.x() += zmpParams[2] * stepTime * (phase - phase2Single) -
               zmpTime * zmpParams[2] *
                   std::sinh(stepTime * (phase - phase2Single) / zmpTime);
  ...
  com.angle() =
      phase * (uLeftFootDestination.angle() + uRightFootDestination.angle()) /
          2 +
...
double OldWalkEngine::getTime() {
  return std::chrono::duration_cast<std::chrono::microseconds>(
             NUClear::clock::now().time_since_epoch())
             .count() *
         1E-6;
}
...
double OldWalkEngine::procFunc(double value, double deadband, double maxvalue) {
  return std::abs(
      std::min(std::max(0.0, std::abs(value) - deadband), maxvalue));
}
