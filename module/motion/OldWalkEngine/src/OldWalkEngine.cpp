...
namespace motion {
    ...
    OldWalkEngine::OldWalkEngine(std::unique_ptr<NUClear::Environment> environment)
        ...
        on<Configuration>("OldWalkEngine.yaml").then([this] (const Configuration& config) {
    ...
    std::unique_ptr<std::vector<ServoCommand>> OldWalkEngine::motionLegs(std::vector<std::pair<ServoID, float>> joints) {
