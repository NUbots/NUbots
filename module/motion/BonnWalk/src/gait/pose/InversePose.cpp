...
namespace pose {
    ...
    void InverseLegPose::fromJointAngles(double hipYaw,
                                         double hipRoll,
                                         double hipPitch,
                                         double kneePitch,
                                         double anklePitch,
    ...
    void InverseLegPose::getJointAngles(double& hipYaw,
                                        double& hipRoll,
                                        double& hipPitch,
                                        double& kneePitch,
                                        double& anklePitch,
        ...
        // - We assume that the lengths of the upper and lower legs (thigh/shank), defined as the perpendicular
        // distances
        //   between the hip and knee, and knee and ankle pitch axes respectively, are equal. This common length is
        //   denoted L and is given by the class member cld.linkLength.
        ...
        // TODO: Case where hip origin and ankle origin coincide? Case where y=z=0 already problematic enough? Multiple
        // solutions!
        ...
        double alpha = std::acos(utility::math::clamp(0.0, legLength / nominalLegLength, 1.0));
        kneePitch    = 2.0 * alpha;
        ...
        Eigen::Vector3d normal =
            (footRotation * Eigen::AngleAxisd(-ankleRoll, Eigen::Vector3d::UnitX()))
            * Eigen::Vector3d::UnitY();  // y-axis of the {A} frame (ankle roll removed) in body-fixed coordinates
        ...
        hipInAnkleFrame = (Eigen::AngleAxisd(-hipYaw, Eigen::Vector3d::UnitZ()) * footRotation
                           * Eigen::AngleAxisd(-ankleRoll, Eigen::Vector3d::UnitX())
                           * Eigen::AngleAxisd(-legAxisPitch, Eigen::Vector3d::UnitY()))
                          * Eigen::Vector3d::UnitZ();

        // Calculate the hip roll and pitch
        hipRoll  = std::atan2(-hipInAnkleFrame.y(), hipInAnkleFrame.z());
        hipPitch = std::asin(utility::math::clamp(-1.0, hipInAnkleFrame.x() / hipInAnkleFrame.norm(), 1.0)) - alpha;
    }
