...
protected:
    ...
    const std::string& gaitName() const {
        return m_gaitName;
    }  //!< Retrieve the name of the gait (e.g. if the parameter string is `cpg_gait::CPGGait` then the gait name is
       //!< everything past the first double colon, i.e. `CPGGait`)
...
private:
    ...
    boost::shared_ptr<config_server::Parameter<bool>> m_enableGait;  // Flag whether to enable this gait motion module.
                                                                     // The config parameter name is specific to the
                                                                     // gait engine name.
    ...
    config_server::Parameter<bool>
        m_publishTransforms;  // Flag whether to publish the ego_floor and gait odometry TF transforms.
    config_server::Parameter<float>
        m_gcvNormP;  // The p parameter with which to calculate the gait command vector norm (i.e. using the p-norm).
    config_server::Parameter<float> m_gcvNormMax;   // The maximum allowed gait command vector norm, beyond which
                                                    // normalisation to this maximum value is performed.
    config_server::Parameter<float> m_gcvZeroTime;  // The minimum time after which the robot has just started walking
                                                    // where a zero gcv is enforced for stability reasons.
    config_server::Parameter<float> m_minWalkingTime;  // The minimum time after which a robot is allowed to be told to
                                                       // stop walking again after it just started walking
    ...
    boost::shared_ptr<const urdf::Link>
        m_rightFootLink;  // Right foot URDF link (used for setting support coefficients)
    ...
    // Motions
    void setPendingMotion(MotionID ID,
                          MotionStance stance = STANCE_DEFAULT,
                          bool adjustLeft     = true,
                          bool adjustRight    = true);
    ...
    bool m_motionAdjustLeftFoot;   // Flag whether the left foot should be adjusted to attain the required foot
                                   // separation (how the required foot separation for various motions is defined is up
                                   // to the gait engine)
    bool m_motionAdjustRightFoot;  // Flag whether the right foot should be adjusted to attain the required foot
                                   // separation (how the required foot separation for various motions is defined is up
                                   // to the gait engine)
    ...
    // Gait state
    enum GaitState { GS_INACTIVE, GS_REACHING_HALT_POSE, GS_STARTING_WALKING, GS_WALKING, GS_STOPPING_WALKING };
