...
namespace contrib {
    ...
    void LimpModel::setState(LimpState ms, Eigen::Vector3f ggcv) {
        ...
        // The best time T to step is when the lateral pendulum is going to reach the desired support exchange location
        // (sex). In the best case we are on a good kind of returning trajectory and the sex will be reached eventually.
        double tts = timeToSel();
        double tta = timeToApex();

        // On a crossing trajectory, the step time is infinity. We just wait and hope that the robot will return after
        // all. In this case we also return and don't modify the previously computed zmp and footstep coordinates. Nice
        // hack dude.
        if (crossing) {
            timeToStep = 2.0;
            return;
        ...
        // can produce a nervous stepping behavior. It worked better to just decay the timeToStep and take an unhurried
        // step and then let the balance controller fix the problem on the other leg.
        ...
        // 3. Sagittal ZMP.
        // The gait control input defines a nominal capture point along with the nominal state. The sagittal zmp is
        // calculated using Englsberger's capture point based ZMP controller from the paper "Bipedal Walking Control
        // Based on Capture Point Dynamics". The controller computes a ZMP based on the capture point of the current CoM
        // state and the desired capture point. The computed ZMP will theoretically move the CoM such that its capture
        ...
        // We forward the model by "timeToStep" time to the expected end of step state c' using the zmp that we just
        // calculated.
        endOfStepState = forwarded(timeToStep).getMotionState();


        // 4. Lateral step size Y.
        // The lateral step size is the next pendulum pivot point relative to the future CoM location at support
        // exchange. The step size is chosen such that the CoM will pass the next apex at distance alpha.
        ...
        // used to reach that velocity? That's going to be our step size. To reproduce the equations: forward the
        // current CoM state by the step time t, calculate the velocity back at the apex using the tanh formula, forward
        // the apex state by the half step time tau and there is your step size.
    ...
    bool LimpModel::forwardThroughStep(double dt) {
        bool stepped = false;
        if (dt >= timeToStep) {
            double tts = std::max(timeToStep, 0.0);
            forward(tts);
            step();
            stepped = true;
            forward(dt - tts);
    ...
    void LimpModel::step() {
        ...
        ls.x         = postStepCorrectionFactor * ls.x
               + (1.0 - postStepCorrectionFactor) * -footStep.x();  // Danger! This explodes without adaptation.
        ls.y              = -footStep.y();
        ls.ax             = C * ls.x;
        ls.ay             = C * ls.y;
        ...
        if (timeToStep <= 0) timeToStep = nominalTimeToStep;
    }
