...
void RobotModel::update(const Pose& pose, double fusedX, double fusedY) {
    ...
    alignModel(
        fusedX,
        fusedY);  // Updates the base transform based on the current support foot location and the measured fused angles
    ...
    if (supportExchangeLock && (fabs(footHeightDiff) >= config->footHeightHysteresis())) supportExchangeLock = false;
...
void RobotModel::applyPose() {
    ...
    // Head
    neck.setRotation(Quaternion(Vec(0, 0, 1), pose.headPose.neck.z) * Quaternion(Vec(0, 1, 0), pose.headPose.neck.y));
    ...
    // Left leg
    lHip.setRotation(Quaternion(Vec(0, 0, 1), pose.leftLegPose.hip.z) * Quaternion(Vec(1, 0, 0), pose.leftLegPose.hip.x)
...
// Align the model to the given fused roll/pitch, add yaw to ensure the fused yaw of the support foot hasn't changed,
// and translate the model to ensure that the support foot floor point is preserved
...
Quaternion RobotModel::quatFromFusedPR(double fusedX,
