...
protected:
    ...
    double haltJointCmd[NUM_JOINTS];     //!< Commanded halt position for each joint (indexed by the `JointID` enum, in
                                         //!< `rad`).
    double haltJointEffort[NUM_JOINTS];  //!< Commanded halt joint effort (indexed by the `JointID` enum, in the range
                                         //!< `[0,1]`).
    bool haltUseRawJointCmds;  //!< Apply the joint commands directly to the hardware in the halt pose, without using
                               //!< compensation or actuator controller(s) in-between.
...
private:
    ...
    void setGaitOwner(const Gait* gaitPtr) {
        m_gait = gaitPtr;
    }
