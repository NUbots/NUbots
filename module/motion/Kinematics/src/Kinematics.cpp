...
namespace module::motion {
    ...
    Kinematics::Kinematics(std::unique_ptr<NUClear::Environment> environment)
        ...
        on<Provide<LeftLegIK>, Needs<LeftLeg>>().then(
            [this](const RunInfo& info, const LeftLegIK& leg_ik, const Uses<LeftLeg>& leg) {
        ...
        on<Provide<RightLegIK>, Needs<RightLeg>>().then(
            [this](const RunInfo& info, const RightLegIK& leg_ik, const Uses<RightLeg>& leg) {
                // If the leg is done moving, then IK is done
                if (info.run_reason == RunInfo::RunReason::SUBTASK_DONE) {
                    if (leg.done) {
                        emit<Task>(std::make_unique<Done>());
        ...
        on<Provide<HeadIK>, Needs<Head>>().then(
            [this](const RunInfo& info, const HeadIK& head_ik, const Uses<Head>& head) {
