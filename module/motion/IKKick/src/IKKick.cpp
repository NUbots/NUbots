...
IKKick::IKKick(std::unique_ptr<NUClear::Environment> environment)
  ...
  on<Configuration>("IKKick.yaml").then([this](const Configuration& config) {
    ...
    emit(std::make_unique<IKKickParams>(
        IKKickParams(config["balancer"]["stand_height"].as<float>())));
  });

  on<Startup>().then("IKKick Startup", [this] {
    // Default kick plan at enemy goals
    emit(std::make_unique<KickPlan>(
        KickPlan(Eigen::Vector2d(4.5, 0), KickPlan::KickType::IK_KICK)));
  });
  ...
  on<Trigger<ExecuteKick>, With<KickCommand, Sensors, KinematicsModel>>().then(
      [this](const KickCommand& command, const Sensors& sensors,
        ...
        // 4x4 homogeneous transform matrices for left foot and right foot
        // relative to torso
        Transform3D leftFoot = convert<double, 4, 4>(
            sensors.forwardKinematics.at(ServoID::L_ANKLE_ROLL));
        Transform3D rightFoot = convert<double, 4, 4>(
        ...
        Transform3D torsoPose =
            (supportFoot == LimbID::LEFT_LEG) ? leftFoot.i() : rightFoot.i();

        // Put the ball position from vision into torso coordinates
        arma::vec3
            targetTorso;  // = Transform3D(convert<double, 4,
        ...
        arma::vec3 directionSupportFoot =
            torsoPose.transformVector(directionTorso);

        arma::vec3 ballPosition = targetSupportFoot;
        ballPosition[2] =
            0.05;  // TODO: figure out why ball height is unreliable
        arma::vec3 goalPosition = directionSupportFoot;
        goalPosition[2] = 0.0;  // TODO: figure out why ball height is
                                // unreliable
  ...
  updater =
      on<Every<UPDATE_FREQUENCY, Per<std::chrono::seconds>>,
         With<Sensors, KinematicsModel>, Single>()
          .then([this](const Sensors& sensors,
            ...
            if (balancer.isRunning()) {
              ...
              kickFootGoal = supportFootPose.translate(
                  arma::vec3({0, negativeIfKickRight * foot_separation, 0}));
            }
            ...
            if (feedback_active) {
              feedbackBalancer.balance(kinematicsModel, supportFootGoal,
                                       supportFoot, sensors);
            }
            ...
            // IK
            auto kickJoints =
                calculateLegJoints(kinematicsModel, kickFootGoal, kickFoot);
            auto supportJoints = calculateLegJoints(
                kinematicsModel, supportFootGoal, supportFoot);

            // Combine left and right legs
            joints.insert(joints.end(), kickJoints.begin(), kickJoints.end());
            joints.insert(joints.end(), supportJoints.begin(),
                          supportJoints.end());
            ...
            for (auto& joint : joints) {
              waypoints->push_back(ServoCommand(subsumptionId, time,
  ...
  emit<Scope::INITIALIZE>(std::make_unique<RegisterAction>(RegisterAction{
      ...
      {std::pair<float, std::set<LimbID>>(
          0, {LimbID::LEFT_LEG, LimbID::RIGHT_LEG, LimbID::LEFT_ARM,
              LimbID::RIGHT_ARM})},
...
void IKKick::updatePriority(const float& priority) {
  emit(std::make_unique<ActionPriorites>(
      ActionPriorites{subsumptionId, {priority}}));
}
