...
namespace module::purpose {
    ...
    TrainTest::TrainTest(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
        ...
        on<Trigger<State>>().then([this](const State& state) {
            ...
            for (uint e = 1; e <= n_epochs; e++) {
                ...
                for (uint i = 0; i < n_iter; i++) {
                    ...
                    out << e << ", " << env.pos_(0) << ", " << env.pos_(1) << ", " << env.goal_(0) << ", "
                        << env.goal_(1) << ", " << std::get<1>(sd) << "\n";
