...
function(IndexArduinoBoards namespace)
  ...
  if(NOT ARDUINO_BOARD_OPTIONS_FILE
     AND NOT _LAST_USED_ARDUINO_BOARD_OPTIONS_FILE
     AND EXISTS "${CMAKE_BINARY_DIR}/BoardOptions.cmake"
  )
  ...
  # there is a change.
  _board_configure_file(
    WRITE "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_FileHeader.cmake.in" _board_options_part1
  )
  ...
  foreach(pl_id IN LISTS _pl_list)
    ...
    if(EXISTS "${pl_path}/programmers.txt")
      properties_read("${pl_path}/programmers.txt" "ard_prog.${boards_idx}")
    ...
    _board_configure_file(
      APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_BoardHeader.cmake.in" _board_options_part1
    )
    ...
    foreach(_build_board IN LISTS _build_board_list)
      ...
      properties_get_value("ard_boards.${boards_idx}" "${_board_prefix}.hide" _board_hide QUIET DEFAULT "FALSE")
      if(_board_hide)
        continue()
      endif()
      list(APPEND _board_identifier_list "${_board_identifier}")
      set("${namespace}.${_board_identifier}/prop_namespace" "ard_boards.${boards_idx}")
      set("${namespace}.${_board_identifier}/prop_namespace"
          "ard_boards.${boards_idx}"
          PARENT_SCOPE
      )
      set("${namespace}.${_board_identifier}/pl_id" "${pl_id}")
      set("${namespace}.${_board_identifier}/pl_id"
          "${pl_id}"
          PARENT_SCOPE
      )
      ...
      if("${ARDUINO_BOARD}" STREQUAL "${_board_name_in_menu}" OR "${ARDUINO_BOARD}" STREQUAL "${_board_identifier}")
      ...
      _board_configure_file(
        APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_BoardSel.cmake.in" _board_options_part1
      )

      # Populate the menu for the options of the board
      properties_get_list("ard_boards.${boards_idx}" "^${_board_prefix}\\.menu\\.[^.]+\\.[^.]+$" _menu_opt_prop_list)
      ...
      foreach(_menu_opt_prop IN LISTS _menu_opt_prop_list)
        ...
        string(REGEX MATCH "^${_board_prefix}\\.menu\\.([^.]+)\\.([^.]+)$" match "${_menu_opt_prop}")
        set(_menu_prefix "${CMAKE_MATCH_1}")
        set(_menu_opt_prefix "${CMAKE_MATCH_2}")
        properties_get_value("ard_boards.${boards_idx}" "menu.${_menu_prefix}" _menu_name QUIET)
        if(NOT _menu_name)
          set(_menu_name "${_menu_prefix}")
        endif()
        properties_get_value(
          "ard_boards.${boards_idx}" "${_board_prefix}.menu.${_menu_prefix}.${_menu_opt_prefix}" _menu_opt_name
        )
        ...
        string(MAKE_C_IDENTIFIER "${_board_identifier}.menu.${_menu_prefix}.${_menu_opt_prefix}" _menu_opt_identifier)
        ...
        set_property(
          CACHE "${_menu_var_name}"
          APPEND
          PROPERTY STRINGS "${_menu_opt_name}"
        )
      ...
      _board_configure_file(
        APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_MenuBoardHdr.cmake.in" _board_options_part2
      )
      ...
      foreach(_menu_identifier IN LISTS _menu_identifier_list)
        ...
        _board_configure_file(
          APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_MenuHeader.cmake.in" _board_options_part2
        )
        foreach(_menu_opt_identifier IN LISTS ARDUINO.${_menu_identifier}.OPTIONS)
          ...
          _board_configure_file(
            APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_Menuoption.cmake.in" _board_options_part2
          )
        endforeach()
      endforeach()
    ...
    _board_configure_file(
      APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_ProgHeader.cmake.in" _board_options_part3
    )
    ...
    foreach(_prog IN LISTS _prog_list)
      ...
      set("${namespace}/prog.${_prog_id}.prop_namespace" "ard_prog.${boards_idx}")
      set("${namespace}/prog.${_prog_id}.prop_namespace"
          "ard_prog.${boards_idx}"
          PARENT_SCOPE
      )
      properties_get_value("ard_prog.${boards_idx}" "${_prog_prefix}.name" _prog_name)
      ...
      if("${ARDUINO_PROGRAMMER}" STREQUAL "${_prog_name_in_menu}" OR "${ARDUINO_PROGRAMMER}" STREQUAL "${_prog_id}")
      ...
      _board_configure_file(
        APPEND "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/BoardOptions_ProgSel.cmake.in" _board_options_part3
      )
  ...
  set(_new_board_options_content "${_board_options_part1}${_board_options_part2}${_board_options_part3}")
  if(NOT _new_board_options_content STREQUAL _old_board_options_content)
    if(EXISTS "${CMAKE_BINARY_DIR}/BoardOptions.cmake")
      file(REMOVE "${CMAKE_BINARY_DIR}/BoardOptions.cmake.bak")
      file(RENAME "${CMAKE_BINARY_DIR}/BoardOptions.cmake" "${CMAKE_BINARY_DIR}/BoardOptions.cmake.bak")
    endif()
  ...
  set(ARDUINO_BOARD
      "${ARDUINO_BOARD}"
      CACHE STRING "Arduino board for which the project is build for"
  )
  ...
  # Arduino programmers
  set(ARDUINO_PROGRAMMER
      "${ARDUINO_PROGRAMMER}"
      CACHE STRING "Arduino programmer used for uploading using programmer and for burning bootloader"
  )
  ...
  foreach(_menu_identifier IN LISTS _sel_board_menu_identifier_list)
    ...
    set(_menu_opt_name "${ARDUINO.${_menu_opt_identifier}.NAME}")
    set(ARDUINO_${_menu_opt_identifier}
        TRUE
        PARENT_SCOPE
    )
  ...
  set(_LAST_USED_ARDUINO_BOARD_OPTIONS_FILE_TS
      "${_ts}"
      CACHE INTERNAL "" FORCE
  )
...
function(boards_get_property namespace board_identifier prop_name return_value)
  ...
  if("${first_letter}" STREQUAL "/")
    ...
    set("${return_value}"
        "${${namespace}.${board_identifier}${prop_name}}"
        PARENT_SCOPE
    )
  else()
    ...
    properties_get_value("${prop_namespace}" "${_board_prefix}.${prop_name}" _value ${ARGN})
    set("${return_value}"
        "${_value}"
        PARENT_SCOPE
    )
...
# <return_value> [OUT]: The value of the property is returned in this variable
#
function(boards_get_ref_platform_property namespace board_identifier ref_pkg_name prop_name return_value)
  ...
  if(NOT _ref_pl_id)
    if(NOT parsed_args_QUIET AND "${parsed_args_DEFAULT}" STREQUAL "")
      message(FATAL_ERROR "Referenced platform ${ref_pkg_name} not found in ${namespace}!!!")
    endif()
    set("${return_value}"
        "${parsed_args_DEFAULT}"
        PARENT_SCOPE
  ...
  platforms_get_property("ard_plat" "${_ref_pl_id}" "${prop_name}" _value ${ARGN})
  set("${return_value}"
      "${_value}"
      PARENT_SCOPE
  )
...
function(programmer_get_property namespace prog_id prop_name return_value)
  ...
  properties_get_value("${prop_namespace}" "${_prog_prefix}.${prop_name}" _value ${ARGN})
  set("${return_value}"
      "${_value}"
      PARENT_SCOPE
  )
...
function(programmer_get_property_list namespace prog_id pattern return_list)
  ...
  string(REGEX MATCH "^([^.]+)\\.([^.]+)$" match "${prog_id}")
  set(_prog_prefix "${CMAKE_MATCH_2}")
  properties_get_list("${prop_namespace}" "^${_prog_prefix}\\.${pattern}" _list)
  foreach(_elem IN LISTS _list)
...
function(check_board_options_changed _ret_var)
  ...
  if(ARDUINO_BOARD_OPTIONS_FILE)
    check_same_file("${ARDUINO_BOARD_OPTIONS_FILE}" "${_LAST_USED_ARDUINO_BOARD_OPTIONS_FILE}" _same_file)
    if(NOT _same_file)
      set("${_ret_var}"
          TRUE
          PARENT_SCOPE
