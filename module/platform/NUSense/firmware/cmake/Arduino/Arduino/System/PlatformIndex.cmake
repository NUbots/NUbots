...
function(platforms_get_property namespace pl_id prop_name return_value)
  ...
  if("${first_letter}" STREQUAL "/")
    ...
    json_get_value("${json_namespace}" "${json_prefix}.${prop_name}" _value ${ARGN})
    set("${return_value}"
        "${_value}"
        PARENT_SCOPE
    )
...
macro(platforms_set_parent_scope namespace)
  ...
  foreach(pl_id IN LISTS "${namespace}/list")
    set("${namespace}.${pl_id}/json_namespace"
        "${${namespace}.${pl_id}/json_namespace}"
        PARENT_SCOPE
    )
    set("${namespace}.${pl_id}/json_prefix"
        "${${namespace}.${pl_id}/json_prefix}"
        PARENT_SCOPE
    )
    set("${namespace}.${pl_id}/path"
        "${${namespace}.${pl_id}/path}"
        PARENT_SCOPE
    )
    set("${namespace}.${pl_id}/local_path"
        "${${namespace}.${pl_id}/local_path}"
        PARENT_SCOPE
    )
    set("${namespace}.${pl_id}/hw_path"
        "${${namespace}.${pl_id}/hw_path}"
        PARENT_SCOPE
    )
    set("${namespace}.${pl_id}/tool_path"
        "${${namespace}.${pl_id}/tool_path}"
        PARENT_SCOPE
    )
  endforeach()
  set("${namespace}/json_count"
...
macro(_platforms_find_installed json_namespace pl_namespace is_bundled)
  ...
  foreach(pkg_idx RANGE 1 "${num_packages}")
    ...
    foreach(pl_idx RANGE 1 ${num_platforms})
      ...
      set("${pl_namespace}.${pl_id}/json_namespace"
          "${json_namespace}"
          PARENT_SCOPE
      )
      set("${pl_namespace}.${pl_id}/json_prefix"
          "${pl}"
          PARENT_SCOPE
      )
      set("${pl_namespace}.${pl_id}/json_pkg"
          "${pkg_name}"
          PARENT_SCOPE
      )
      set("${pl_namespace}.${pl_id}/path"
          "${pl_path}"
          PARENT_SCOPE
      )
      set("${pl_namespace}.${pl_id}/local_path"
          "${local_path}"
          PARENT_SCOPE
      )
      set("${pl_namespace}.${pl_id}/hw_path"
          "${hw_path}"
          PARENT_SCOPE
      )
      set("${pl_namespace}.${pl_id}/tool_path"
          "${tool_path}"
          PARENT_SCOPE
      )
