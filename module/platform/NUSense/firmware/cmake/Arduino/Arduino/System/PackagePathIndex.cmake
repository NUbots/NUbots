...
function(InitializeArduinoPackagePathList)
  ...
  if(${CMAKE_HOST_APPLE})
    ...
    set(install_search_paths "$ENV{HOME}/Applications" "$ENV{HOME}/Library/Arduino15" /Applications
  ...
  elseif(${CMAKE_HOST_UNIX}) # Probably Linux or some unix-like
    ...
    execute_process(
      COMMAND which arduino
      OUTPUT_VARIABLE _bin_path
      ERROR_VARIABLE _ignore
      RESULT_VARIABLE _cmd_result
    )
    if(_cmd_result STREQUAL 0)
      string(STRIP "${_bin_path}" _bin_path)
      execute_process(
        COMMAND readlink -f "${_bin_path}"
      ...
      if(EXISTS "${_app_path}/arduino-arduinoide.desktop")
        file(STRINGS "${_app_path}/arduino-arduinoide.desktop" _exec_prop REGEX "^Exec=")
        if("${_exec_prop}" MATCHES "^Exec=\"?(.*)\"?")
          get_filename_component(_install_path "${CMAKE_MATCH_1}" DIRECTORY)
          list(APPEND install_search_paths "${_install_path}")
        endif()
    ...
    file(
      GLOB
      other_search_paths
      "$ENV{HOME}/.local/share/arduino*"
      /usr/share/arduino*
      /usr/local/share/arduino*
      /opt/local/arduino*
      /opt/arduino*
      "$ENV{HOME}/opt/arduino*"
    )
    list(APPEND install_search_paths "${other_search_paths}")

    file(GLOB package_search_paths "$ENV{HOME}/.arduino15" "${other_search_paths}")
    set(package_path_suffixes "")

    file(GLOB sketchbook_search_paths "$ENV{HOME}/.arduino15" "${other_search_paths}")
    set(sketchbook_path_suffixes "")

  elseif(${CMAKE_HOST_WIN32})

    set(Prog86Path "ProgramFiles(x86)")
    set(install_search_paths "$ENV{${Prog86Path}}/Arduino" "$ENV{ProgramFiles}/Arduino" "$ENV{LOCALAPPDATA}/Arduino15")
    set(install_path_suffixes "")
    ...
    get_filename_component(_reg_documents "[${_reg_explorer}\\User Shell Folders;Personal]" ABSOLUTE)

    file(GLOB sketchbook_search_paths "$ENV{LOCALAPPDATA}/Arduino15" "${_reg_documents}/ArduinoData")
    set(sketchbook_path_suffixes "")
  else()
  ...
  if(NOT ARDUINO_INSTALL_PATH
     AND NOT "${ARDUINO_ENABLE_PACKAGE_MANAGER}"
     AND "${ARDUINO_BOARD_MANAGER_URL}" STREQUAL ""
  )
    message(
      FATAL_ERROR
        "Arduino IDE installation is not found!!!\n"
  ...
  if(NOT ARDUINO_PKG_MGR_DL_CACHE)
    set(ARDUINO_PKG_MGR_DL_CACHE "${ARDUINO_PACKAGE_MANAGER_PATH}/downloads")
  endif()
  set(ARDUINO_PKG_MGR_DL_CACHE
      "${ARDUINO_PKG_MGR_DL_CACHE}"
