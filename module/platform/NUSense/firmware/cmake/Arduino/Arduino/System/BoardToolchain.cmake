...
function(SetupBoardToolchain)
  ...
  if(_core_pkg_name)
    ...
    _board_get_ref_platform_property("${_core_pkg_name}" "/path" ARDUINO_CORE_SPECIFIC_PLATFORM_PATH)
  ...
  properties_set_value("ard_global" "runtime.platform.path" "${ARDUINO_BOARD_RUNTIME_PLATFORM_PATH}")
  ...
  properties_set_value("ard_global" "build.core.path" "${ARDUINO_BOARD_BUILD_CORE_PATH}")
  ...
  if(_prop_value)
    ...
    if(_variant_pkg_name)
      _board_get_ref_platform_property("${_variant_pkg_name}" "/path" _variant_pl_path)
    else()
      set(_variant_pl_path "${ARDUINO_BOARD_RUNTIME_PLATFORM_PATH}")
    endif()
    set(ARDUINO_BOARD_BUILD_VARIANT_PATH "${_variant_pl_path}/variants/${_build_variant}")
  else()
    set(ARDUINO_BOARD_BUILD_VARIANT_PATH)
  endif()

  properties_set_value("ard_global" "build.variant.path" "${ARDUINO_BOARD_BUILD_VARIANT_PATH}")
  string_escape_quoting(ARDUINO_BOARD_BUILD_VARIANT_PATH)

  # Set build.system.path
  properties_set_value("ard_global" "build.system.path" "${ARDUINO_BOARD_RUNTIME_PLATFORM_PATH}/system")
  ...
  set(_tool_recipe_regex_list "upload\\.pattern|upload\\.network_pattern" "program\\.pattern|erase\\.pattern"
                              "bootloader\\.pattern"
  )
  ...
  foreach(_tool_prop_idx RANGE 0 ${_tool_last_idx})
    ...
    if(_ref_tool_pkg_name AND NOT "${_ref_tool_pkg_name}" STREQUAL "${_core_pkg_name}")
      list(APPEND _ref_tool_pkgs "${_ref_tool_pkg_name}")
      if(NOT _ref_tool_pkg_name STREQUAL last_tool_pkg_name) # Optimization
        _load_platform_properties(ard_tool_local "${_ref_tool_pkg_name}" RESET)
        properties_resolve_all_values(ard_tool_local)
      endif()
      set(last_tool_pkg_name "${_ref_tool_pkg_name}")
      properties_get_list(ard_tool_local "^tool\\.${_tool_name}\\.(${_tool_recipe_regex})$" _pattern_name_list)
  ...
  foreach(_ref_pkg_name IN LISTS _ref_tool_pkgs)
    _board_get_ref_platform_property("${_ref_pkg_name}" "/tool_path" _tool_path)
    _board_get_ref_platform_property("${_ref_pkg_name}" "toolsDependencies.N" _num_tools)
    if(${_num_tools} GREATER 0)
      foreach(_tool_idx RANGE 1 "${_num_tools}")
        _board_get_ref_platform_property("${_ref_pkg_name}" "toolsDependencies.${_tool_idx}.name" _tool_name)
        _board_get_ref_platform_property("${_ref_pkg_name}" "toolsDependencies.${_tool_idx}.version" _tool_version)
        _board_get_ref_platform_property("${_ref_pkg_name}" "toolsDependencies.${_tool_idx}.packager" _tool_packager)
        string(REPLACE "{tool_name}" "${_tool_name}" _prop_value "${_tool_path}")
        string(REPLACE "{tool_version}" "${_tool_version}" _prop_value "${_prop_value}")
        string(REPLACE "{tl_packager}" "${_tool_packager}" _prop_value "${_prop_value}")
        properties_set_value("ard_global" "runtime.tools.${_tool_name}.path" "${_prop_value}")
        properties_set_value("ard_global" "runtime.tools.${_tool_name}-${_tool_version}.path" "${_prop_value}")
  ...
  foreach(_option IN LISTS _board_options)
    ...
    if(match)
      ...
      string(MAKE_C_IDENTIFIER "${ARDUINO_BOARD_IDENTIFIER}.menu.${_menu_prefix}.${_menu_opt_prefix}" OPT_VARIABLE)
      ...
      if("${ARDUINO_${OPT_VARIABLE}}" AND NOT "${_menu_opt_action}" STREQUAL "")
        ...
        properties_set_value("ard_global" "${_menu_opt_action}" "${_prop_value}")
      endif()
    else()
  ...
  if(ARDUINO_PROGRAMMER_ID)
    programmer_get_property_list(ard_brd "${ARDUINO_PROGRAMMER_ID}" "" _prog_options)
    # message("_prog_options:${_prog_options}")
    foreach(_option IN LISTS _prog_options)
      programmer_get_property(ard_brd "${ARDUINO_PROGRAMMER_ID}" "${_option}" _prop_value)
  ...
  foreach(
    _pattern_name_regex
    "recipe\\.hooks\\.sketch\\.prebuild\\.[0-9]+\\.pattern" "recipe\\.hooks\\.libraries\\.prebuild\\.[0-9]+\\.pattern"
    ...
    foreach(_pattern_name IN LISTS _pattern_name_list)
      ...
      string(REGEX MATCHALL "{build\\.path}/[-a-zA-Z0-9._/]+" _pattern_gen_file_list "${_pattern}")
  ...
  string(REPLACE "{build.path}" "{cmake_binary_dir}" _gen_replace_list "${_gen_file_list}")
  string(REPLACE "{build.project_name}" "{cmake_project_name}" _gen_replace_list "${_gen_replace_list}")
  ...
  foreach(_pattern_name_regex "recipe\\..*pattern" "tools\\..*pattern")
    ...
    foreach(_pattern_name IN LISTS _pattern_name_list)
      ...
      foreach(_gen_file IN LISTS _gen_file_list)
        ...
        math(EXPR idx "${idx} + 1")
        string(REPLACE "${_gen_file}" "${_replace_file}" _replace_pattern "${_replace_pattern}")
      endforeach()
      # message("${_pattern_name}:${_pattern}:${_replace_pattern}")
      properties_set_value(ard_global "${_pattern_name}" "${_replace_pattern}")
    endforeach()
  endforeach()

  # before resolving the values, overwrite the host specific properties
  properties_get_list(ard_global ".*\\.${ARDUINO_BOARD_HOST_NAME}$" _host_prop_list)
  foreach(_host_prop IN LISTS _host_prop_list)
    properties_get_value(ard_global "${_host_prop}" _prop_value)
    string(REGEX REPLACE "\\.${ARDUINO_BOARD_HOST_NAME}$" "" _prop "${_host_prop}")
  ...
  # CMAKE_CXX_COMPILER
  _resolve_build_rule_properties("recipe.cpp.o.pattern" _build_cmd _build_string)
  ...
  # CMAKE_ASM_COMPILER
  _resolve_build_rule_properties("recipe.S.o.pattern" _build_cmd _build_string)
  ...
  _resolve_build_rule_properties("recipe.c.combine.pattern" _build_cmd _build_string)
  ...
  _resolve_build_rule_properties("recipe.ar.pattern" _build_cmd _build_string)
  ...
  configure_file(
    "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/ArduinoSystem.cmake.in" "${CMAKE_BINARY_DIR}/ArduinoSystem.cmake" @ONLY
  )
...
function(_board_get_property prop return_value)
  boards_get_property(ard_brd "${ARDUINO_BOARD_IDENTIFIER}" ${prop} _return_value ${ARGN})
  set("${return_value}"
      "${_return_value}"
      PARENT_SCOPE
  )
...
function(_board_get_platform_property prop return_value)
  boards_get_platform_property(ard_brd "${ARDUINO_BOARD_IDENTIFIER}" ${prop} _return_value ${ARGN})
  set("${return_value}"
      "${_return_value}"
      PARENT_SCOPE
  )
...
function(_board_get_ref_platform_property ref_pkg_name prop return_value)
  boards_get_ref_platform_property(
    ard_brd "${ARDUINO_BOARD_IDENTIFIER}" "${ref_pkg_name}" ${prop} _return_value ${ARGN}
  )
  set("${return_value}"
      "${_return_value}"
      PARENT_SCOPE
...
function(_resolve_tool_properties tool tool_string return_string)
  ...
  properties_get_list(ard_local "^[^.]+\\.params\\.${_suffix}$" _action_property_list)
  foreach(_action_property IN LISTS _action_property_list)
    string(REGEX MATCH "^([^.]+)\\.params\\.${_suffix}$" match "${_action_property}")
...
function(_resolve_command_patterns)
  ...
  set(_rule_name_regex_list "^recipe\\..*\\.pattern$" "^tools\\..*\\.network_pattern$" "^tools\\..*\\.pattern$")
  set(ARDUINO_RULE_NAMES_LIST)
  foreach(_rule_name_regex IN LISTS _rule_name_regex_list)
    properties_get_list(ard_global "${_rule_name_regex}" _rule_name_list)
    ...
    foreach(_rule_name IN LISTS _rule_name_list)
      ...
      properties_get_value(ard_global "${_rule_name}" _rule_string)
      if(NOT _rule_string)
        continue()
      endif()

      if("${_rule_name}" MATCHES "^tools\\.([^.]+)")
        _resolve_tool_properties("${CMAKE_MATCH_1}" "${_rule_string}" _rule_string)
      endif()

      set("ARDUINO_RULE_${_rule_name}"
          "${_rule_string}"
          PARENT_SCOPE
      )
    endforeach()
...
function(_resolve_build_rule_properties rule return_cmd return_string)
  ...
  string(REPLACE "{build.source.path}" "<TODO_SOURCE_DIR>" _rule_string "${_rule_string}")
  string(REPLACE "{cmake_binary_dir}" "${CMAKE_BINARY_DIR}" _rule_string "${_rule_string}")
  string(REPLACE "{cmake_project_name}" "${CMAKE_PROJECT_NAME}" _rule_string "${_rule_string}")
  if(CMAKE_VERSION VERSION_LESS 3.4.0)
    string(REPLACE "{includes}" "<DEFINES> <FLAGS>" _rule_string "${_rule_string}")
  else()
    string(REPLACE "{includes}" "<DEFINES> <INCLUDES> <FLAGS>" _rule_string "${_rule_string}")
  endif()
  string(REGEX REPLACE "\"?{source_file}\"?" "<SOURCE>" _rule_string "${_rule_string}")
  if("${rule}" STREQUAL "recipe.ar.pattern")
    string(REGEX REPLACE "\"?{object_file}\"?" "<LINK_FLAGS> <OBJECTS>" _rule_string "${_rule_string}")
    string(REGEX REPLACE "\"?{archive_file_path}\"?" "<TARGET>" _rule_string "${_rule_string}")
    string(REGEX REPLACE "\"?{build.path}/{archive_file}\"?" "<TARGET>" _rule_string "${_rule_string}")
  else()
    string(REGEX REPLACE "\"?{build.path}/{build.project_name}\\.elf\"?" "<TARGET>" _rule_string "${_rule_string}")
    string(REGEX REPLACE "\"?{build.path}/{archive_file}\"" "" _rule_string "${_rule_string}")
    string(REGEX REPLACE "(\"?{archive_file_path}\"?|\"?{archive_file}\"?)" "" _rule_string "${_rule_string}")
    string(REGEX REPLACE "\"?{object_file}\"?" "<OBJECT>" _rule_string "${_rule_string}")
    string(REPLACE "{object_files}" "<OBJECTS> <LINK_LIBRARIES>" _rule_string "${_rule_string}")
  endif()

  # message("_rule_cmd:${_rule_cmd}")
  # message("_rule_string:${_rule_string}")

  string(REPLACE "{build.path}" "${CMAKE_BINARY_DIR}" _rule_string "${_rule_string}")
  string(REPLACE "{build.project_name}" "<TARGET_BASE>" _rule_string "${_rule_string}")
...
function(_resolve_target_properties target target_string return_string)
  ...
  string(REPLACE "{build.source.path}" "$<TARGET_PROPERTY:${target},SOURCE_DIR>" _target_string "${_target_string}")
  string(REPLACE "{build.path}" "$<TARGET_PROPERTY:${target},BINARY_DIR>" _target_string "${_target_string}")
  _gen_non_empty_string("$<TARGET_PROPERTY:${target},OUTPUT_NAME>" "$<TARGET_PROPERTY:${target},NAME>" proj_name)
  string(REPLACE "{build.project_name}" "${proj_name}" _target_string "${_target_string}")
  string(REPLACE "{cmake_binary_dir}" "${CMAKE_BINARY_DIR}" _target_string "${_target_string}")
  string(REPLACE "{cmake_project_name}" "${CMAKE_PROJECT_NAME}" _target_string "${_target_string}")
...
function(_gen_non_empty_string str1 str2 ret)
  if(CMAKE_VERSION VERSION_LESS 3.9.6)
    set(${ret}
        "$<$<BOOL:${str1}>:${str1}>$<$<NOT:$<BOOL:${str1}>>:${str2}>"
        PARENT_SCOPE
    )
  else()
    set(${ret}
        "$<IF:$<BOOL:${str1}>,${str1},${str2}>"
        PARENT_SCOPE
    )
  endif()
endfunction()
...
function(_escape_arduino_quote str return_str)
  ...
  while(TRUE)
    ...
    if("${_state}" STREQUAL "\"") # Explicit quoted
      if((_str_chr STREQUAL "\"" OR _str_chr STREQUAL "'") AND (_nxt_chr MATCHES "${_space_chars}" OR _nxt_chr STREQUAL
                                                                                                      "")
      )
        set(_state " ")
        if(_quote_needed)
    ...
    elseif("${_state}" STREQUAL " ") # State after space
      if(_str_chr STREQUAL "\"" OR _str_chr STREQUAL "'")
        set(_state "\"")
      elseif(_str_chr MATCHES "${_space_chars}")
...
function(_find_system_program_path)
  ...
  foreach(_rule_name_regex IN LISTS _rule_name_regex_list)
    ...
    foreach(_rule_name IN LISTS _rule_name_list)
      ...
      string(REGEX REPLACE "^(\"[^\"]+\"|[^ ]+)" "" _match "${_rule_string}")
      set(_rule_cmd "${CMAKE_MATCH_1}")
      string(REGEX REPLACE "^\"?([^\"]+)\"?" "\\1" _rule_cmd "${_rule_cmd}")
...
function(_gen_arduino_size_script)
  ...
  foreach(size_regex_name IN LISTS _size_regex_name_list)
    ...
    if(_size_name STREQUAL "")
      ...
      properties_get_value(ard_global "upload.maximum_${_size_name}_size" _maximum_size DEFAULT 0)
    endif()
  ...
  if(${SIZE_REGEX_COUNT} GREATER 0)
    configure_file(
      "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/FirmwareSizePrint.cmake.in"
...
function(_gen_arduino_upload_script)
  configure_file(
    "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/FirmwareUpload.cmake.in" "${CMAKE_BINARY_DIR}/FirmwareUpload.cmake"
    @ONLY
  )
endfunction()
...
function(_gen_execute_recipe_script)
  configure_file(
    "${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/ExecuteRecipe.cmake.in" "${CMAKE_BINARY_DIR}/ExecuteRecipe.cmake" @ONLY
  )
endfunction()
