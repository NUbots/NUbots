...
define_property(
  ...
  FULL_DOCS "If the target builds an Arduino library, then " "this property holds the name of the library"
...
function(target_link_arduino_libraries target_name)
  ...
  if(_list_DEFAULT)
    set(empty_list)
    _link_ard_lib_list("${target_name}" _list_DEFAULT "" empty_list empty_list)
  endif()
  ...
  foreach(_link_type IN ITEMS PRIVATE PUBLIC INTERFACE)
    if(_list_${_link_type})
      set(empty_list)
      _link_ard_lib_list("${target_name}" _list_${_link_type} "${_link_type}" empty_list empty_list)
    endif()
  endforeach()
  ...
  if(_list_AUTO_present)
    _get_auto_link_libs("${target_name}" _list_AUTO _list_IGNORE _list_OVERRIDE _link_libs)
    # message("_link_libs:${_link_libs}")
    _link_ard_lib_list("${target_name}" _link_libs "" _list_IGNORE _list_OVERRIDE)
  endif()
  ...
  foreach(_link_type IN ITEMS PRIVATE PUBLIC INTERFACE)
    if(_list_AUTO_${_link_type}_present)
      _get_auto_link_libs("${target_name}" _list_AUTO_${_link_type} _list_IGNORE _list_OVERRIDE _link_libs)
      # message("_link_libs:${_link_type}:${_link_libs}")
      _link_ard_lib_list("${target_name}" _link_libs "${_link_type}" _list_IGNORE _list_OVERRIDE)
    endif()
  endforeach()
...
function(target_enable_arduino_upload target)
  ...
  _set_arduino_target_hooks("${target}" "sketch.postbuild;linking.prelink" PRE_LINK) # Yes, as per order
  _set_arduino_target_hooks("${target}" "linking.postlink;objcopy.preobjcopy" POST_BUILD) # Yes, as per order

  # Set image generation as a post build event
  arduino_board_get_target_cmd("${target}" "^recipe\\.objcopy\\..*\\.pattern$" objcopy_list)
  # message("objcopy_list:${objcopy_list}")
  foreach(objcopy IN LISTS objcopy_list)
    # message("${objcopy}:${${objcopy}}")
    string(REGEX MATCH "^recipe\\.objcopy\\.(.*)\\.pattern" match "${objcopy}")
    string(TOUPPER "${CMAKE_MATCH_1}" file_ext)
    separate_arguments(cmd_with_args_list UNIX_COMMAND "${${objcopy}}")
    add_custom_command(
      TARGET "${target}"
      POST_BUILD
      COMMAND ${cmd_with_args_list}
      COMMENT "Generating ${file_ext} image"
      VERBATIM
    )
  endforeach()

  _set_arduino_target_hooks("${target}" "objcopy.postobjcopy" POST_BUILD) # Yes, as per order

  # Post build event for size calculation
  arduino_board_get_target_cmd("${target}" "^recipe\\.size\\.pattern$" size_recipe_list)
  ...
  foreach(size_recipe IN LISTS size_recipe_list)
    ...
    add_custom_command(
      TARGET "${target}"
      POST_BUILD
      COMMAND ${CMAKE_COMMAND} ARGS "-DRECIPE_SIZE_PATTERN=${size_recipe_str}" -P
              "${CMAKE_BINARY_DIR}/FirmwareSizePrint.cmake"
  ...
  if(tool)
    arduino_board_get_target_cmd("${target}" "^tools\\.${tool}\\.upload\\.pattern$" serial_upload_pattern)
    arduino_board_get_target_cmd("${target}" "^tools\\.${tool}\\.upload\\.network_pattern$" network_upload_pattern)

    # Simplify certain long variables in the pattern
    string(REPLACE "{upload.network." "{network." UPLOAD_NETWORK_PATTERN "${UPLOAD_NETWORK_PATTERN}")
    ...
    separate_arguments(serial_upload_pattern_str UNIX_COMMAND "${${serial_upload_pattern}}")
    separate_arguments(network_upload_pattern_str UNIX_COMMAND "${${network_upload_pattern}}")
    add_custom_target(
      "upload-${target}"
      ${CMAKE_COMMAND}
      ARGS
      ...
      ${_network_defs}
      -P
      "${CMAKE_BINARY_DIR}/FirmwareUpload.cmake"
  ...
  if(ARDUINO_PROGRAMMER_ID AND tool)
    arduino_board_get_target_cmd("${target}" "^tools\\.${tool}\\.program\\.pattern$" program_pattern)
    if(program_pattern)
      # message("${program_pattern}:${${program_pattern}}")
      _get_def_env_options("${${program_pattern}}" _program_defs)
      separate_arguments(program_pattern_str UNIX_COMMAND "${${program_pattern}}")
      add_custom_target(
        "program-${target}"
        ${CMAKE_COMMAND}
        ARGS
        ...
        ${_program_defs}
        -P
        "${CMAKE_BINARY_DIR}/ExecuteRecipe.cmake"
  ...
  if(ARDUINO_PROGRAMMER_ID
     AND tool
     AND NOT TARGET erase-flash
  )
    arduino_board_get_target_cmd("" "^tools\\.${tool}\\.erase\\.pattern$" erase_pattern)
    if(erase_pattern)
      # message("${erase_pattern}:${${erase_pattern}}")
      _get_def_env_options("${${erase_pattern}}" _erase_defs)
      separate_arguments(erase_pattern_str UNIX_COMMAND "${${erase_pattern}}")
      add_custom_target(
        "erase-flash"
        ${CMAKE_COMMAND}
        ARGS
  ...
  if(ARDUINO_PROGRAMMER_ID
     AND tool
     AND NOT TARGET burn-bootloader
  )
    arduino_board_get_target_cmd("" "^tools\\.${tool}\\.bootloader\\.pattern$" bootloader_pattern)
    if(bootloader_pattern)
      ...
      separate_arguments(bootloader_pattern_str UNIX_COMMAND "${${bootloader_pattern}}")
      add_custom_target(
        "burn-bootloader"
        ${CMAKE_COMMAND}
        ARGS
...
function(find_arduino_library lib return_lib_path)
  ...
  list(APPEND search_paths ${parsed_args_HINTS} ${parsed_args_PATHS})

  if(NOT ARDUINO_LIB_${lib}_PATH)
    _library_search_process("${lib}" search_paths parsed_args_PATH_SUFFIXES "ARDUINO_LIB_${lib}_PATH")
    if(ARDUINO_LIB_${lib}_PATH OR NOT parsed_args_QUIET)
      set(ARDUINO_LIB_${lib}_PATH
          "${ARDUINO_LIB_${lib}_PATH}"
          CACHE STRING "Path found containing the arduino library ${lib}" FORCE
      )
    endif()
  ...
  if(NOT ARDUINO_LIB_${lib}_PATH)
    if(NOT parsed_args_QUIET)
      message(SEND_ERROR "Arduino library ${lib} could not be found in " "${search_paths}")
    endif()
    set("${return_lib_path}"
        "${lib}-NOTFOUND"
        PARENT_SCOPE
...
set(ARDUINO_LIBRARIES_SEARCH_PATHS_EXTRA
    ""
    CACHE PATH "Paths to search for Arduino libraries in addition to standard paths"
)
...
# or source list
function(_get_auto_link_libs target_name src_list_var ignore_list_var override_list_var ret_list_var)
  ...
  foreach(file IN LISTS _target_sources)
    get_filename_component(_file_path "${file}" ABSOLUTE BASE_DIR "${target_source_dir}")
...
function(_link_ard_lib_list target_name lib_list_var link_type ignore_list_var override_list_var)
  ...
  foreach(_lib IN LISTS ${lib_list_var})
    ...
    elseif("${_lib}" STREQUAL "core")
      ...
      _add_internal_arduino_library("_arduino_lib_${_lib}" "${_lib}")
      if(NOT TARGET "_arduino_lib_${_lib}")
        return()
      endif()
      target_link_arduino_libraries(
        "_arduino_lib_${_lib}" AUTO_PUBLIC IGNORE ${${ignore_list_var}} OVERRIDE ${${override_list_var}}
      )
      set(_link_target "_arduino_lib_${_lib}")
    endif()
  ...
  if(_link_targets)
    target_link_libraries("${target_name}" ${link_type} ${_link_targets})
  endif()
...
function(_add_internal_arduino_library target lib)
  ...
  if(NOT lib_sources)
    ...
    configure_file(
      ${ARDUINO_TOOLCHAIN_DIR}/Arduino/Templates/DummySource.cpp.in ${CMAKE_CURRENT_BINARY_DIR}/${target}_dummy.cpp
    )
...
function(_add_internal_arduino_core target)
  ...
  # Add the library and set the include directories
  add_library("${target}" STATIC ${core_headers} ${core_sources} ${variant_headers} ${variant_sources})
  # target_include_directories(${target} PUBLIC ${include_dirs})
  target_include_directories(${target} PUBLIC "${ARDUINO_BOARD_BUILD_CORE_PATH}" "${ARDUINO_BOARD_BUILD_VARIANT_PATH}")
...
function(_find_linked_arduino_libs target_name ret_list_var)
  ...
  get_target_property(_intf_link_libs "${target_name}" INTERFACE_LINK_LIBRARIES)
  if(_intf_link_libs)
  ...
  foreach(_link_lib IN LISTS _link_libs)
    ...
    if(_ard_lib_name)
      if(NOT "${_link_lib}" MATCHES "^_arduino_lib_" AND NOT "${_link_lib}" STREQUAL "core")
...
function(_library_search_process lib search_paths_var search_suffixes_var return_var)
  ...
  foreach(path IN LISTS "${search_paths_var}")
    ...
    foreach(
      suffix IN
      LISTS "${search_suffixes_var}"
      ITEMS "libraries" "dependencies"
    )
...
function(_get_def_env_options str return_defs)
  ...
  properties_resolve_value_env("${str}" _tmp_str _req_var_list _opt_var_list _all_resolved)
  ...
  foreach(var_name IN LISTS _req_var_list _opt_var_list)
    ...
    if(DEFINED "${var_id}")
      ...
      set("${var_id}"
          "${${var_id}}"
          CACHE STRING "Default value for ${var_id} used in upload scripts"
      )
    endif()
  endforeach()
