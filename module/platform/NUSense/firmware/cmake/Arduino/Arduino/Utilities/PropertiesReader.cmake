...
function(properties_resolve_value value return_value namespace)
  ...
  _properties_expand_value("${value}" _result_value "${namespace}" _expanded_prop_list)
  set("${return_value}"
      "${_result_value}"
      PARENT_SCOPE
  )
...
function(properties_resolve_all_values namespace)
  ...
  foreach(_prop IN LISTS _prop_list)
    ...
    _properties_expand_value("${${namespace}.${_prop}}" _resolved_value "${namespace}" _expanded_prop_list)
...
function(properties_resolve_value_env value ret_value ret_req_var_list ret_opt_var_list ret_all_resolved)
  ...
  set("${ret_req_var_list}"
      "${_ret_req_var_list}"
      PARENT_SCOPE
  )
  set("${ret_opt_var_list}"
      "${_ret_opt_var_list}"
      PARENT_SCOPE
  )
  set("${ret_all_resolved}"
      "${_ret_all_resolved}"
      PARENT_SCOPE
  )
    ...
    while(NOT "${_previous_value}" STREQUAL "${_value}")
      ...
      foreach(_var_str IN LISTS _var_list)
        ...
        if(NOT DEFINED "/prop_int_resolved.${_var_name}")
          ...
          # references
          properties_set_value("/prop_int_resolved" "${_var_name}" "{${_var_name}}")

          # message("=> Resolve *** ${_var_name} *** : "
          #	"${${namespace}.${_var_name}}")
          _properties_expand_value("${${namespace}.${_var_name}}" _var_value "${namespace}")
          properties_set_value("/prop_int_resolved" "${_var_name}" "${_var_value}")
          # message("=> EXPANDED ${_var_name}: ${_var_value}")
        endif()

        string(REPLACE "${_var_str}" "${/prop_int_resolved.${_var_name}}" _value "${_value}")
