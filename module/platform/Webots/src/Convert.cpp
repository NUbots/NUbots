...
namespace module::platform::webots {
    ...
    float Convert::accelerometer(uint16_t value) {
        return (value - 78.48) * ACCELEROMETER_CONVERSION_FACTOR;
    }

    float Convert::gyroscope(uint16_t value) {
        return (value - 100) * GYROSCOPE_CONVERSION_FACTOR;
    }
    ...
    //     // See:
    //     //
    //     http://support.robotis.com/en/product/webots-op/references/reference/hardware_specifications/electronics/optional_components/fsr.htm
    //     else if (left) {
    ...
    //     return value >= 100 ? 254                                         // If we are greater then 100, then set to
    //     100
