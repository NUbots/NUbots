...
namespace module::platform::cm740 {
    ...
    RawSensors HardwareIO::parseSensors(const CM740::BulkReadResults& data) {
        ...
        sensors.platform_error_flags =
            data.cm740ErrorCode == 0xFF ? RawSensors::Error::_TIMEOUT : RawSensors::Error(data.cm740ErrorCode).value;
        ...
        if (sensors.battery <= cfg.battery.charged_voltage) {
            sensors.platform_error_flags &= ~RawSensors::Error::_INPUT_VOLTAGE;
        }
        ...
        sensors.fsr.right.error_flags =
            data.fsrErrorCodes[0] == 0xFF ? RawSensors::Error::_TIMEOUT : RawSensors::Error(data.fsrErrorCodes[0]).value;
        ...
        sensors.fsr.left.error_flags =
            data.fsrErrorCodes[1] == 0xFF ? RawSensors::Error::_TIMEOUT : RawSensors::Error(data.fsrErrorCodes[1]).value;
        ...
        for (int i = 0; i < 20; ++i) {
            ...
            // If we are using real data, get it from the packet
                ...
                // Error code
                servo.error_flags = data.servoErrorCodes[i] == 0xFF ? RawSensors::Error::_TIMEOUT
                ...
                if (servo.voltage <= cfg.battery.charged_voltage) {
                    servo.error_flags &= ~RawSensors::Error::_INPUT_VOLTAGE;
                }
            }
        }
        return sensors;
