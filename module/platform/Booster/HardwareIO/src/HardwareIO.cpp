...
HardwareIO::HardwareIO(std::unique_ptr<NUClear::Environment> environment) : NUClear::Reactor(std::move(environment)) {
    ...
    on<Startup>().then([this]() {
        ...
        low_state_channel =
            ChannelFactory::Instance()->CreateRecvChannel<booster_interface::msg::LowState>(
                "rt/low_state",
                [this](const void* msg) { low_state_handler(msg); });

        battery_channel =
            ChannelFactory::Instance()->CreateRecvChannel<booster_interface::msg::BatteryState>(
                "rt/battery_state",
                [this](const void* msg) { battery_handler(msg); });

        fall_down_channel =
            ChannelFactory::Instance()->CreateRecvChannel<booster_interface::msg::FallDownState>(
                "rt/fall_down",
        ...
        rc_channel =
            ChannelFactory::Instance()->CreateRecvChannel<booster_interface::msg::RemoteControllerState>(
                "rt/remote_controller_state",
                [this](const void*) {});

        robot_states_channel =
            ChannelFactory::Instance()->CreateRecvChannel<booster_interface::msg::RobotStatesMsg>(
                "rt/robot_states",
                [this](const void*) {});

        odometer_channel =
            ChannelFactory::Instance()->CreateRecvChannel<booster_interface::msg::Odometer>(
                "rt/odometer_state",
                [this](const void*) {});
    });
    ...
    on<Trigger<BoosterWalk>>().then([this](const BoosterWalk& move) {
        ...
        log<DEBUG>("Sending move command with velocity: " + std::to_string(move.velocity.x()) + " " + std::to_string(move.velocity.y()) + " " + std::to_string(move.velocity.z()));
    ...
    on<Trigger<BoosterVisualKick>>().then([this](const BoosterVisualKick& kick) {
        ...
        log<DEBUG>("Sending visual kick command: start=" + std::to_string(kick.start) + ", version=" + std::to_string(int(version)));
