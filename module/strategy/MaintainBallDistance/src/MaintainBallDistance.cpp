...
namespace module::strategy {
    ...
    MaintainBallDistance::MaintainBallDistance(std::unique_ptr<NUClear::Environment> environment)
        ...
        on<Provide<MaintainBallDistanceTask>, With<Ball>, With<Field>, With<Sensors>>().then(
            [this](const MaintainBallDistanceTask& task, const Ball& ball, const Field& field, const Sensors& sensors) {
                ...
                log<DEBUG>("Backing away from ball: current distance ",
                           current_distance,
                           "m, target ",
                           task.min_distance,
                           "m.");
                ...
                emit<Task>(
                    std::make_unique<WalkToFieldPosition>(pos_rpy_to_transform(target, Eigen::Vector3d(0, 0, heading)),
                                                          true));
            });
    }
