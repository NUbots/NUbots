    ...
    PlanKick::PlanKick(std::unique_ptr<NUClear::Environment> environment) : BehaviourReactor(std::move(environment)) {
        ...
        on<Provide<KickTo>, Uses<Kick>, Trigger<FilteredBall>, Trigger<Stability>>().then(
            [this](const KickTo& kick_to,
                ...
                float ball_angle    = std::abs(std::atan2(ball.rBGg.y(), ball.rBGg.x()));
                float ball_distance = ball.rBGg.head(2).norm();
                ...
                float align_angle = std::abs(std::atan2(kick_to.rPGg.y(), kick_to.rPGg.x()));
                ...
                if (cfg.kick_leg == LimbID::LEFT_LEG || (cfg.kick_leg == LimbID::UNKNOWN && ball.rBGg.y() > 0.0)) {
