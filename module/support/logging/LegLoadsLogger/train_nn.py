...
def loadfile(path, label=None, max_heigh_delta=0.005):
    with open(path, 'r') as f:
        ...
        for point in zip(data, data[1:]):
            ...
            # Left
            output.append((
                int(label if label != None else point[1][1] < point[1][0] + max_heigh_delta),
            ...
            output.append((
                int(label if label != None else point[1][0] < point[1][1] + max_heigh_delta),
...
negfiles = [
    'walk_load_datasets/airstand.csv', 'walk_load_datasets/airstand_old.csv', 'walk_load_datasets/airwalk.csv',
    'walk_load_datasets/airwalkforward.csv', 'walk_load_datasets/evenmoreairwalk_old.csv',
    'walk_load_datasets/moreairwalk_old.csv', 'walk_load_datasets/back_turtle.csv', 'walk_load_datasets/lyingback.csv'
]

# Files that have the feet down
posfiles = ['walk_load_datasets/standing.csv', 'walk_load_datasets/standing_old.csv']

# Files that have the feet changing based on the walk
mixedfiles = [
    'walk_load_datasets/morewalking_old.csv', 'walk_load_datasets/walkforward.csv',
    'walk_load_datasets/walking_old.csv', 'walk_load_datasets/walkonspot.csv', 'walk_load_datasets/walkspin.csv'
]
...
clf = MLPClassifier(
    alpha=1e-3, hidden_layer_sizes=(5), max_iter=5000, verbose=True, random_state=numpy.random.RandomState(1)
)
