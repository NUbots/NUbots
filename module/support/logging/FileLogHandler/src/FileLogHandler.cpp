    ...
    , logFile(logFileName, std::ios_base::out | std::ios_base::app | std::ios_base::ate) {
        ...
        on<Trigger<ReactionStatistics>>().then([this](const ReactionStatistics & stats) {
            if (stats.exception) {
                ...
                for (auto& s : utility::support::evil::stack) {
                    logFile << "\t" << Colour::brightmagenta << s.file
        ...
        on<Trigger<LogMessage>>().then([this] (const LogMessage& message) {
            ...
            switch(message.level) {
                case NUClear::TRACE:
                    logFile << source << "TRACE: ";
                    break;
                case NUClear::DEBUG:
                    logFile << source << Colour::green << "DEBUG: ";
                    break;
                case NUClear::INFO:
                    logFile << source << Colour::brightblue << "INFO: ";
                    break;
                case NUClear::WARN:
                    logFile << source << Colour::yellow << "WARN: ";
                    break;
                case NUClear::ERROR:
                    logFile << source << Colour::brightred << "ERROR: ";
                    break;
                case NUClear::FATAL:
                    logFile << source << Colour::brightred << "FATAL: ";
                    break;
            }
