...
namespace nsga2
{
	Individual::Individual(const IndividualConfigurator& _config)
	{
		...
		if (config->binBits.size() != config->binVars)
			std::cout << "binBits size != binVars" << std::endl;
	...
	void Individual::Decode()
	{
		...
		for (int i = 0; i < config->binVars; i++)
		{
			...
			bins[i] = config->binLimits[i].first +
				(double)sum * (config->binLimits[i].second -
				config->binLimits[i].first) /
		...
		for (int i = 0; i < config->realVars; i++)
		{
			if (config->randGen->Realu() <= config->realMutProb)
			{
				...
				if (randReal <= 0.5)
				{
					...
					value = 2.0 * randReal + (1.0 - 2.0 * randReal) *
						(std::pow(deltaInverse, (config->etaM + 1.0)));
					deltaq = std::pow(value, mutPower) - 1.0;
				}
				else
				{
					deltaInverse = 1.0 - delta2;
					value = 2.0 * (1.0 - randReal) + 2.0 * (randReal - 0.5) *
						(std::pow(deltaInverse, (config->etaM + 1.0)));
