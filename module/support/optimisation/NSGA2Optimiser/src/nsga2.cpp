...
namespace nsga2
{
	...
	void NSGA2::InitStreams()
	{
		...
		fpt1 << "# of objectives = " << objectives
			<< ", # of constraints = " << constraints
			<< ", # of real variables = " << realVars
			<< ", # of bits of binary variables = " << bitLength
			<< ", constraint violation, rank, crowding_distance\n";
		fpt2 << "# of objectives = " << objectives
			<< ", # of constraints = " << constraints
			<< ", # of real variables = " << realVars
			<< ", # of bits of binary variables = " << bitLength
			<< ", constraint violation, rank, crowding_distance\n";
		fpt3 << "# of objectives = " << objectives
			<< ", # of constraints = " << constraints
			<< ", # of real variables = " << realVars
			<< ", # of bits of binary variables = " << bitLength
			<< ", constrViolation, rank, crowding_distance\n";
		fpt4 << "# of objectives = " << objectives
			<< ", # of constraints = " << constraints
			<< ", # of real variables = " << realVars
			<< ", # of bits of binary variables = " << bitLength
			<< ", constrViolation, rank, crowding_distance\n";
	...
	void NSGA2::Selection(Population& _oldPop, Population& _newPop)
	{
		...
		{
			Individual& p11 = Tournament(_oldPop.inds[indList1[i]],
				_oldPop.inds[indList1[i + 1]]);
			Individual& p12 = Tournament(_oldPop.inds[indList1[i + 2]],
				_oldPop.inds[indList1[i + 3]]);
			Crossover(p11,p12,_newPop.inds[i],_newPop.inds[i + 1]);

			Individual& p21 = Tournament(_oldPop.inds[indList2[i]],
				_oldPop.inds[indList2[i + 1]]);
			Individual& p22 = Tournament(_oldPop.inds[indList2[i + 2]],
				_oldPop.inds[indList2[i + 3]]);
	...
	void NSGA2::Crossover(const Individual& _parent1, const Individual& _parent2,
		Individual& _child1, Individual& _child2)
	{
		if (realVars)
	...
	void NSGA2::Realcross(const Individual& _parent1, const Individual& _parent2,
		Individual& _child1, Individual &_child2)
	{
	...
	void NSGA2::Bincross(const Individual &_parent1, const Individual &_parent2,
		Individual &_child1, Individual &_child2)
	{
    ...
    void NSGA2::PostEvaluationAdvance()
    {
		...
		std::sort(mixedPop->front[i].begin(), mixedPop->front[i].end(),
