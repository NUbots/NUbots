        ...
        namespace optimisation {
            ...
            NSGA2Optimiser::NSGA2Optimiser(std::unique_ptr<NUClear::Environment> environment)
                ...
                on<Configuration>("NSGA2Optimiser.yaml").then([this](const Configuration& config) {
                    ...
                    nsga2_algorithm.SetRealCrossoverProbability(
                        config["probabilities"]["real"]["crossover"].as<double>());
                    nsga2_algorithm.SetBinCrossoverProbability(
                        config["probabilities"]["binary"]["crossover"].as<double>());
                    nsga2_algorithm.SetRealMutationProbability(
                        config["probabilities"]["real"]["mutation"].as<double>());
                    nsga2_algorithm.SetBinMutationProbability(
                ...
                on<Trigger<NSGA2FitnessScores>, Single>().then([this](const NSGA2FitnessScores& scores) {
                    ...
                    nsga2_algorithm.getCurrentPop()->SetEvaluationResults(scores.id,
                                                                          scores.obj_score,
                                                                          scores.constraints);
                    ...
                    if (nsga2_algorithm.getCurrentPop()->AreAllEvaluated()) {
                        ...
                        if (nsga2_algorithm.HasMetOptimisationTerminalCondition()) {
                            ...
                            log<NUClear::INFO>("Advanced to new generation",
                                               nsga2_algorithm.getCurrentPop()->generation);
                        }
                    }
                    else {
