...
namespace nsga2
{
	Population::Population(const int _size, const int _realVars, const int _binVars,
		const int _constraints, const std::vector<int> _binBits,
	...
	void Population::CrowdingDistance(int _frontI)
	{
		...
		for (int i = 0; i < limit; i++)
		{
			...
			for (int j = 1; j < l-1; j++)
			{
				...
				{
					if (crowdObj && inds[F[l-1]].objScore[i] !=
						inds[F[0]].objScore[i])
					{
						inds[F[j]].crowdDist += (inds[F[j+1]].objScore[i] -
							inds[F[j-1]].objScore[i]) / (inds[F[l-1]].objScore[i] -
							inds[F[0]].objScore[i]);
					}
					else if (!crowdObj && inds[F[l-1]].reals[i] !=
						inds[F[0]].reals[i])
					{
						inds[F[j]].crowdDist += (inds[F[j+1]].reals[i] -
							inds[F[j-1]].reals[i]) / (inds[F[l-1]].reals[i] -
							inds[F[0]].reals[i]);
					}
				}
			}
	...
	void Population::Merge(const Population& _pop1, const Population& _pop2)
	{
		...
		std::copy(_pop1.inds.begin(), _pop1.inds.end(),
			inds.begin());
		std::copy(_pop2.inds.begin(), _pop2.inds.end(),
			inds.begin() + _pop1.GetSize());
	}
