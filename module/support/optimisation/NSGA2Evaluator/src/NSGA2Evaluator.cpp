    ...
    namespace optimisation {
        ...
        NSGA2Evaluator::NSGA2Evaluator(std::unique_ptr<NUClear::Environment> environment)
            ...
            on<Configuration>("NSGA2Evaluator.yaml").then([this](const Configuration& config) {
                ...
                optimizeScript    = true;
                if (optimizeScript) script = YAML::LoadFile("scripts/nubotsvm/KickRightArmsExtended.yaml").as<Script>();
            ...
            on<Trigger<Sensors>, Single>().then([this](const Sensors& sensors) {
                ...
                if (!terminating && evaluating && !finished && fieldPlaneSway > maxFieldPlaneSway && simTime > 0.25) {
        ...
        void NSGA2Evaluator::CalculateFitness() {
            ...
            if (fallenOver || maxFieldPlaneSway == 0 || maxFieldPlaneSway > 1000.0) maxFieldPlaneSway = 1000.0;
