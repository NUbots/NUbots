...
namespace module::input {
    ...
    void SensorFilter::integrate_walkcommand(const double dt,
        ...
        if (stability == Stability::DYNAMIC && walking_state.state == WalkingState::State::WALKING) {
            // Integrate the walk command to estimate the change in position and yaw orientation
            double dx = walking_state.walk_command.x() * dt * cfg.deadreckoning_scale.x();
            double dy = walking_state.walk_command.y() * dt * cfg.deadreckoning_scale.y();
            yaw += walking_state.walk_command.z() * dt * cfg.deadreckoning_scale.z();
    ...
    void SensorFilter::update_odometry_mahony(std::unique_ptr<Sensors>& sensors,
        ...
        integrate_walkcommand(dt, stability, walking_state);
