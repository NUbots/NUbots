...
namespace module::input {
    ...
    void Stella::start_socket_once() {
        if (socket_thread_running.exchange(true))
            return;
    ...
    Stella::Stella(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
        on<Configuration>("Stella.yaml").then([this](const Configuration& config) {
            ...
            socket_publisher = std::make_shared<socket_publisher::publisher>(config["SocketPublisher"],
                                                                             slam_system,
                                                                             frame_publisher,
                                                                             map_publisher);
        ...
        on<Trigger<Image>, Single, MainThread>().then([this](const Image& image) {
            ...
            if (!debug_frame.empty()) {
                ...
                if (log_level <= DEBUG) {
                    log<DEBUG>("Emitted Stella debug frame with ORB detections, frame: ",
                               debug_image->id,
                               ", size: ",
                               jpeg_data.size(),
                               " bytes");
