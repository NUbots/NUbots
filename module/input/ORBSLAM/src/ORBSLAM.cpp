...
namespace module::input {
    ...
    ORBSLAM::ORBSLAM(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
        ...
        on<Configuration>("ORBSLAM.yaml").then([this](const Configuration& config) {
            ...
            slamSystem = std::make_unique<ORB_SLAM3::System>(config["vocabulary"].as<std::string>(),
        ...
        on<Trigger<Image>, Single>().then([this](const Image& image) {
            ...
            switch (imageFourColourCode) {
                ...
                case utility::vision::FOURCC::JPEG:
                    cvImageInput =
                        cv::Mat(imageDimensionsY, imageDimensionsX, CV_8UC3, const_cast<uint8_t*>(image.data.data()));
                ...
                case utility::vision::FOURCC::GREY:
                    cvImageInput =
                        cv::Mat(imageDimensionsY, imageDimensionsX, CV_8UC1, const_cast<uint8_t*>(image.data.data()));
            ...
            log<NUClear::DEBUG>("Hcw: ", Hcw.matrix());
        });
    }
