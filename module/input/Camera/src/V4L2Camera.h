...
/**
 * @brief This struct encapsulates the physical camera device. It will setup a
 * camera device and begin streaming images
 *
 * @details
 *    This class uses the Video4Linux2 kernel drivers in order to connect to and
 * get data from the darwins built in webcam. It allocates 2 kernel mode buffers
 * which are mapped into user space. These buffers are alternated and the
 * data from the most recently filled one is used to construct an image object.
 * This class also provides easy access to all the settings that are available
 * on the camera. It is provided as a map in order to make accessing the
 * paramters by name (from a config file) easier
 *
...
struct V4L2Camera {
 private:
  ...
  /// @brief this map is used to hold several ioctl wrappers that let us set
  /// settings easily
 ...
 public:
  ...
  /// @brief this enum holds important constants (we are c++ we don't use
  /// defines for this kind of thing)
  enum {
  ...
  /**
   * @brief Constructs a new DarwinCamera class using the passed string as the
   * device
   *
   * @param device the path to the video device to use (e.g. /dev/video0)
   */
  V4L2Camera(const ::extension::Configuration& config,
  ...
  /**
   * @brief Gets a pointer to the latest image from the camera so that it can be
   * sent out to the rest of the system
   *
   * @details
   *   This function blocks until the camera device provides a new frame of
   * video data, at which point it copies the frame into a new Image and
   * returns. The camera device must already be set up (using resetCamera) and
   *
   ...
   */
  void resetCamera(const std::string& device, const std::string& fmt,
  ...
  NUClear::threading::ReactionHandle& setCameraHandle() { return cameraHandle; }
  ...
  /**
   * @brief returns the format that the camera is currently reading (YUYV or
   * MJPG)
   */
  ...
  /**
   * @brief This method is to be called when shutting down the system. It does
   * cleanup on the cameras resources
   */
  void closeCamera();
