...
namespace input {
    ...
    CameraContext::TimeCorrection sync_clocks(ArvDevice* device) {
        ...
        for (auto& s : samples) {
            ...
            arv_device_execute_command(device, latch_command.c_str());
            ...
            if (arv_device_get_status(device) == ARV_DEVICE_STATUS_SUCCESS) {
                s.cam   = arv_device_get_integer_feature_value(device, read_command.c_str());
                s.valid = arv_device_get_status(device) == ARV_DEVICE_STATUS_SUCCESS;
            }
            else {
                s.valid = false;
            }
