...
V4L2Camera Camera::initiateV4L2Camera(const Configuration& config) {
  // This trigger gets us as close as we can to the frame rate as possible (as
  // high resolution as we can)
  V4L2FrameRateHandle =
      on<Every<V4L2Camera::FRAMERATE, Per<std::chrono::seconds>>, Single>()
          .then("Read V4L2Camera", [this] {
  ...
  V4L2SettingsHandle = on<Every<1, std::chrono::seconds>>().then(
      "V4L2 Camera Setting Applicator", [this] {
          ...
          try {
            if (camera.second.isStreaming()) {
              ...
              for (auto& setting : camera.second.getConfig().config) {
                ...
                if (it != settings.end()) {
                  if (camera.second.setSetting(
                          it->second, setting.second.as<int>()) == false) {
                    log<NUClear::DEBUG>("Failed to set", it->first, "on camera",
                                        camera.first);
                  }
                }
              }
  ...
  cameraParameters->imageSizePixels << config["imageWidth"].as<uint>(),
      config["imageHeight"].as<uint>();
  cameraParameters->FOV << config["FOV_X"].as<double>(),
      config["FOV_Y"].as<double>();
  ...
  cameraParameters->pixelsToTanThetaFactor << (tanHalfFOV[0] / imageCentre[0]),
      (tanHalfFOV[1] / imageCentre[1]);
  cameraParameters->focalLengthPixels = imageCentre[0] / tanHalfFOV[0];

  emit<Scope::DIRECT>(std::move(cameraParameters));

  log("Emitted camera parameters for camera",
  ...
  try {
    ...
    NUClear::log<NUClear::DEBUG>(
        std::string("Exception while setting camera configuration: ") +
        e.what());
    throw e;
  }
}
...
message::input::Image V4L2Camera::getImage() {
  ...
  if (ioctl(fd, VIDIOC_DQBUF, &current) == -1) {
    throw std::system_error(errno, std::system_category(),
                            "There was an error while de-queuing a buffer");
  }
  ...
  auto monotonicTime = std::chrono::microseconds(current.timestamp.tv_usec) +
                       std::chrono::seconds(current.timestamp.tv_sec);
  auto mclock = std::chrono::duration_cast<std::chrono::microseconds>(
      std::chrono::steady_clock::now().time_since_epoch());
  auto nclock = std::chrono::duration_cast<std::chrono::microseconds>(
      NUClear::clock::now().time_since_epoch());
  auto timestamp =
      NUClear::clock::time_point(monotonicTime + (nclock - mclock));
  ...
  requeue.m.userptr =
      reinterpret_cast<unsigned long int>(buffers[current.index].data());
  requeue.length = buffers[current.index].capacity();

  if (ioctl(fd, VIDIOC_QBUF, &requeue) == -1) {
    throw std::system_error(errno, std::system_category(),
                            "There was an error while re-queuing a buffer");
  };
...
void V4L2Camera::resetCamera(const std::string& device, const std::string& fmt,
                             const FOURCC& cc, size_t w, size_t h) {
  ...
  if (fd < 0) {
    throw std::runtime_error(
        std::string("We were unable to access the camera device on ") +
        deviceID);
  }
  ...
  if (ioctl(fd, VIDIOC_S_FMT, &format) == -1) {
    throw std::system_error(
        ...
        "There was an error while setting the cameras format");
  }
  ...
  if (ioctl(fd, VIDIOC_G_PARM, &param) == -1) {
    throw std::system_error(
        errno, std::system_category(),
        "We were unable to get the current camera FPS parameters");
  }
  ...
  if (ioctl(fd, VIDIOC_REQBUFS, &rb) == -1) {
    throw std::system_error(errno, std::system_category(),
                            "There was an error configuring user buffers");
  }
  ...
  settings.insert(
      std::make_pair("power_line_frequency", V4L2_CID_POWER_LINE_FREQUENCY));
  settings.insert(
      std::make_pair("auto_white_balance", V4L2_CID_AUTO_WHITE_BALANCE));
  settings.insert(std::make_pair("white_balance_temperature",
                                 V4L2_CID_WHITE_BALANCE_TEMPERATURE));
  settings.insert(std::make_pair("auto_exposure", V4L2_CID_EXPOSURE_AUTO));
  settings.insert(std::make_pair("auto_exposure_priority",
                                 V4L2_CID_EXPOSURE_AUTO_PRIORITY));
  settings.insert(
      std::make_pair("absolute_exposure", V4L2_CID_EXPOSURE_ABSOLUTE));
  settings.insert(std::make_pair("backlight_compensation",
...
void V4L2Camera::startStreaming() {
  if (!streaming) {
    ...
    if (ioctl(fd, VIDIOC_STREAMON, &command) == -1) {
      throw std::system_error(errno, std::system_category(),
                              "Unable to start camera streaming");
    }
...
void V4L2Camera::stopStreaming() {
  if (streaming) {
    ...
    if (ioctl(fd, VIDIOC_STREAMOFF, &command) == -1) {
      throw std::system_error(errno, std::system_category(),
                              "Unable to stop camera streaming");
    }
...
void V4L2Camera::setConfig(const ::extension::Configuration& _config) {
  ...
  if (width != static_cast<size_t>(w) || height != static_cast<size_t>(h) ||
      format != fmt || deviceID != ID) {
...
int32_t V4L2Camera::getSetting(unsigned int id) {
  ...
  if (ioctl(fd, VIDIOC_QUERYCTRL, &queryctrl) == -1) {
    throw std::system_error(errno, std::system_category(),
                            "There was an error while attempting to get the "
                            "status of this camera value");
  }
  ...
  if (ioctl(fd, VIDIOC_G_CTRL, &control_s) < 0) {
    throw std::system_error(
        errno, std::system_category(),
        "There was an error while trying to get the current value");
  }
