...
Camera::Camera(std::unique_ptr<NUClear::Environment> environment)
    : Reactor(std::move(environment)),
      ...
      SpinnakerCamList(),
      SpinnakerCameras() {
  on<Configuration>("Camera.yaml")
      .then("Camera system configuration",
            [this](const Configuration& /*config*/) {
              // Use configuration here from file Camera.yaml
            });

  on<Configuration>("Cameras").then(
      "Camera driver loader", [this](const Configuration& config) {
        ...
        if (driver == "V4L2") {
          ...
          if (cam == V4L2Cameras.end()) {
            V4L2Cameras.insert(std::make_pair(
                deviceID, std::move(initiateV4L2Camera(config))));
            cameraCount++;
          }
