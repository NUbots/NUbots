                    ...
                    if (device_no == -1) {
                        ...
                        if (!ARV_IS_CAMERA(camera.get())) {
                            ...
                            if (!ARV_IS_STREAM(stream.get())) {
                                throw std::runtime_error(fmt::format("Failed to create stream for {} camera ({})",
                                                                     name,
                                                                     device_description));
                            }
                            ...
                            log<NUClear::INFO>(
                                fmt::format("Connected to the {} camera ({})", name, device_description));
                        }
                    }
                }
                ...
                if (arv_buffer_get_status(buffer) == ARV_BUFFER_STATUS_SUCCESS) {
                    ...
                    /* Mutex Scope */ {
                        ...
                        if (reactor.Hwps.empty()) {
                            ...
                            auto Hwp_it =
                                std::lower_bound(reactor.Hwps.begin(),
                                                 reactor.Hwps.end(),
