...
namespace module
{
    namespace input
    {
        ...
        void Camera::resetSpinnakerCamera(std::map<std::string, std::unique_ptr<SpinnakerImageEvent>>::iterator& camera, const Configuration& config)
        {
            ...
            if (!utility::vision::setNumericParam(sNodeMap, "StreamDefaultBufferCount", config["buffer_count"].as<int64_t>()))
            {
            ...
            if (!utility::vision::setEnumParam(nodeMap, "ExposureAuto", (std::isfinite(exposure.value) ? "Off" : "Continuous")))
            {
            ...
            */
            ...
            if (!utility::vision::setEnumParam(nodeMap, "BlackLevelAuto", ((std::isfinite(blackLevel.value) && (blackLevel.value != 0.0)) ? "Off" : "Continuous")))
            {
            ...
            if (!utility::vision::setEnumParam(nodeMap, "SharpnessAuto", (std::isfinite(sharpness.value) ? "Off" : "Continuous")))
            {
            ...
            */
            ...
            if (!utility::vision::setEnumParam(nodeMap, "SaturationAuto", (std::isfinite(saturation.value) ? "Off" : "Continuous")))
            {
