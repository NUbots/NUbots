...
json_weak struct json_value_s* json_parse_ex(const void* src,
                                             size_t src_size,
                                             size_t flags_bitset,
    ...
    while (quote_to_use != src[offset]) {
        if ('\\' == src[offset]) {
            ...
            switch (src[offset++]) {
                ...
                case '"': data[bytes_written++] = '"'; break;
                case '\\': data[bytes_written++] = '\\'; break;
                case '/': data[bytes_written++] = '/'; break;
                case 'b': data[bytes_written++] = '\b'; break;
                case 'f': data[bytes_written++] = '\f'; break;
                case 'n': data[bytes_written++] = '\n'; break;
                case 'r': data[bytes_written++] = '\r'; break;
                case 't': data[bytes_written++] = '\t'; break;
                case '\r':
...
void json_parse_number(struct json_parse_state_s* state, struct json_number_s* number) {
    ...
    while (offset < size) {
        ...
        switch (src[offset]) {
            ...
            case 'E':
            case '+':
            case '-': data[bytes_written++] = src[offset++]; break;
            default: end = 1; break;
        }
...
struct json_extract_result_s json_extract_get_value_size(const struct json_value_s* const value) {
    ...
    switch (value->type) {
        ...
        case json_type_object:
            ...
            break;
        case json_type_array: result = json_extract_get_array_size((const struct json_array_s*) value->payload); break;
...
int json_write_minified_get_value_size(const struct json_value_s* value, size_t* size) {
    switch (value->type) {
        default:
            ...
            return 1;
        case json_type_number: return json_write_get_number_size((struct json_number_s*) value->payload, size);
        case json_type_string: return json_write_get_string_size((struct json_string_s*) value->payload, size);
        case json_type_array: return json_write_minified_get_array_size((struct json_array_s*) value->payload, size);
        case json_type_object: return json_write_minified_get_object_size((struct json_object_s*) value->payload, size);
        case json_type_true:
    ...
    for (i = 0; i < string->string_size; i++) {
        switch (string->string[i]) {
            ...
            case '\t':
                ...
                *data++ = 't';
                break;
            default: *data++ = string->string[i]; break;
        }
    }
...
char* json_write_minified_value(const struct json_value_s* value, char* data) {
    switch (value->type) {
        ...
        case json_type_array: return json_write_minified_array((struct json_array_s*) value->payload, data);
        case json_type_object: return json_write_minified_object((struct json_object_s*) value->payload, data);
        case json_type_true:
            data[0] = 't';
            data[1] = 'r';
            data[2] = 'u';
...
int json_write_pretty_get_value_size(const struct json_value_s* value,
    ...
    switch (value->type) {
        ...
        case json_type_array:
            return json_write_pretty_get_array_size((struct json_array_s*) value->payload,
                                                    depth,
                                                    indent_size,
                                                    newline_size,
                                                    size);
        case json_type_object:
            return json_write_pretty_get_object_size((struct json_object_s*) value->payload,
                                                     depth,
                                                     indent_size,
                                                     newline_size,
                                                     size);
