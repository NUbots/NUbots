...
Builds a single, size-bounded packet for debugging localisation during an actual RoboCup Humanoid Soccer League match.
...
- Reduces each `message::localisation::Robot` down to a `RobotSummary` (`id`/`rRWw`/`teammate` only)
