...
ImageCompressor::ImageCompressor(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
    ...
    on<Configuration>("ImageCompressor.yaml").then("Configure Compressors", [this](const Configuration& cfg) {
        ...
        for (const auto& c : cfg["compressors"].config) {
            if (c["name"].as<std::string>() == "vaapi") {
                ...
                config.factories.emplace_back(std::make_shared<compressor::turbojpeg::Factory>(c["quality"].as<int>()),
                                              c["concurrent"].as<int>());
            }
        }
    });
