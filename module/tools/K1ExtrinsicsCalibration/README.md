...
Place the robot at the centre of the field facing the goal, then run the `k1extrinsicscalibration` role (the K1
stack: K1Sensors / K1Camera / K1Walk / K1GetUp / K1Look + Booster HardwareIO). The robot stands itself up and sweeps
its head automatically — no manual control needed. Select the camera to calibrate and the field type via
configuration. Run again with the other camera to calibrate both.
