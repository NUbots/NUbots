    ...
    on<Configuration>("SystemConfiguration.yaml").then([this](const Configuration& config) {
        ...
        for (const auto& l : config["links"].config) {
            ...
            if (!fs::exists(target)) {
                ...
                if (!fs::is_symlink(link)) {
                    log<NUClear::WARN>(fmt::format("File '{}' already exists but is not a symlink. Deleting it", link));
        ...
        for (const auto& g : config["groups"].config) {
            ...
            if (pos == std::string::npos) {
                log<NUClear::INFO>(fmt::format("Group {} doesn't exist. Creating it and adding user {}", group, user));
