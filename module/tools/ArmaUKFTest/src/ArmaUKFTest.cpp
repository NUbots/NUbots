...
namespace tools {
    ...
    ArmaUKFTest::ArmaUKFTest(std::unique_ptr<NUClear::Environment> environment)
        : Reactor(std::move(environment)), time_count(0) {

        on<Configuration>("ArmaUKFTest.yaml").then([this](const Configuration& config) {
            ...
            model_filter.setState(config["initial_state"].as<arma::vec2>(),
        ...
        filter_handle =
            on<Every<20, Per<std::chrono::seconds>>>()
                .then([this] {
                    ...
                    innovations.push_back(measurements[time_count] - model_filter.get()(0));
