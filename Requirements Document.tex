...
\begin{document}
    ...
    \section{Scenarios}
        ...
        \subsection{New Component Added to System}
            \todo[inline]{Write the scenario for a new component added}
            
        \subsection{Debugging When Bug is Found}
            \todo[inline]{Write the scenario for a bug occuring}
        
        \subsection{The System is moved to a new platform}
            \todo[inline]{Write the scenario for the system is moved to a new platform}
            
        \subsection{The Robot is used to perform a new task}
            \todo[inline]{Write the scenario for when the robot is given a new task
            (e.g. catering for university)}
    ...
    \section{Non-Goals}
        ...
        \subsection{Algorithm Changes}
            \todo[inline]{Write the nongoal that we will not do changes to the algorithm}
            
        \subsection{Hardware Changes}
            \todo[inline]{write the nongoal that states that the project will not cause hardware
            changes}
    ...
    \section{Overview}
        ...
        \subsection{Existing Architecture} 
            ...
            \begin{figure}
                ...
                \end{tikzpicture}
                \caption {The existing architecture} \label {fig:HighLevelExistingArchitecture}
            \end{figure}

            The existing architecture is composed of a number of interconnected components that communicate through individually defined interfaces and mechanisms.
            For example: The vision system communicates utilizing a two step process. First the vision system requests that the sensors retrieve a new frame. It then waits
            on the sensor system to place the frame information on blackboard. Once the frame information is placed on blackboard the vision system then reads the information
            from blackboard and continues it's processing. This is only one of the myriad of ways in which two components can communicate and we believe that this reduces
            the ability to both learn and modify the system.
        ...
        \subsection{Proposed Architecture}
            ...
            \begin{figure}[h]
                \centering
                \begin{tikzpicture}
                    \node [draw,star,star points=8,star point ratio=0.875,minimum width=5cm] (powerplant) {\Huge Power Plant};
                    \foreach [count=\i] \reactor in {Vision,Localization,Behavior,Sensors,Kinematics,Movement} {
            ...
            of as callbacks that are called when new data comes in. For example: A reaction that is called whenever new image data is retrived. \emph{Emit} allows reactors to
            ...
            Reactors also provide one special type of event named \textbf{Every} which lets you subscribe to an event that is garunteed to tick at consistent intervals. 
                ...
                \begin{tikzpicture}[>=latex,
                	...
                	\begin{scope}[start chain=going below, chained]
                         ...
                         \begin{scope}[start branch=on going right, every join/.style={<-}]
                             \node [block,join] (onimage) {on\textless \textbf{Image}\textgreater \\(processimage)};
                         \end{scope}
                         
                         \begin{scope}[start branch=on going left, every join/.style={<-}]
                             \node [block,join] (onevery) {on\textless \textbf{Every 30ms} \textgreater\\(getimage)};
                         \end{scope}
                         
                         \node[block,join] (start) {Start game};
                	    
                	    \node [block, join] (runtimeStart) {Begin sending\\\textbf{Every\textless ...\textgreater} events};
                         \node [block, join, on chain=going left] {Execute reaction(s) \\for \textbf{Every\textless 30ms\textgreater}};
                         \node [block, join] {Get image\\from camera};
                         \node [block, join] {Emit(\textbf{Image})};
                         \node [block, join, on chain=going right] {Call all reactions\\interested in \textbf{Image}};
                         \node [block, join, on chain=going right] {Execute reaction(s)\\for \textbf{Image}};
                	...
                	\coordinate (setupTextPoint) at ([xshift=11em]$ (start) !.5! (runtimeStart) $);
                     ...
                     \draw [dashed, ultra thick, shorten >= -0.4cm, shorten <= -0.4cm] (setuplineA) -- (setuplineB);
                     ...
                     \begin{scope}[on background layer]
                         \filldraw [gray!15] (sensorsHeader.south west) rectangle (sensorsHeader.east |- final.south);
                         \filldraw [gray!15] (visionHeader.south west) rectangle (visionHeader.east |- final.south);
                     \end{scope}
                     %\coordinate (sensorsBottom) at (sensorsHeader |- finalEmit);
            ...
            hardware-dependant camera system with a completely different module that reads a preproduced video stream for testing purposes. It also means that adding a new
            ...
            Unlike humans, computers are becoming exceptionally good at multitasking. Unfortutely the current system only takes advantage of at most two cores. You wouldn't 
            ride horse that only used one of it's legs so it's equally important that the proposed architecture takes full advantage of the robots ever-improving multitaking 
            ...
            how to get the robot to deliver pizza to the lab. When a reaction is triggered it doesn't neccecarily run immediately. Instead the reaction is put into a blocking priority queue
            ...
            \begin{figure}[b]
                \begin{tikzpicture}
                \end{tikzpicture}
                \missingfigure {Power Plant Threading Overview Todo!}
            ...
            for your data. This means you can emit data on one robot and recieve it on any of the other robots. 
            ...
            \begin{figure}[b]
                ...
                \begin{tikzpicture}[x=15em,y=5em,>=latex,
                    ...
                    %%% Left Power Plant
                    \node at (0, 0) [draw,star,star points=8,star point ratio=0.875,minimum width=3cm] (plantA) {Power Plant A};
                    ...
                    %%% Right Power Plant
                    \node at (2, 0) [draw,star,star points=8,star point ratio=0.875,minimum width=3cm] (plantB) {Power Plant B};
                    ...
                    \node at (1, 0) [draw,cloud,cloud puffs=10,minimum width=3.5cm] (network) {Network};
                ...
                \end{tikzpicture}
                \caption {An example of how robots can communicate camera data over a network to build a 2d map of the field} \label{fig:NetworkExampleDiagram}
            \end{figure}
    ...
    \section{Requirements}
        ...
        \subsection{Robot Platforms}
            ...
            \todo[inline]{Double check that AIBO is the appropriate name for the platform.}
        ...
        \subsection{Networking}
            ...
            \todo[inline]{Provide a citation for protocol buffers}
        ...
        \subsection{Debugging Tools}
            ...
            \todo[inline]{Provide a measurable for the debugging tools}
    ...
    \section{Extension Goals}
        ...
        \subsection{The architecture could support ROS (Robot Operating System) integration}
            ...
            Additionally no other Robocop teams have any degree of ROS support. Having ROS support
