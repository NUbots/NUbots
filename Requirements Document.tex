...
\usepackage{hyperref}
% Lets us rotate figures 90 degrees
\usepackage{rotating}
...
\begin{document}
    ...
    \section{Overview}
        ...
        \subsection{Existing Architecture} 
            ...
            The existing architecture is composed of a number of interconnected components that
            communicate through individually defined interfaces and mechanisms. For example: The
            vision system communicates utilising a two step process. First the vision system
            requests that the sensors retrieve a new frame. It then waits on the sensor system to
            place the frame information on blackboard. Once the frame information is placed on
            blackboard the vision system then reads the information from blackboard and continues
            it's processing. This is only one of the myriad of ways in which two components can
            communicate and we believe that this reduces the ability to both learn and modify the
            system.
        ...
        \subsection{Proposed Architecture}
            ...
            \begin{figure}[b]
                \begin{tabular}{l|l|l|l}
                    ...
                    Every 20ms       & Read Camera Image      & CameraImage      & 5ms      \\
                    Every 20ms       & Read Sensor Data       & SensorData       & 4ms      \\
                    SensorData       & Calculate Odometry     & OdometryData     & 3ms      \\
                    CameraImage      & Process Image          & ProcessedImage   & 7ms      \\
                    ProcessedImage   & Calculate Localisation & LocalizationData & 3ms      \\
                    LocalizationData & Behaviour              & MotorTask        & 2ms      \\
                \end{tabular}
                
                \begin{tikzpicture}
                    
                    % Add headers for the three threads
                    \node (everyThread) {Every Thread};
                    \node [below=of everyThread] (t2header) {Thread 1};
                    \node [below=of t2header] (t3header) {Thread 2};
                    \node [below=of t3header] (t4header) {Thread 3};
                    
                    % Add nodes to represent tasks
