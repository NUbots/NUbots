	...
	\section{Scenarios}
		...
		\subsection{New Functionality Added to System}
			...
			\subsubsection{Current System}
				...
				To add a new behavior we also need to understand the Job and Motion system and the high level Behavior finite state machine.
			...
			\subsubsection{Proposed System}
				...
				We also still need to add a new behavior to look upwards. We no longer need to change the job system as it's merged
		...
		\subsection{Component expanded to use Networking with other Robots}
			...
			\subsubsection{Current System}
				...
				To implement this behavior sharing mechanism a new packet type needs to be declared and a deep
				understanding of Win32 sockets, POSIX sockets and the existing network code is needed. Once the new
				packet is defined serialization methods need to be written to convert it to a network compatible representation
				and back. Custom network code then needs to be written for the behavior system to handle sending/recieving the
				networked data to ensure it is used correctly.
				
				Once the network transfer is complete the behavior system then needs to be modified to utilize the new information.
			...
			\subsubsection{Proposed System}
				...
				to the Behavior class which is a tiny modification in the proposed architecture.
				
				Once the handler is hooked up you simply use the standard api tools to send the networked packet
				between the robots. The architecture will handle details like serialization, sockets and other network
				issues.
				
				As with the existing architecture you still need to modify the behavior system to utilize the new data.
				\todo[inline]{Write how the proposed system would resolve this scenario}
		...
		\subsection{The System is moved to a new platform}
			...
			\subsubsection{Proposed System}
				Because of the loose coupling between components it's only neccecary to replace the components that directly depend on
		...
		\subsection{The Robot is used to perform a new task}
			...
			"So you think you can RoboDance".
