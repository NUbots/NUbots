    ...
    \section{Overview}
        ...
        \subsection{Proposed Architecture}
            ...
            \begin{figure}[b]
                ...
                \begin{tikzpicture}%[task/.style={rectangle,draw,anchor=west,minimum width=2em}]
                    
                    % Show 3 cycles of the system
                    
                    % Add headers for the three threads
                    %\node at (0,2) (thread1) {Thread 1};
                    %\node at (0,1) (thread2) {Thread 2};
                    %\node at (0,0) (thread3) {Thread 3};

                    \begin{scope}[task/.style={draw=black,fill=blue!10},scale=.80]
                        \draw (1, 1) grid (21, 4);

                        %% Thread 1
                        \draw[task] (1, 3) rectangle node {Camera} (6, 4);             
                        \draw[task] (13, 3) rectangle node {Localisation} (17, 4);
                        \draw[task] (19, 3) rectangle node {Motors} (21, 4);
                        
                        %% Thread 2
                       \draw[task] (1, 2) rectangle node {Sensors} (5, 3);
                       \draw[task] (6, 2) rectangle node {Vision} (13, 3);
                       \draw[task] (17, 2) rectangle node {Behavior} (19, 3);
                       
                        %% Thread 3
                        \draw[task] (5, 1) rectangle node {Odometry} (8, 2);
...
%                    \node[task, minimum width=10em, right=1em of thread1] (C) {C};
%                    \node[task, minimum width=8em, right=1em of thread2] (S)  {S};
%                    \node[task, minimum width=6em,right=0 of S] {O};
%                    \node[task, minimum width=14em, right=0 of C] {V};
