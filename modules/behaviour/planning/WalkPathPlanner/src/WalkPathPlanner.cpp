            ...
            WalkPathPlanner::WalkPathPlanner(std::unique_ptr<NUClear::Environment> environment) : Reactor(std::move(environment)) {
                ...
                on<Trigger<Every<20, Per<std::chrono::seconds>>>,
                   ...
                   >([this] (
                    ...
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    if(planType == messages::behaviour::WalkApproach::StandStill){
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        emit(std::make_unique<WalkStopCommand>());
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        return;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                     } else if(planType == messages::behaviour::WalkApproach::DirectCommand){
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        std::unique_ptr<WalkCommand> command = std::make_unique<WalkCommand>();
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        command->velocity = currentTargetPosition;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        command->rotationalSpeed = currentTargetHeading[0];
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        emit(std::move(command));
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        emit(std::move(std::make_unique<WalkStartCommand>()));
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        return;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    }

                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    // std::cout << "starting path planning" << std::endl;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    // arma::vec normed_heading = arma::normalise(selfs.front().heading);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    // arma::mat robotToWorldRotation;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    // robotToWorldRotation << normed_heading[0] << -normed_heading[1] << arma::endr
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    //                      << normed_heading[1] <<  normed_heading[0];
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    // arma::vec ballPos = robotToWorldRotation * arma::vec(ball.position) + arma::vec(selfs.front().position);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    arma::vec ballPos = RobotToWorldTransform(selfs.front().position, selfs.front().heading, ball.position);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    //std::cout << "ball pos found" << std::endl;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    arma::vec targetPos,targetHead;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    //work out where we're going
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    if (targetPosition == messages::behaviour::WalkTarget::Robot) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        //XXX: check if robot is visible
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    } else if (targetPosition == messages::behaviour::WalkTarget::Ball) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        targetPos = ballPos;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    } else { //other types default to position/waypoint location
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        targetPos = currentTargetPosition;
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    }
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    //work out where to face when we get there
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    if (targetHeading == messages::behaviour::WalkTarget::Robot) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        //XXX: check if robot is visible
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    } else if (targetHeading == messages::behaviour::WalkTarget::Ball) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        targetHead = arma::normalise(ballPos-targetPos);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    } else { //other types default to position/waypoint bearings
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        targetHead = arma::normalise(arma::vec(currentTargetHeading)-targetPos);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    }
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    //calculate the basic movement plan
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    arma::vec movePlan = arma::vec({0.0,0.0,0.0});

                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    if(planType == messages::behaviour::WalkApproach::ApproachFromDirection) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        movePlan = approachFromDirection(selfs.front(),targetPos,targetHead);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    } else if(planType == messages::behaviour::WalkApproach::WalkToPoint) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        movePlan = goToPoint(selfs.front(),targetPos,targetHead);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    } else if(planType == messages::behaviour::WalkApproach::OmnidirectionalReposition) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        movePlan = goToPoint(selfs.front(),targetPos,targetHead);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    }

                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    //work out if we have to avoid something
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    if (useAvoidance && (robots != NULL) && robots->size() > 0) {
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        //this is a vision-based temporary for avoidance
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                        movePlan = avoidObstacles(*robots,movePlan);
                    std::cerr<<__FILE__<<", "<<__LINE__<<": "<<__func__<<std::endl;                    }
