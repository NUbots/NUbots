...
void FieldLine::set(const LSFittedLine& screenLine, const LSFittedLine& groundLine) {
    ...
    if(screenLine.valid) {
        ...
        m_endPoints[0].screenCartesian = arma::vec2({-1, -1});
        m_endPoints[1].screenCartesian = arma::vec2({-1, -1});   //-1 is an invalid pixel location
    }
    ...
    if(groundLine.valid) {
        ...
        m_endPoints[0].groundCartesian = arma::vec2({-1, -1});
        m_endPoints[1].groundCartesian = arma::vec2({-1, -1});   //-1 is an impossible ground location
    }
}
...
double FieldLine::findScreenError(VisionFieldObject* other) const {
    ...
    double d1 = (arma::norm(m_endPoints[0].screenCartesian - l->m_endPoints[0].screenCartesian, 2) + arma::norm(m_endPoints[1].screenCartesian - l->m_endPoints[1].screenCartesian, 2));
    double d2 = (arma::norm(m_endPoints[0].screenCartesian - l->m_endPoints[1].screenCartesian, 2) + arma::norm(m_endPoints[1].screenCartesian - l->m_endPoints[0].screenCartesian, 2));
...
double FieldLine::findGroundError(VisionFieldObject* other) const {
    ...
    // distances vary depending on endpoint assignment
    double d1 = (arma::norm(m_endPoints[0].groundCartesian - l->m_endPoints[0].groundCartesian, 2) + arma::norm(m_endPoints[1].groundCartesian - l->m_endPoints[1].groundCartesian, 2));
    double d2 = (arma::norm(m_endPoints[0].groundCartesian - l->m_endPoints[1].groundCartesian, 2) + arma::norm(m_endPoints[1].groundCartesian - l->m_endPoints[0].groundCartesian, 2));
...
std::ostream& operator<< (std::ostream& output, const FieldLine& line) {
    ...
    output << "\trelative field coords: [" << line.m_location.neckRelativeRadialine.x << ", " << line.m_location.neckRelativeRadial[1] << ", " << line.m_location.neckRelativeRadial[2] << "]" << std::endl;
