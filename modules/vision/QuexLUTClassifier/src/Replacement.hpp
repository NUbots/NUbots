 ...
 * (4) Lexical Analyzer class Replacement and its memento class.
 * (5) Constructor and init core of Replacement.
...
#ifndef __QUEX_INCLUDE_GUARD__ANALYZER__GENERATED____QUEX____REPLACEMENT
#define __QUEX_INCLUDE_GUARD__ANALYZER__GENERATED____QUEX____REPLACEMENT
...
#   undef   __QUEX_INCLUDE_GUARD__ANALYZER__CONFIGURATION____QUEX____REPLACEMENT
#else
#   define  __QUEX_INCLUDE_INDICATOR__ANALYZER__MAIN
#endif
#define     __QUEX_SIGNAL_DEFINED_LEXER_IN_NAMESPACE___QUEX__

#include "Replacement-configuration.hpp"
...
QUEX_NAMESPACE_MAIN_OPEN 
    ...
    class  QUEX_TYPE0_ANALYZER;    /* Replacement */
...
#   line 4 "Replacement.qx"

    // Header stuff goes here

#   line 60 "Replacement.hpp"
...
#include "Replacement-token_ids.hpp"
#include "Replacement-token.hpp"

QUEX_NAMESPACE_MAIN_OPEN 

enum {
    QUEX_NAME(ModeID_ReplacementRules) = 0
};

        extern QUEX_NAME(Mode)  QUEX_NAME(ReplacementRules);


extern     __QUEX_TYPE_ANALYZER_RETURN_VALUE QUEX_NAME(ReplacementRules_analyzer_function)(QUEX_TYPE_ANALYZER*);
#ifdef QUEX_OPTION_RUNTIME_MODE_TRANSITION_CHECK
extern     bool QUEX_NAME(ReplacementRules_has_base)(const QUEX_NAME(Mode)*);
extern     bool QUEX_NAME(ReplacementRules_has_entry_from)(const QUEX_NAME(Mode)*);
extern     bool QUEX_NAME(ReplacementRules_has_exit_to)(const QUEX_NAME(Mode)*);
#endif
...
#   include <quex/code_base/analyzer/Engine_body>
    ...
    /* DISABLED */ Replacement(const Replacement&);
public:
    typedef QUEX_TYPE_TOKEN      token_type;
    typedef QUEX_TYPE_TOKEN_ID   token_id_type;
    typedef QUEX_TYPE_CHARACTER  char_type;

    Replacement(QUEX_TYPE_CHARACTER* BufferMemoryBegin, size_t  BufferMemorySize,
                QUEX_TYPE_CHARACTER* BufferEndOfContentP   = 0x0,
                const char*          CharacterEncodingName = 0x0,
                bool                 ByteOrderReversionF   = false);
    Replacement(const std::string&   Filename,       
                const char*          CharacterEncodingName = 0x0,
                bool                 ByteOrderReversionF   = false);
    Replacement(std::istream*        p_input_stream, 
                const char*          CharacterEncodingName = 0x0,
                bool                 ByteOrderReversionF   = false);
#   if defined(__QUEX_OPTION_WCHAR_T)
    Replacement(std::wistream*       p_input_stream,
                const char*          CharacterEncodingName = 0x0,
                bool                 ByteOrderReversionF   = false);
#   endif
    Replacement(std::FILE*           input_fh,       
                const char*          CharacterEncodingName = 0x0,
                bool                 ByteOrderReversionF   = false);
#   if defined(__QUEX_OPTION_UNIT_TEST)
    template <class UnderlyingStreamT>
        Replacement(quex::StrangeStream<UnderlyingStreamT>*  p_input_stream, 
                    const char*                              CharacterEncodingName = 0x0,
                    bool                                     ByteOrderReversionF   = false);
#   endif
...
private:
    ...
    friend     __QUEX_TYPE_ANALYZER_RETURN_VALUE QUEX_NAME(ReplacementRules_analyzer_function)(QUEX_TYPE_ANALYZER*);
#ifdef QUEX_OPTION_RUNTIME_MODE_TRANSITION_CHECK
    friend     bool QUEX_NAME(ReplacementRules_has_base)(const QUEX_NAME(Mode)*);
    friend     bool QUEX_NAME(ReplacementRules_has_entry_from)(const QUEX_NAME(Mode)*);
    friend     bool QUEX_NAME(ReplacementRules_has_exit_to)(const QUEX_NAME(Mode)*);
#endif
...
{
    __quex_assert(QUEX_NAME(ModeID_ReplacementRules) < 1);

     QUEX_NAME(ReplacementRules).id   = QUEX_NAME(ModeID_ReplacementRules);
     QUEX_NAME(ReplacementRules).name = "ReplacementRules";
     QUEX_NAME(ReplacementRules).analyzer_function = QUEX_NAME(ReplacementRules_analyzer_function);
#    if      defined(QUEX_OPTION_INDENTATION_TRIGGER) \
        && ! defined(QUEX_OPTION_INDENTATION_DEFAULT_HANDLER)
     QUEX_NAME(ReplacementRules).on_indentation = QUEX_NAME(Mode_on_indentation_null_function);
#    endif
     QUEX_NAME(ReplacementRules).on_entry       = QUEX_NAME(Mode_on_entry_exit_null_function);
     QUEX_NAME(ReplacementRules).on_exit        = QUEX_NAME(Mode_on_entry_exit_null_function);
#    if      defined(QUEX_OPTION_RUNTIME_MODE_TRANSITION_CHECK)
     QUEX_NAME(ReplacementRules).has_base       = QUEX_NAME(ReplacementRules_has_base);
     QUEX_NAME(ReplacementRules).has_entry_from = QUEX_NAME(ReplacementRules_has_entry_from);
     QUEX_NAME(ReplacementRules).has_exit_to    = QUEX_NAME(ReplacementRules_has_exit_to);
#    endif
        me->mode_db[QUEX_NAME(ModeID_ReplacementRules)] = &(QUEX_NAME(ReplacementRules));
...
#endif /* __QUEX_INCLUDE_GUARD__ANALYZER__GENERATED____QUEX____REPLACEMENT */
