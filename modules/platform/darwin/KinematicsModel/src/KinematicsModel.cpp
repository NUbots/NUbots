...
namespace darwin {
    ...
    void KinematicsModel::configureLeg (DarwinKinematicsModel::Leg& leg, const YAML::Node& objLeg) {
        ...
        DarwinModel::Leg::UPPER_LEG_LENGTH = leg.upperLegLength = objLeg["upper_leg_length"].as<Expression>();
        DarwinModel::Leg::LOWER_LEG_LENGTH = leg.lowerLegLength = objLeg["lower_leg_length"].as<Expression>();

        DarwinModel::Leg::HEEL_LENGTH = leg.heelLength = objLeg["heel_length"].as<Expression>();

        DarwinModel::Leg::LENGTH_BETWEEN_LEGS = leg.lengthBetweenLegs = leg.hipOffset[1] * 2;
        DarwinModel::Leg::FOOT_CENTRE_TO_ANKLE_CENTRE = leg.footCentreToAnkleCentre = objLeg["foot_centre_to_ankle_centre"].as<Expression>();

        auto& foot = leg.foot;
        auto& objFoot = objLeg["foot"];
        DarwinModel::Leg::FOOT_WIDTH = foot.width = objFoot["width"].as<Expression>();
        DarwinModel::Leg::FOOT_HEIGHT = foot.height = objFoot["height"].as<Expression>();
        DarwinModel::Leg::FOOT_LENGTH = foot.length = objFoot["length"].as<Expression>();
        DarwinModel::Leg::TOE_LENGTH = foot.toeLength = objFoot["toe_length"].as<Expression>();
    }

    void KinematicsModel::configureHead (DarwinKinematicsModel::Head& head, const YAML::Node& objHead) {
        DarwinModel::Head::CAMERA_DECLINATION_ANGLE_OFFSET = head.cameraDeclinationAngleOffset = objHead["camera_declination_angle_offset"].as<Expression>();
        ...
        DarwinModel::Head::NECK_LENGTH = neck.length = objNeck["length"].as<Expression>();
    ...
    void KinematicsModel::configureArm (DarwinKinematicsModel::Arm& arm, const YAML::Node& objArm) {
        ...
        DarwinModel::Arm::DISTANCE_BETWEEN_SHOULDERS = objArm["distance_between_shoulders"].as<Expression>();
        ...
        DarwinModel::Arm::SHOULDER_LENGTH = objShoulder["length"].as<Expression>();
        DarwinModel::Arm::SHOULDER_WIDTH = objShoulder["width"].as<Expression>();
        DarwinModel::Arm::SHOULDER_HEIGHT = objShoulder["height"].as<Expression>();
        DarwinModel::Arm::UPPER_ARM_LENGTH = objUpperArm["length"].as<Expression>();
        ...
        DarwinModel::Arm::LOWER_ARM_LENGTH = objLowerArm["length"].as<Expression>();
