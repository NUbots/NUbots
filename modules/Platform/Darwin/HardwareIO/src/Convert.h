...
namespace Darwin {
    class Convert {
        ...
        static constexpr double GYROSCOPE_CONVERSION_FACTOR = (1600 * (M_PI / 180.0)) / 512;
        ...
        static constexpr double GAIN_CONVERSION_FACTOR = 100 / 254;

        /// The angle is given as a value between 0 and 4095
        static constexpr double POSITION_CONVERSION_FACTOR = (2 * M_PI) / 4095;
        ...
        static constexpr float offset[20] = {
        ...
        float acceleronometer(const uint16_t value);
        float gyroscope(const uint16_t value);
        float voltage(const uint8_t value);
        float fsrForce(const uint16_t value);
        float fsrCentre(const uint8_t value);

        std::tuple<uint8_t, uint8_t, uint8_t> colourLED(uint16_t value);
        uint16_t colourLEDInverse(uint8_t r, uint8_t g, uint8_t b);

        float gain(const uint8_t value);
        uint8_t gainInverse(const float value);

        float servoPosition(const uint16_t value);
        uint16_t servoPositionInverse(const float value);

        float speed(const uint16_t value);
        float speedInverse(const float value);

        float torqueLimit(const uint16_t value);

        float load(const uint8_t value);

        float temperature(const uint8_t value);
    };
}
}
}
